...
2. **Kommunikationsdaten werden NICHT eingefroren** (`db_kunden.py::_KOMM_FELDER`: E-Mail, Telefon, Mobil, Fax, Ansprechpartner, die vier Versandschalter sowie `e_rechnung_aktiv`/`e_rechnung_version`/`leitweg_id`). Sie beschreiben nicht den Beleg, sondern den **Weg zum Empfänger**: Ändert ein Kunde nach der Rechnungsstellung seine Adresse, muss der Nachversand an die neue gehen. `kunde_fuer_beleg` überlagert den Snapshot deshalb **immer** mit dem Live-Stamm — auch bei Altbelegen, deren Snapshot die Felder noch mitführt. Ist der Kunde nicht auffindbar (`kunden_id` fehlt), bleibt der Snapshot stehen: lieber der alte Stand als eine still verlorene Adresse. Gelesen wird mit der **`firma_id` des Belegs** (`_kunde_kontakt`), weil der Kunde einer anderen als der aktiven Firma gehören kann.
   **Wer den Snapshot ersetzt, muss die `kunden_id` kappen**, sonst holt genau diese Überlagerung die echten Daten zurück.
3. **Neuer Belegtyp** → Spalte `kunde_snapshot TEXT DEFAULT ''` (DB-Schema-Regel: beide Stellen) und beim Erstdruck/Festschreiben über `db_belege.py::_snapshot_kunde_in_beleg` einfrieren.
4. **Neues personenbezogenes Kundenfeld** → in `db_kunden.py::_ANON_LEER_FELDER` (Anonymisierung leert es) **und** in `dsgvo_export.py::_STAMM_FELDER` (Auskunft zeigt es) aufnehmen. Ist es ein **Kommunikationsfeld**, zusätzlich in `_KOMM_FELDER` (siehe Punkt 2).
5. **Anonymisierung ist fristgebunden:** erst nach Ablauf von `firma.aufbewahrung_jahre` ab dem jüngsten Beleg (`frist_offen`); vorher nur „Verarbeitung einschränken" (Art. 18). Nie umgehen.
6. **DSGVO-Funktionen zentral:** Anonymisierung/Einschränkung/Frist in `db_kunden.py`, Auskunft (Art. 15/20) + Sammellauf-Protokoll (Art. 5 Abs. 2) in `dsgvo_export.py`. Protokolle **pseudonym** (Kundennr., keine Klarnamen). Ablage konventionsbasiert über `firma.dsgvo_pfad` (Firmenstamm → Pfade), Fallback `{Exportpfad}/{SUBDIR_DSGVO}`.
