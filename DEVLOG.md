...
- **Neuer Abschnitt** „Statusfarben in der Liste" (`<h4 id="belege-statusfarben">`) im Kapitel *Belege bearbeiten*, eingefügt direkt nach der Beschreibung von Suchfeld/Statusfilter/igL-Spalte und vor dem Hinweis zu Spaltenbreiten. Eine Farbtabelle im Format der bereits vorhandenen Tabelle aus dem Abschnitt *Postausgang-Fenster*: **Grau** = *Entwurf*, **Bernstein** = *offen*, **Grün** = erfolgreiche Endzustände (*angenommen*, *geliefert*, *abgerechnet*, *bezahlt*, *abgeschlossen*, *erfolgreich*), **Rot** = *storniert*/*Storno*.
- **Bewusst nicht gedoppelt:** Die *Bedeutung* der einzelnen Status steht bereits in den belegtypspezifischen Abschnitten (Angebote, Rechnungen, …) — der neue Abschnitt erklärt nur die Farbzuordnung und verweist dorthin. Ebenso wurde die Farbtabelle des Postausgangs **nicht** angefasst: Deren Statusfarben (`status_info/ok/error/muted`) sind unverändert geblieben, die Beschreibung stimmt weiterhin.
- **Ausnahme dokumentiert:** Ein Beleg, dessen zuletzt gedruckte Original-PDF nicht mehr zum aktuellen Datenstand passt, wird als ganze Zeile in roter Schrift dargestellt; dieser Hinweis hat Vorrang und die Statuszelle bleibt dann **ohne** Farbfläche. Als `div.warn` ausgezeichnet, mit der Handlungsempfehlung, den Beleg erneut zu drucken.
- **Gegen den Code verifiziert statt aus dem Gedächtnis geschrieben:** Die zehn Statuswerte stammen aus den `STATUS_LIST`-Klassenattributen der fünf Belegmodule, die Farbzuordnung aus `theme.py::_BELEG_STATUS_SEMANTIK`, die Hintergrundfarben aus den Palettenschlüsseln `status_*_bg` (`#eceef1` / `#fdecd2` / `#e6f4ea` / `#fbe4e2`) und die **angezeigten** Statusbezeichnungen aus `language.json`. Dabei zwei eigene Fehlannahmen korrigiert: englisch heißt `storno` **„Cancel"** (nicht „cancellation"), und deutsch werden *Entwurf* und *Storno* groß geschrieben.
- **Textfarben in der Doku-Tabelle** wurden gegenüber der App bewusst abgedunkelt (z. B. `#5a6068` statt `#999999`), damit die Zellen im Fließtext lesbar bleiben; die Hintergrundfarben entsprechen exakt der Palette.
- **Dateien:** `app/doku.de.html`, `app/doku.en.html`, `DOKU-TODO.md` (Punkt entfernt, Stand auf 2026-07-20 gesetzt).
- **Verifikation:** Prüfskript über alle vier Doku-Dateien (DE/EN in beiden Projekten): Abschnitt vorhanden ✓ · alle zehn Statusbezeichnungen in der jeweiligen Sprache genannt ✓ · `<tr>`/`<td>` ausgeglichen ✓ · kein Mojibake ✓ · alle vier Hintergrundfarben stimmen mit `LIGHT_PALETTE` überein ✓ · Verweisziele `#angebote`, `#rechnungen`, `#igl` existieren ✓.
- **Parallel gepflegt:** Derselbe Abschnitt wurde in `Order-Management-Multi` eingefügt (dort ebenfalls DE + EN), damit die Anwenderdoku beider Varianten synchron bleibt.

## 2026-07-20 15:43 — Design „Klar-geschäftlich" aus dem Mehrplatz-Ableger übernommen

- **Anlass (Walter):** Das Erscheinungsbild wirkte veraltet („sieht nicht modern aus"). Die Frage stand im Raum, ob dafür eine **.NET-Portierung** nötig sei — Antwort: nein, das Aussehen steckt zu über 90 % in `app/theme.py`; eine Neuentwicklung hätte 9–18 Monate gekostet und dieselbe Design-Arbeit trotzdem erfordert. Umgesetzt wurde die Umstellung zuerst im Mehrplatz-Ableger (`Order-Management-Multi`, dort abgestimmt über ein HTML-Mockup mit nachgebauten echten Fenstern) und anschließend hierher übernommen.
- **Übernahmeweg:** Kein Nachbau, sondern ein Patch der neun Design-Commits des Ablegers (Vergleich „Stand davor → Endstand", also inklusive aller Nachbesserungen und **ohne** die dort verworfenen Zwischenschritte). Vorher gemessen, wie weit die Projekte auseinanderliegen: `theme.py`, `konto_helper.py`, `beleg_dialoge.py`, `beleg_kette.py`, `mod_firma_pfade.py` waren **inhaltlich identisch** (0 Abweichungen ohne Zeilenende-Rauschen), die übrigen wichen nur an DB-Stellen ab (`rollback`/`leseabschluss` für PostgreSQL, KIS-Sidebar-Button) — **keine** Abweichung an einer der geänderten Design-Stellen. `git apply --check` lief für alle 10 Dateien konfliktfrei.
- **Gestaltung neu:** Indigo-Akzent `#4f46e5` (hell) / `#6259ea` (dunkel) statt Windows-8-Blau `#0078D7`; `2px groove`-Buttonrahmen (Windows-95-Ära) durch `1px solid` ersetzt; Radien 7–12 px statt 3–4 px; großzügigere Innenabstände; transparente Tabellenköpfe statt hellblauer Kästen (Office-2003-Optik); globale Schriftfamilie „Segoe UI Variable Text" (`theme.FONT_FAMILY`) samt app-weitem `setFont`; Zeilenhöhe der Beleglisten 34 px.
- **Neue Theme-Konventionen** (für künftige UI-Arbeit): `selection_bg`/`fg_on_accent` in getrennte `table_selection_*`/`menu_selection_*`-Paare aufgeteilt (Tabellenauswahl dezent, Menü-Auswahl kräftig); Property-Selektoren `QPushButton[primary="true"]` (hervorgehobene Hauptaktion, bisher nur „Neu" in den Beleglisten) und `QPushButton[compact="true"]` (kleine Buttons mit fester Größe); `status_cell_colors()` + `_BELEG_STATUS_SEMANTIK` färbt die Statuszelle der Beleglisten (grau=entwurf, bernstein=offen, grün=erfolgreiche Endzustände, rot=storniert/storno). Ein „stale"-Beleg behält Vorrang und bleibt roter Text ohne Farbchip.
- **Qt-Fallstricke, dokumentiert im Code:** `extra_rules` (nur DARK_PALETTE) wird **nicht erneut** durch `format_map()` interpoliert → dort nur Literal-Hex. Qt-QSS kennt **kein** `text-transform`/`letter-spacing` → Versalien für Tabellenköpfe aus dem Mockup bewusst weggelassen. Hartkodierte `QFont("Helvetica", …)` gewinnen immer gegen die Stylesheet-`font-family` → alle 15 Stellen auf `theme.FONT_FAMILY` umgestellt (`main.py`, `main_sidebar.py`, `beleg_dialoge.py`, `beleg_kette.py`).
- **Zwei Fehler aus dem Ableger, die hier bereits behoben ankommen:** (1) Der Zell-**Fokusrahmen** — Qt zeichnet bei `SelectRows` zusätzlich zur Zeilenmarkierung einen Rahmen um die zuletzt angeklickte Einzelzelle, der seit dem `::item`-Styling wie ein loses Eingabefeld wirkte → `outline: none`. (2) Das für die Zeilenhöhe eingeführte `QTableWidget::item { padding: 10px 10px }` nahm Zellen mit **eingesetztem Widget** (`setCellWidget`) die Fläche weg: Das `KontoZelleEdit` der MwSt.-Konten war real nur **9 px** hoch (bei 30 px Zeilenhöhe und `sizeHint()` von 24 px) → −5 px für eine 13-px-Textzeile, daher halb abgeschnittene Kontonummern. Item-Padding jetzt nur horizontal (`0px 8px`), Zeilenhöhe über `verticalHeader().setDefaultSectionSize(34)`. **Miterledigt** sind damit die drei weiteren Tabellen mit Zell-Widgets: Benutzerverwaltung (Rechte-Matrix), Sprachdatei-Generator und Firmenstamm → Einheiten.
- **Weitere Korrekturen:** aktiver Reiter im Firmenstamm war fast weiß auf fast-weißem Grund → Indigo-Hervorhebung wie in der Sidebar; `bg_main` war im Hellmodus identisch mit `bg_input` → Eingabefelder verschmolzen mit dem Hintergrund, jetzt dezent grauer Fensterhintergrund bei weißen Feldern; Gitterlinien waren komplett unsichtbar gesetzt → zurück auf dezent sichtbar samt Spaltentrenner im Kopf; Reiter „Pfade" wurde gestaucht → `QScrollArea` (Muster wie `mod_firma_ki.py`), SaveBar bewusst außerhalb; „…"-Suchbuttons einheitlich 20/22 px → 24 px.
- **Dateien:** `app/theme.py` (Kern), `app/main.py`, `app/main_sidebar.py`, `app/konto_helper.py`, `app/modul/beleg_liste.py`, `app/modul/beleg_dialoge.py`, `app/modul/beleg_kette.py`, `app/mod_firma_tabs/mod_firma_layout.py`, `app/mod_firma_tabs/mod_firma_pfade.py`, `app/mod_firma_tabs/mod_firma_anbindung_fibu.py`.
- **Verifikation:** `python -m ruff check app` → *All checks passed!* ✓ · `py_compile` über alle 10 Dateien ✓ · Headless-Smoke (`QT_QPA_PLATFORM=offscreen`): beide Paletten bauen fehlerfrei zu einem Qt-Stylesheet (hell 6028, dunkel 7189 Zeichen), `status_cell_colors()` liefert für alle vier Belegstatus gültige Farben ✓ · **Geometrie gemessen** statt geschätzt: MwSt.-Zellwidget 29 px → 15 px für eine 13-px-Textzeile, Suchbutton 24×24 px, Beleglisten-Zeile 34 px ✓ · `theme.py` ist jetzt **zeichengleich** mit dem Mehrplatz-Ableger, weitere Design-Änderungen lassen sich also künftig genauso patchen ✓.
- **Lehre aus dem Ableger (hier vermerkt, weil sie allgemein gilt):** `sizeHint()` ist **nicht** die tatsächliche Größe. Ein Test, der `sizeHint()` prüfte, meldete fälschlich „OK", während der Realfall zerquetscht war — bei Layout-Fragen immer die reale `height()`/`width()` nach `processEvents()` messen und das Ergebnis zusätzlich als Bild rendern.

## 2026-07-18 08:03 — Fehlende Übersetzungen für alle 10 Zusatzsprachen nachgezogen (145 Keys je Sprache)

- **Anlass (Walter):** „ziehe für alle sprachen die fehlenden Übersetzungen nach".
- **Ausgangslage:** In **allen zehn** Zusatzsprachen (bg, dk, es, fr, gr, it, nl, pt, se, si) fehlten dieselben **145** UI-Keys — im Wesentlichen die drei jüngsten Ausbaustufen: **Benutzerverwaltung/Login** (`benutzer.*`, `login.*`, `rechte.modul.*`, `rechte.stufe.*`, `msg.kein_recht*`), **Adressprüfung inkl. DSGVO-Attestierung** (`adresse.*`, `firma.adresspruefung.*`) sowie Restposten aus Firma-Löschen, Schlüsseldatei-Status und Locking (`firma.loeschen.*`, `firma.steuerung.keydatei*`, `msg.lock_*`, `col.lock_seit`). Zusätzlich waren **0** Keys veraltet (`ts > src_ts`), es blieb also nur die Fehlmenge.
- **Vorgehen (Skill `app-sprachuebersetzung`, Teil A):** Claude Code hat **selbst** übersetzt und geprüft — **keine** KI-API der App. Je Sprache: Vorwärts-Übersetzung → Rückübersetzung ins Deutsche → sinngemäße Prüfung, Einpflegen ausschließlich über die CLI (`apply` / `review` / `normalize`, abschließend `stamp`). Vor dem Übersetzen wurde die je Sprache **bereits etablierte Terminologie** aus den vorhandenen Keys abgeleitet (`beleg.singular.*`, `tab.*`, `col.*`, `btn.*`, `firma.tab.*`) und konsequent weitergeführt — z. B. Firmenstamm als `Anagrafica azienda` (it) / `Основни данни на фирмата` (bg) / `Företagsstamdata` (se), Beleg als `bilag` (dk) / `παραστατικό` (gr) / `comprobante` (es). Auch die je Sprache übliche Anredeform wurde beibehalten (usted/Lei/u-Form, dänisch/schwedisch du).
- **Singhalesisch (si) — offen gekennzeichnet:** Claude hat die eigene Sprachbeherrschung ehrlich auf **8/10** eingeschätzt, also **unterhalb** der 6er-Schwelle des Skills. Auf Rückfrage hat Walter die Standardvariante gewählt (**„trotzdem versuchen, aber deutlich kennzeichnen"**): alle **145** si-Einträge stehen in `language.si.review.json` mit `"bewertung": "gut"` und der Begründung *„Claude-Sprachbeherrschung unterhalb der 6/10-Schwelle — bitte muttersprachlich verifizieren."* Die übrigen neun Sprachen lagen innerhalb der Schwelle (fr/es/it/pt 2, nl 3, dk/se 4, gr/bg 5) und sind je Key mit echter Einstufung (`identisch` / `sehr_gut`) und, wo von einer wörtlichen Übertragung abgewichen wurde, mit Begründung hinterlegt (9–18 solcher Fälle je Sprache).
- **Bewusste Abweichung Schwedisch:** Für `msg.belegnr_konflikt`, `msg.gsjahr_abweichung` und `firma.loeschen.step_belege` wurde `dokument` statt des rein buchhalterischen `verifikat` gewählt, weil der Beleg auch ein Angebot oder Auftrag sein kann; im DEVLOG festgehalten, weil `col.belegnr` dort abweichend `Kvittonr.` heißt.
- **Dateien:** `app/language.{bg,dk,es,fr,gr,it,nl,pt,se,si}.json` + die zehn zugehörigen `*.review.json`; `app/language.json` (nur `ts`-Stempel für 3 Items durch `Sprachdatei.py stamp`, dazu je Sprache 3 `src_ts` nachgetragen).
- **Verifikation:** `Sprachdatei.py missing <code>` = **0** für alle zehn Sprachen ✓ · Prüfskript über alle 145 Keys × 10 Sprachen: **0** Abweichungen bei `{…}`-Platzhaltern (Anzahl je Marker) und `\n`-Umbrüchen, keine leeren Werte ✓ · `lang_tools`-Auswertung: offen = 0, veraltet = 0 in jeder Sprache ✓ · Idempotenz-Check (`normalize` zweimal, SHA-256 vorher/nachher gleich) für alle zehn ✓ · `python -m ruff check app` → *All checks passed!* (prüft `language.*.json` auf doppelte Keys) ✓.
- **Nebenbefund (nicht angefasst, außerhalb des Auftrags):** In den Bestandsdaten stehen einige unübersetzte oder fehlerhafte Altwerte, u. a. `pt: app.title = "Auftragsabwicklung"`, `dk: btn.speichern = "Gem't"`, `dk/it/gr/si: col.user = "User"`, `dk: col.hinweis = "Note"`, `fr: tab.kunden = "client"` (Singular/klein), `es: tab.buchungsexport = "Exportación de reservas"` (Hotel-„Reservierungen" statt Buchungssätze), `fr: tab.buchungsexport = "Export des Belegs"` (deutsches Restwort). Diese Keys gelten als „erledigt" und werden vom Generator nicht erneut angefasst — sie müssten gezielt nachgebessert werden.

## 2026-07-16 22:18 — Anwenderdoku nachgezogen: alle 11 DOKU-TODO-Punkte, DE+EN synchron, Admin-Readmes

- **Anlass (Walter):** „aktualisiere die Anwender doku"; auf Rückfrage Umfang **„Alles: DE + EN + Admin-Readme"**. `DOKU-TODO.md` ist danach **leer**.
- **`app/doku.de.html` — 7 Punkte nachgezogen:** Kapitel **Benutzerverwaltung** (Login-Dialog kommt immer + Windows-Anmeldung nur am eigenen Konto; neuer Abschnitt *Benutzer anlegen, deaktivieren, löschen* mit den drei Zuständen und dem Wiederherstellen über denselben Login, Anker `benutzer-zustaende` + Navigationseintrag; Rechtestufe **lesen** = Datensatz voll lesbar, Speichern ausgegraut, **keine Sperre** beim Ansehen; Firmenstamm-Reiter einzeln mit **34** Programmteilen inkl. Update-Hinweis; vier „Alle: …"-Buttons); **Firmenverwaltung** (Kopieren um Marken/Warengruppen/Einheiten/Sprachen/Länder/Konten erweitert, Artikel verweisen auf Marken der Kopie, Belege nicht „exportiert", Kopie einer gelöschten Firma ist aktiv, eigene Schlüsseldatei; Wiederherstellen reaktiviert nur kaskadierte Sätze; Hard-Delete-Optionen als Tabelle inkl. Schlüsseldatei-Löschung; erste/aktive Firma nicht löschbar; Dateien-Hinweis); **Start & Navigation** (neuer Abschnitt *Aktive Firma beim Start*, Firma je Benutzer + Meldung bei gelöschter Firma); **Belegnummern** (fünf Zähler inkl. Mahnung, „Nächste …-Nr." überspringt vergebene, Rückfrage beim Herabsetzen, automatisches Ausweichen bei Nummernkonflikt, GJ-Warnung nach Jahreswechsel).
- **Kapitel „Sperren-System" komplett neu geschrieben (DE+EN):** Die bisherige Fassung beschrieb ein **Konzept, das es nicht gibt** („Module sperren", Spalten *Modul/Gesperrt/Sperre bis*). Tatsächlich wird **je Datensatz** gesperrt (`lock_manager.py`); dokumentiert sind jetzt der atomare Lock bei gleichzeitigem „Bearbeiten", die Konflikt-Rückfrage des Firmenstamms statt einer Sperre, die echten Spalten (Tabelle, ID, User, Modul, **Gesperrt seit**, Anzahl der Änderungen, Geändert am), das Aufheben hängender Sperren (Admins) und der Weg über die Fehler-Nachverfolgung.
- **`app/doku.en.html` — vollständig auf den DE-Stand gebracht** (Rückstand vom 2026-07-14 + Benutzerverwaltung + die 7 neuen Punkte). Strukturell fehlten **11 Blöcke**, jetzt übersetzt: *Festschreiben & eingefrorene Werte*, *Digitale Signatur* (inkl. Vertrauens- und Kaufcheckliste), *Beleg-Archiv*, Kapitel *Benutzerverwaltung* (4 Unterabschnitte) und *Verschlüsselte Zugangsdaten*. Inhaltlich nachgezogen: Artikelstamm (Kategorie-Sidebar, Suche/Filter, Listen-Spalten, zweispaltiger Dialog, 3 KI-Rückmeldungen), Mahnungen (Pflicht-Steuerklasse, Druck-Layout, Festschreiben, Stornieren), E-Rechnung (Besonderheiten: Storno 381 positiv, Steuerbefreiung, Rundung, Fälligkeit, gelbe Fälle), Buchungsexport (Stornos negativ, eigene Mahnstufe, Cent-Ausgleich, Parallel-Schutz, „Wiederholen" ersetzt die Datei), Fehler-Nachverfolgung (4 fehlende Quellen + berechnete Werte), ZM (anzeige/widerruf-Kopplung, negative Zeilen/CSV-Limit, igL über Steuerschlüssel, USt-IdNr.-Normalisierung, deutsche USt-IdNr.), Drucktexte (read-only/zentral, Zwei-Quellen-Trennung), Geschäftsjahre, Steuerung (Archiv-Prüfjahre, Schlüsseldatei-Status), Adresse/Kunden (BIC/Bank aus IBAN), E-Mail (Versand-Vorgabe, fehlender Anhang stoppt Versand), Status-Übergänge Angebote/Aufträge, App-Sprachen („Nur Drucktexte", Kursiv-Legende, Übersetzungs-Protokoll).
- **Sachfehler in der EN-Doku behoben:** „API keys are stored **unencrypted in the database**" — seit DB v71 falsch und sicherheitsrelevant; jetzt „verschlüsselt in einer eigenen Datei je Firma". Ebenso das **entfernte** Kapitel *Grammar Errors in the Source Text* samt zugehörigem **SVG-Zweig** (gelber Kasten + Pfeil) gelöscht und der Phase-3-Text auf den aktuellen Ablauf gebracht (eine Rückübersetzung, Korrektur ohne Kontroll-Rückübersetzung, Ausgangstext wird nicht mehr geprüft).
- **Sachfehler in der DE-Doku behoben:** „Neue Belegnummer: Wird beim **ersten Druck** (nicht beim Speichern) vergeben" — widerspricht dem eigenen Kapitel *Belegnummern* und dem Code (`beleg_edit.py::_speichern` ruft `beleg_zahl_erhoehen` bei neuen Belegen). Korrigiert auf **beim Speichern**; die EN-Fassung war hier bereits richtig. Der fehlende *Entwurf-Status* wurde in EN ergänzt.
- **`Readme.admin.de.md` / `Readme.admin.en.md`:** Paketliste um die sechs fehlenden Einträge ergänzt (PyMuPDF, lxml, **pyhanko**, **cryptography**, schwifty, httpx) + Signatur-Hinweis; Wörterbuch-Installer deckt **alle** Sprachen aus `installed_languages.txt` (nicht nur DE+EN); neuer Abschnitt *Benutzer: erster Start und Rollout* (erster Start = Admin, weitere Arbeitsplätze erst nach Benutzeranlage, Anmeldearten, Passwort-Mails nur über Brevo/Gmail/SMTP) + Verweis in der Update-Sektion. Korrigiert: `ruff check app tools` → `ruff check app` (Hook prüft nur `app`, `tools/` ist ausgelagert) und die veraltete Schema-Angabe „v25" → Verweis auf `CURRENT_VERSION` (aktuell v77). Der API-Keys-Punkt war in der Readme bereits vollständig beschrieben.
- **Verifikation:** eigenes Prüfskript über beide Dokus ✓ — je **107 ids / 87 Anker-Links, identische id-Mengen**, wohlgeformt (HTMLParser), keine doppelten ids, keine toten `#`-Links, kein Mojibake, keine CJK-Zeichen; **HELP_ANCHOR-Abgleich**: alle **39** Werte aus `app/**/*.py` haben eine id in DE **und** EN ✓; Kapitel-Blockzahl DE/EN von 21 auf 3 erklärte Abweichungen reduziert (`firma`: „Speichern/Abbrechen" liegt in EN an anderer Stelle; `marker`: EN hat einen erklärenden Zusatzabsatz zu den deutschen Marker-Namen; `drucken-beleg`: DE führt „PDF-Aufbau" **und** „PDF-Inhalt" redundant — bewusst nicht nach EN dupliziert); beide Flussdiagramme per QtSvg zu PNG gerendert und **gesichtet** (EN jetzt strukturgleich zu DE, kein Quelltext-Kasten mehr); `python -m ruff check app` ✓. Alle Aussagen gegen den Code verifiziert (`rechte.py` = 34 Module, `session.py::MAX_FEHLVERSUCHE = 5`, `db_firma.py`, `lock_manager.py`, `email_direkt.py`, `db_belegzaehler.py`, `language.json`-Meldungstexte).
- **Zwei Fehler im DOKU-TODO selbst gefunden und codegetreu umgesetzt:** „nach 3 Fehlversuchen" (Code: **5**) und „Passwort-Mail immer mit der SMTP-Benutzeradresse" (gilt nur für **Gmail/SMTP**; **Brevo** nutzt die Firmen-Adresse — jetzt als Tabelle je Client dokumentiert).
- **Dateien:** `app/doku.de.html`, `app/doku.en.html`, `Readme.admin.de.md`, `Readme.admin.en.md`, `DOKU-TODO.md` (geleert).

## 2026-07-16 19:07 — Aufräumen: nicht mehr aktive Projektbestandteile nach `Backup-old/` ausgelagert

- **Anlass (Walter):** „Alles was nicht mehr aktiv zum Projekt gehört in das Verzeichnis `Backup-old` verschieben."
- **Prüfung vor dem Verschieben:** Für jeden Kandidaten wurde belegt, dass er inaktiv ist — die 14 Plan-Dokumente haben je einen Abschluss-Eintrag im DEVLOG (u. a. Benutzerverwaltung 2026-07-14 19:40, Locking 2026-07-14 12:05, ZM 2026-07-11 19:30, Schriften-LazyLoad 2026-06-29 07:14, API-Keys 2026-07-13 15:10); Code-Grep über `app/` fand keine Referenz auf die verschobenen Dateien; die verbliebenen Nennungen stehen ausschließlich in DEVLOG-Rückblicken (Historie, bewusst unverändert).
- **Verschoben nach `Backup-old/Plaene/` (16, waren getrackt):** `PLAN-Beleg-Archiv.md`, `PLAN-Belegnummern-Vergabe.md`, `PLAN-Benutzerverwaltung.md`, `PLAN-Drucktexte-Uebersetzung.md`, `PLAN-E-Rechnung.md`, `PLAN-Firma-Loeschen-Wiederherstellen.md`, `PLAN-KI-Prompts-verschlanken.md`, `PLAN-Locking-Review.md`, `PLAN-PDF-Druck-Signatur.md`, `PLAN-Schriften-LazyLoad.md`, `PLAN-Sprachdatei-Optimierung.md`, `PLAN-Zusammenfassende-Meldung.md`, `plan_adressvalidierung.md`, `plan_api_keys_verschluesselung.md`, `_claude_code_prompt_adressvalidierung.md` (Einweg-Spezifikation, Modul `app/address_validation.py` ist umgesetzt), `_Next.txt` (leer).
- **Verschoben nach `Backup-old/app/` (9, waren gitignoriert):** `app/language.{bg,dk,es,fr,gr,it,nl,se,si}.log.json` (1,4 MB Übersetzungs-Protokolle, laut DEVLOG 2026-07-09 18:31 Session-/Debug-Artefakt). Die `.gitignore`-Regel `app/language.*.log.json` bleibt — neue Läufe erzeugen die Dateien wieder.
- **Verschoben nach `Backup-old/Test/` bzw. `Backup-old/tools/` (waren gitignoriert):** `Test/` (`test_druck.py`, `check_data.py`, Stand Mai) und `tools/` komplett (`import_heima24.py`, `import_kontenrahmen.py`, `import_kunden_schauspieler.py`, `bp_korrektur.py`, `diagnose_bp.py`, `diagnose_konten_luecken.py`, `README.import_heima24.md`) — Walters Entscheidung; der heima24-Import ist abgeschlossen. Beide Verzeichnisse in der Wurzel entfernt.
- **Bewusst behalten:** `app/_alte_migrationen.py` (CLAUDE.md verweist ausdrücklich als Referenz darauf), `app/language.*.review.json` (der Sprach-Generator liest sie aktiv), `app/test_{address_validation,key_store,pruefung_parser}.py` (aktive Verifikationsskripte), `README.md` (GitHub-Einstieg), `EU-AI-Act-Pruefung.md` (Nachweisdokument), Startskripte, `ERROR.txt`, Caches.
- **`.gitignore`:** neuer Abschnitt `Backup-old/` — die Ablage bleibt lokal. Die zuvor getrackten Plan-Dateien wurden per `git rm --cached` aus dem Index genommen (im Repo also gelöscht, lokal vorhanden, in der Git-Historie vollständig erhalten).
- **Verifikation:** `python -m ruff check app` ✓ („All checks passed"); `python app/audit_firma_id.py` ✓ (Exit 0); Import-Smoke `import main, lang_tools, address_validation` ✓; `git check-ignore -v` bestätigt, dass `Backup-old/` greift; `git status` zeigt außer `.gitignore` nur die 16 Löschungen.
- **Dateien:** `.gitignore`, `DEVLOG.md` (+ Verschiebungen, s. o.).
...
- **DB v70** (beide Stellen laut STRENGER REGEL): `DB-Pflege.py::_to_v70` (CURRENT_VERSION 70, MIGRATIONEN-Eintrag, Header-Changelog) + `db/db_schema.py::_SCHEMA_SQL` — drei `firma`-Spalten `adress_provider` (Default `'nominatim'`), `adress_google_api_key`, `adress_nominatim_url` sowie globale Tabelle **`adress_attestierungen`** (bewusst ohne firma_id — Betreiber-Ebene wie db_version; append-only: nur INSERT/SELECT, Widerruf = neuer ungültiger Eintrag).
- **DB-Schicht `app/db/db_adress.py`** (neu, in `database.py` als `DBAdressMixin` eingebunden): `add_adress_attestierung` / `get_adress_attestierung_latest` + Adapter **`DbAttestationStore`** mit der Schnittstelle des JSON-`AttestationStore` (`attest`/`latest_valid` → `Attestation`), sodass `create_validator` unverändert bleibt.
- **Firmenstamm → Parameter → „Adressprüfung"** (`mod_firma_tabs/mod_firma_adresspruefung.py`, neu; registriert in `mod_firma_parameter.py`): Provider-Combo (nominatim/google, itemData = technischer DB-Wert), Nominatim-Basis-URL, Google-API-Key mit **Admin-Maskierung** nach dem `mod_firma_ki.py`-Muster (Nicht-Admins: Sterne + read-only, echter Wert nie ins Widget, beim Speichern unverändert zurück), Attestierungs-Status-Label (gültig: wer/wann; fehlend: rot nur bei gewähltem Google) und Button „Attestierung erfassen…" (nur Admins) → **`AttestierungDialog`** (DialogSizeMixin; Hinweistext „ersetzt keine rechtliche Prüfung", Checkboxen DPA/VVT mit Vorbelegung aus der letzten Attestierung, Erfasser-Anzeige; OK hängt neuen Eintrag an — auch als Widerruf).
- **Kundenstamm** (`modul/mod_kunden.py`): Button **„Adresse prüfen"** auf der Land-Zeile (BIC-Button-Muster: NoFocus, Hinweis-Label in eigener Formularzeile). `_adresse_pruefen`: AddressInput aus strasse/adresszusatz/plz/ort/land(ISO via currentData), ValidatorConfig aus der Firma, Vorab-Prüfung „nicht konfiguriert" (kein sinnloser HTTP-Call), synchroner Aufruf mit WaitCursor. Ergebnis: ACCEPT → Hinweis „bestätigt"; CONFIRM → Ja/Nein-Dialog mit erfasster ↔ standardisierter Anschrift, bei Ja Felder übernehmen (nur nicht-leere Werte; weitere address_lines → adresszusatz; Land via `_select_land_combo`; Dirty via Feld-Signale); REJECT → rotes Hinweis-Label mit i18n-Grund.
- **Modul-Ergänzung** `address_validation.py`: neues Feld `ValidationResult.reason` (`incomplete`/`no_match`/`unreachable`) als maschinenlesbarer Ablehnungsgrund fürs i18n-Mapping der UI (deutsche `notes` bleiben Log/Debug).
- **i18n** (`language.json`): Gruppen `adresse.*` (11 Keys: Button, Titel, bestätigt/übernommen/abgelehnt inkl. 3 Gründe, Übernahme-Frage, nicht konfiguriert) und `firma.adresspruefung.*` (14 Keys: Felder, Provider-Optionen, Attestierungs-Dialog/-Status, Reiter-Hinweis) + `firma.tab.adresspruefung`, DE+EN, alphabetisch eingeordnet.
- **Tests** (`test_address_validation.py`): +3 Tests für `DbAttestationStore` gegen In-Memory-SQLite mit dem echten `_SCHEMA_SQL` (attest/latest_valid, Widerruf append-only mit 2 verbleibenden Zeilen, Gate google/nominatim) → **22/22 bestanden**.
- **Verifikation:** `python -m ruff check app` grün; `py_compile` aller geänderten Dateien ok; `python app/audit_firma_id.py` → FEHLER: keine (nur bekannte Alt-Warnungen); JSON-Validierung `language.json` ok; App-Start (`python Order-Management.py`) → **Migration v70 automatisch gelaufen** (DB-Version 70, 3 Spalten, Tabelle vorhanden, read-only geprüft), App läuft fehlerfrei. GUI-Smoke-Test (Firma 990: neuer Reiter, Attestierung, Kundenstamm-Button) durch Walter. Hinweis: Starter heißt `Order-Management.py` (CLAUDE.md nennt noch `Auftragsabwicklung.py`).
- **DOKU-TODO:** neuer Punkt (Kundenstamm-Button + Firmenstamm-Reiter + Attestierung).

## 2026-07-13 13:43 — Adressvalidierung mit DSGVO-Gate (Modul, provider-agnostisch, ohne UI)

- **Anlass (Walter):** Für die Bestätigung von Kundenanschriften wird eine Adressprüfung gebraucht. Umsetzung nach der Spezifikation `_claude_code_prompt_adressvalidierung.md` (Plan: `_plan_adressvalidierung.md`): geprüft wird ausschließlich die **Anschrift** (kein Name/Firmenname, Datenminimierung); externer Dienst (Google) nur nach dokumentierter Betreiber-Attestierung, sonst automatisch datenschutzfreundlicher Fallback (self-hosted Nominatim) — Privacy by Default (Art. 25 DSGVO). **Nur das Modul + Tests**, keine UI-Integration (Kundenstamm/Firmenstamm) und keine DB-Änderung — folgt in einem späteren Plan.
- **Neues Modul `app/address_validation.py`** (Qt-/DB-frei, HTTP via **httpx** — Walter-Entscheidung laut Spezifikation, neue Dependency in `requirements.txt`):
  - **Modell:** `ValidationVerdict` (ACCEPT/CONFIRM/REJECT), `AddressInput` (ohne Name; `region_code` = ISO-2 wie `kunden.land`), `NormalizedAddress`, `ValidationResult` (verdict, provider, Flags `has_inferred/has_replaced/has_unconfirmed`, notes, raw nur Debug — nie loggen).
  - **Provider** (Protokoll `AddressValidator`): `GoogleAddressValidator` (validateAddress; Mapping als reine Funktion `_map_google_verdict`: nicht addressComplete oder Komponente UNCONFIRMED_AND_SUSPICIOUS → REJECT, Inferred/Replaced/Unconfirmed → CONFIRM, sonst ACCEPT; `possibleNextAction=FIX` nur als Verschärfungs-Override → REJECT; Netzfehler → REJECT „manuell prüfen") und `NominatimValidator` (self-hosted `/search` jsonv2, bewusst konservativ: Treffer → maximal CONFIRM, **nie ACCEPT**; kein Treffer/Fehler → REJECT). Beide mit injizierbarem `transport` für Tests (httpx.MockTransport).
  - **DSGVO-Gate:** `Attestation` (provider, confirmed_by, dpa_confirmed, vvt_confirmed, confirmed_at; `is_valid` = beide bestätigt) + `AttestationStore` (JSON append-only, „lock instead of delete", Widerruf = neuer ungültiger Eintrag; Default `app/daten/adress_attestierungen.json`; Schnittstelle schmal für späteren DB-Repository-Tausch). `create_validator(config, store)` mit Provider-Registry (`_PROVIDERS`): attestierungspflichtige Provider nur bei Key + gültiger Attestierung, sonst Nominatim; dritter Provider ohne Factory-Umbau ergänzbar.
  - **Audit-Wrapper `validate_address`:** loggt nur provider + verdict + Flags (`logging.getLogger("address_validation")`), niemals Anschrift oder `raw`. Beispiel unter `__main__` (Gate-Demo offline, echter Aufruf nur bei `NOMINATIM_BASE_URL`).
- **Tests `app/test_address_validation.py`** (Standalone + pytest-kompatibel, Muster `test_pruefung_parser.py`): 19 Tests — Google-Verdict-Mapping (ACCEPT/CONFIRM/REJECT/Suspicious/FIX-Override/Netzfehler), Gate-Verhalten (Akzeptanzkriterien 1+2, unvollständige Attestierung, fehlender Key, Widerruf/append-only, defekte Store-Datei), Nominatim nie ACCEPT (Mapping + MockTransport), Audit-Log ohne Anschrift.
- **Verifikation:** `python -m ruff check app` grün, `py_compile` beider Dateien ok, `python app/test_address_validation.py` → **19/19 bestanden**, Beispielaufruf `python app/address_validation.py` → Gate-Demo korrekt (ohne Attestierung nominatim, mit Attestierung google, nach Widerruf nominatim). Kein DOKU-TODO-Punkt (keine Anwender-UI-Wirkung).

## 2026-07-13 10:32 — App-Sprachübersetzung CLI-only: neuer Skill, `Sprachdatei.py review`, 10 Sprachen (1 neu, 9 aktualisiert)

- **Anlass (Walter):** Ein Claude-Code-Skill soll beschreiben, wie die App-UI-Sprachübersetzung abläuft, und beim Auslösen „übersetze Sprache <x>" automatisch angewendet werden. Auf Walters Präzisierung hin **ausschließlich über die CLI `Sprachdatei.py`** (kein Aufruf der KI-API der App) — Claude Code ist selbst Übersetzer und Prüfer.
- **Neuer Skill** `.claude/skills/app-sprachuebersetzung/SKILL.md` (gitignored, lokal): Teil A = ausführbarer CLI-only-Workflow (Sprache/Code klären → Sprachbeherrschungs-Schwelle ≥6/10 prüfen → offene/geänderte Keys ermitteln → vorwärts übersetzen → rückübersetzen + sinngemäß prüfen → über CLI einpflegen → verifizieren); Teil B = Referenz, wie der In-App-Generator übersetzt (Vorwärts/Rück/sinngemäße Prüfung), als Qualitätsmaßstab und Grundlage für Codearbeit an der Übersetzungspipeline. Enthält die Walter-Entscheidung „bei Sprachen unter der Kompetenzschwelle trotzdem übersetzen, aber mit `bewertung=\"gut\"` + Standardhinweis deutlich kennzeichnen" sowie einen Python-Helfer zum Auffinden **geänderter** (nicht nur fehlender) Keys, da `Sprachdatei.py missing` nur fehlende Keys liefert.
- **Neues CLI-Kommando `Sprachdatei.py review <code> <datei>`** (`Sprachdatei.py`): pflegt `language.<code>.review.json` (`ok`/`rueck`/`bewertung`/`begruendung`) über `lang_tools.schreibe_review` — merged mit bestehenden Einträgen, ergänzt `src_ts` aus dem aktuellen `language.json`-Zeitstempel. Damit markiert auch der CLI-Weg geprüfte Keys als erledigt (der In-App-Generator übersetzt sie danach nicht erneut).
- **10 Sprachen bearbeitet**, alle via `apply`/`review`/`normalize`/`stamp`, keine KI-API:
  - **fr, es, it, nl, dk, se** — je 34 offene Keys (15 fehlend: IBAN/BIC-Block, Bankverbindung, „Nur Drucktexte"; 19 geändert: MwSt.-Terminologie-Vereinheitlichung, igL-Tooltip, ZM-Fehlermeldungen). Vor dem Übersetzen bestehende „geändert"-Werte gegen die etablierte Terminologie geprüft statt blind neu übersetzt (surgical) — deckte echte Fehler auf: **Dänisch** hatte 8 Keys mit unübersetzt stehen gebliebenem Deutsch (`„MwSt-Klassen (Übersetzung)"` etc.) und einer falschen Abkürzung (`USt-Klasse`→`Momsklasse`); **Italienisch** nutzte `„KI"` statt `„IA"`; **Spanisch** hatte ein fehlendes `¿` und unübersetztes `„USt-IdNr"`. Niederländisch/Schwedisch bereits korrekt, nur bestätigt.
  - **bg, gr, si** — auf Walters Vorgabe trotz Sprachbeherrschung unter der 6/10-Schwelle übersetzt (bg: 14 fehlend + 2 geändert; gr/si: dieselben 34 wie oben), **alle Einträge mit `bewertung=\"gut\"`** (nie „identisch") + Standardhinweis „Claude-Sprachbeherrschung unterhalb der 6/10-Schwelle — bitte muttersprachlich verifizieren" markiert.
  - **pt (neu angelegt)** — vollständige Erstübersetzung, alle **1557 UI-Keys** (europäisches Portugiesisch, EU-Geschäftskontext). In 26 Modul-Batches erstellt (u. a. `firma.druck.*`, `firma.ki.*`, `firma.parameter.*`, `dlg.sprachdatei.*`, `msg.*`) mit konsistenter Terminologie (`IVA`/`categoria de IVA`, `NIF-IVA`, `entrega intracomunitária`, `fatura`/`orçamento`/`encomenda`/`guia de remessa`/`aviso de cobrança`, `RGPD`, `retrotradução`).
- **Verifikation:** je Sprache Vollständigkeit (`fehlend=0`/`geändert=0`), `{…}`-Platzhalter-Integrität (`lang_tools.marker_stimmig`, 0 Fehler bei allen 10), UTF-8 direkt in den Dateien stichprobenartig geprüft (keine Mojibake), `ruff check app` durchgehend grün. Für pt zusätzlich: exakter Abgleich der 1557 Keys gegen `language.json` (kein fehlender/überzähliger Key), Build-Skript (`build_pt.py`/`batches.py`) im Scratch-Verzeichnis validiert alle Übersetzungen vor dem Einpflegen.
- **`language.json`-Diff:** nur `ts`/`h`-Zeitstempel-Ergänzungen durch `Sprachdatei.py stamp` (keine de/en-Inhaltsänderungen).
- **Offen:** Wörterbuch-Installation (`spellcheck.py`/`Install_Woerterbuecher.py`) deckt die neuen/aktualisierten Sprachen noch nicht zwangsläufig ab (bestehender, unveränderter Zustand — kein DOKU-TODO-Punkt, da `app/doku.de.html` keine konkrete Sprachliste führt, siehe bestehender Eintrag vom 2026-06-27).
...
- **Phase 0** — neues `app/drucktext_keys.py`: explizite Mapping-Tabelle `txt_*` ↔ `druck.*` (62 Einträge, inkl. 8 Ausnahmen `druck.pos.*`/`druck.typ.*`/`jt_*`) + Sprachname→Code-Resolver `sprachcode_fuer_name` (Alias „Englisch"→„en").
- **Phase 1** — Einmal-Seed (Skript): effektive DE/EN-Drucktexte der Firma 990 nach `language.json` `druck.*` übernommen (12× DE, 28× EN; EN nur echte Übersetzungen, nicht die 7 unübersetzten deutschen Kopien). 3 englische Tippfehler (`No..`, `VAT ID No..:`, `VAT.`) dabei bereinigt; `{ss}`-Vereinfachungen der Summenzeilen als 990er-Stand übernommen. `ts`/`h` neu gestempelt (31 Items).
- **Phase 2** — `app/uebersetzung.py::_overlay_sprach_drucktexte` neu: baut `firma[txt_*]` aus `i18n.werte(code_ziel)` (gemappt), Fallback auf Firmensprache; `firma_drucktexte` für `txt_*` **nicht** mehr gelesen. Fallback-Kontext (`_fb_uebersetzt`) = die in der Zielsprache **echt** vorhandenen Keys (neuer Helfer `_vorhandene_druck_keys`); alle übrigen → `druck_basis._fb_protokoll` markiert gelb + ERROR.DB (Hinweis auf Sprach-Generator angepasst). Journale (`firma_mit_drucktexten`) laufen über denselben Weg firmensprachig.
- **Nicht betroffen:** `kond_*` (weiter aus `firma_drucktexte` via `_overlay_konditionen`), Einheiten, KI-Positionsübersetzung.
- **Konsequenz (bewusst, Model B):** Bis Phase 3 kommen Standard-Drucktexte **aller** Firmen aus dem globalen i18n (geseedet aus 990) — firmenspezifische Standardtext-Anpassungen in `firma_drucktexte` werden vorübergehend nicht mehr berücksichtigt (Wiederherstellung als echte Abweichungen in Phase 3; Datenbereinigung Phase 4).
- **Verifikation:** `ruff check app` ✓, `py_compile` ✓, `language.json` valide ✓. Mapping-Check: 62/62 im Druck genutzte `txt_*`-Keys gemappt, keine Lücke. Headless-Reproduktion `_overlay_sprach_drucktexte` (Firma 990): IT-Kundenkopie → italienische Label (`Fattura`, `Coordinate bancarie:`), Original DE unverändert, `_fb_ziel` nur bei Kundenkopie; FR (unübersetzt) → deutscher Fallback für alle 62 (würden gelb + ERROR.DB). **Offen (Walter/GUI):** echten it-Beleg drucken; **wichtig:** nach dem DE/EN-Master-Update die `druck.*` per „Nur Drucktexte" in alle Zusatzsprachen **neu übersetzen** (z. B. it `mwst_satz` noch alte `{ss}`-Fassung).
...
- **Ursache (per isoliertem Qt-Test verifiziert):** `QTableWidget.setRowCount(0)` löst die per `setCellWidget` gesetzten Zell-Widgets (Checkbox in COL_OK, Button in COL_AKTION) nur über `deleteLater()` aus. Während eines laufenden Batchlaufs (der Events pumpt, aber die DeferredDelete-Events nicht zwingend verarbeitet) blieben die alten Widgets an ihren früheren Pixelpositionen im Viewport sichtbar hängen und lagen — da neue Zeilen per `resizeRowToContents` andere Höhen haben — versetzt über den neuen Zeilen. Messung: nach `setRowCount(0)` blieben 5 von 5 Checkbox-Widgets im Viewport; im Folgelauf 6 statt 1.
- **Fix:** neue zentrale Methode `SprachdateiDialog._clear_table()` in `app/modul/mod_sprachdatei.py`: entfernt vor `setRowCount(0)` die Zell-Widgets beider Spalten (COL_OK, COL_AKTION) **explizit** mit `removeCellWidget` und nimmt sie via `setParent(None)` **sofort** aus dem Viewport (unabhängig vom `deleteLater`-Timing). Alle fünf Tabellen-Leerungen (`_on_combo` ×2, Quellsprachenwechsel, „Alle anzeigen"-Umschalter, Lauf-Start `_lauf`) rufen jetzt `_clear_table()` statt `setRowCount(0)`+`_row_index={}`.
- **Verifikation:** `ruff check app` ✓; `py_compile` ✓; **isolierter Qt-Test** (Nachbau) — `removeCellWidget`+`setParent(None)` → 0 verwaiste Widgets sofort; **Integrationstest mit der echten `_clear_table`** an einer QTableWidget mit Checkbox+Button je Zeile: nach dem Leeren (0 Checkboxen, 0 Buttons, rows=0, row_index leer), Folgelauf exakt (1 Checkbox, 4 Buttons) ohne Altlasten. In-App-Abnahme durch Walter (Sprach-Generator, mehrere aufeinanderfolgende Läufe) steht aus.
...
- **KRITISCH 1 — igL-Erkennung über den Steuerschlüssel:** `app/db/db_buchungsexport.py` — `zm_daten`/`zm_ohne_ust_id` matchten igL-Positionen bisher über die umbenennbare `mwst_klassen.bezeichnung`; nach Umbenennen der igL-Klasse fehlten Altbeleg-Umsätze **still** in der ZM, nach Löschen war die ZM **komplett leer**. Neuer gemeinsamer Kern `_zm_kern`/`_zm_igl_filter` matcht primär über den eingefrorenen `steuerschluessel` (inkl. gelöschter Klassen/Sätze, analog `druck_daten._mwst_klassen_map`), Bezeichnung nur noch als Zusatz-ODER für Ur-Altpositionen. Verifiziert per Headless-Smoke: igL-Umsatz bleibt nach Umbenennen und Soft-Delete der Klasse erhalten.
- **KRITISCH 2 — LKZ-Regeln nach SSB Tabelle 8:** `app/zm_elma_modell.py` — `DE` aus `ERLAUBTE_LKZ` entfernt (keine zulässige Meldezeile, nur der Unternehmer-Teil trägt die eigene USt-IdNr); eigener Validierungstext `zm.elma.err.lkz_de` statt generischem „unzulässiges LKZ". Neuer Helfer `pruefe_zeilen_lkz(daten)` sichert jetzt auch den CSV-/PDF-Pfad ab (`mod_zm._pruefe_lkz`, vor `_pdf`/`_csv`): bei Fund Warnung + Fallback-Protokoll (ERROR.DB), Ausgabe abgebrochen (kein stiller Fallback).
- **KRITISCH 3 — anzeige/widerruf typgebunden:** `app/modul/mod_zm.py` — `_on_typ_changed` aktiviert `anzeige` nur bei Quartal, `widerruf` nur bei Monat (SSB Tabelle 5, Backend-Fehlercode 17), Tooltips ergänzt. `zm_elma_modell.validiere` prüft zusätzlich als zweite Verteidigungslinie den Meldezeitraum-Code gegen die Flags.
- **MITTEL — USt-IdNr-Normalisierung:** neuer Helfer `_norm_ust_id` (Großschreibung, Leerzeichen/Punkte/Bindestriche entfernt) in `zm_daten`/`zm_ohne_ust_id`; verhindert doppelte ZM-Zeilen bei „DE 123…" vs. „DE123…" und entfernt innere Leerzeichen aus der CSV (ELSTER-Importregel).
- **MITTEL — Euro-Rundung + 0-Zeilen:** gemeinsamer Helfer `zm_gen.euro_betrag` (kaufmännisch auf Cent runden, dann Richtung Null kürzen — vorher `int(...)` mit Float-Artefakten) in `zm_gen.baue_zm_csv`, `zm_elma_modell.baue_modell` und `druck_journal.drucke_zm`; 0-€-Zeilen nach Rundung entfallen in allen drei Ausgaben. Negative Zeilensummen (Storno überwiegt) bleiben erlaubt, neuer nicht-blockierender Hinweis `zm_elma_modell.hinweise` vor dem ELMA-Export (Entscheidung Walter offen im Plan war „Hinweis reicht" — als Hinweis umgesetzt, nicht als Blocker).
- **MITTEL — CSV-Zeilenlimit:** `zm_gen.baue_zm_csv` wirft `ValueError` bei > `MAX_ZEILEN` (1500) Datenzeilen; `mod_zm._csv` fängt ihn ab und zeigt eine Meldung, statt still eine vom ELSTER-Import abgelehnte Datei zu erzeugen.
- **Optimierung — ein Query statt N+1:** `_zm_kern` lädt Rechnungen × Positionen × Kunden in einem JOIN (statt je Rechnung einen `get_rechnung_pos`-Aufruf) und aggregiert in Python; gemeinsamer Kern für `zm_daten`/`zm_ohne_ust_id` (Parameter `mit_ust_id`).
- **Kleinigkeiten:** `mod_zm._pdf` warnt jetzt bei leerer Datenmenge statt eine leere Tabelle zu drucken; Speicherdialoge (`_csv`, `_elma_xml`) starten im Firmen-Exportpfad (`settings.get_exportpfad`); Docstring/Kommentar von `ERLAUBTE_LKZ` auf SSB-Tabelle-8-Verweis korrigiert (DE/GB-Ausschluss begründet).
- **i18n:** neue Keys `zm.elma.err.anzeige_monat`, `zm.elma.err.lkz_de`, `zm.elma.err.widerruf_quartal`, `zm.hinweis.negative_zeilen`, `zm.msg.csv_limit`, `zm.tt.anzeige`, `zm.tt.widerruf` (de+en; ts/h per `Sprachdatei.py stamp` gesetzt).
- **Dateien:** `app/db/db_buchungsexport.py`, `app/zm_gen.py`, `app/zm_elma_modell.py`, `app/druck_journal.py`, `app/modul/mod_zm.py`, `app/language.json`, `DOKU-TODO.md`.
- **Verifikation:** `ruff check app` ✓; `py_compile` ✓; `audit_firma_id.py` ✓ (neuer JOIN firma-gefiltert, keine neuen Warnungen). **Headless-Smokes** (Testkopie der DB, Scratchpad, danach gelöscht; synthetische igL-Testrechnung in Firma 990 angelegt und wieder entfernt): (1) igL-Klasse umbenennen/soft-löschen → Umsatz bleibt in `zm_daten` ✓; (2) „DE 123 456 789"/„de-123.456-789" → normalisiert `DE123456789` ✓; (3) `euro_betrag(100.999999)=101`, `euro_betrag(0.40)=0`, CSV ohne 0-€-Zeile ✓; (4) ELMA-Validierung: DE-Zeile, Quartal+widerruf, Monat+anzeige → je ein Fehler, gültiges Modell → keine Fehler ✓; (5) CSV mit 1501 Zeilen → `ValueError` ✓. In-App-Abnahme durch Walter (Firma 990: ZM-Dialog Checkbox-Verhalten, PDF/CSV/ELMA-Erzeugung) steht aus.

## 2026-07-11 11:50 — Drucktexte-Übersetzung: 6 Fehler behoben + Wartung (Review)

- **Anlass (Plan `PLAN-Drucktexte-Uebersetzung.md`, Walters Entscheidungen):** Review des Programmzweigs „Übersetzen Drucktexte". Kernbefund: Der Reiter speichert alle Sprachen (inkl. Firmensprache) nur noch in `firma_drucktexte`, zwei Druckpfade bekamen dieses Overlay aber nie. Entscheidungen: Schritt 4c (Snapshot-Scan ZK/Mahnstufe) **nein** — (a)+(b) reichen; Schritt 8 (Batching) **nein** — weggelassen. Keine DB-Schema-Änderung.
- **FEHLER 1 — Overlay ohne KI:** `app/uebersetzung.py` — `bereite_firmensprache` und `uebersetze_beleg` führen das Drucktexte-/Einheiten-/Konditionen-Overlay jetzt **vor** dem `ki_aktiv`-Check aus (KI-frei); nur die KI-Übersetzung dynamischer Inhalte bleibt gated. Im Reiter gepflegte Firmensprache-Texte wirken damit auch ohne KI-Anbindung auf allen Belegen. Randfall: `mod_firma_drucktexte._save` speichert nicht mehr unter `sprache=''` (Hinweis-Meldung `firma.druck.keine_firmensprache` — Firmensprache zuerst im Reiter Adresse pflegen).
- **FEHLER 2 — Journal-Druck ohne Overlay:** neuer KI-freier Helfer `uebersetzung.firma_mit_drucktexten(db, firma)`; angewendet an allen drei `db.get_firma()`-Stellen in `app/druck_journal.py` (`_drucke_journal`, `drucke_buchungsbeleg_liste`, `drucke_zm`). Die Reiter-Gruppen „Journal-Spalten"/„Journal-Namen" (inkl. `txt_journal_typ_*`) wirken jetzt im Journaldruck. Andere Export-Pfade (dsgvo, ZM-XML) nutzen keine `txt_*`-Keys (geprüft per grep).
- **FEHLER 3 — Key-Lücken Reiter↔Druck:** Neue Reiter-Zeilen `txt_zinssatz_wert` (Beleginfo), `txt_zins_stufe` (Mahnung), `txt_journal_anzahl` (Journal-Spalten); `druck_beleg.py:431` nutzt statt hartkodiertem `"{stufe}:"` den neuen i18n-Default `druck.default.zins_stufe`. Tote Zeile `txt_pos_mwst` aus dem Reiter entfernt (im Druck seit Entfall der Steuersch.-Spalte ungenutzt; DB-Spalte/i18n-Keys bleiben).
- **FEHLER 4 — Eingefrorene Konditions-Bezeichnungen pflegbar:** `_rebuild_kond_rows` speist sich jetzt zusätzlich aus (a) Waisen in `firma_drucktexte` (neuer Getter `db_firma.get_firma_drucktext_kond_keys`) und (b) DISTINCT eingefrorenen `mwst_bezeichnung` aus allen 5 Positions-Tabellen (neuer Getter `db_belege.get_verwendete_mwst_bezeichnungen`, firma-isoliert per UNION). Umbenannte Klassen/früher gepflegte Bezeichnungen bleiben damit übersetzbar (Altbeleg-Nachdrucke). `save_firma_drucktexte`: Waisen werden bewusst nicht gelöscht (Code-Kommentar).
- **FEHLER 5 — „Übersetzen=aus" überschreibt nicht mehr:** `_uebersetze_sprache_core` Schritt 0 übernimmt den Firmensprache-Text nur noch, wenn das Zielsprach-Feld leer ist oder dem Firmensprache-Wert entspricht — manuell abweichend gepflegte Texte bleiben stehen (Tooltip `firma.druck.uebersetzen_chk_tt` präzisiert).
- **FEHLER 6 — Journal-Zeilen:** `txt_journal*`-Keys sind zentral über `_journal_key` von Gelb-Markierung, Unstimmigkeits-Filter und Vorwärts-/Rückübersetzung ausgenommen (Journale drucken nie in Kundensprache; `_fb_protokoll` nimmt sie schon aus). Die Gruppen „Journal-Spalten"/„Journal-Namen" werden in der Zielsprachen-Ansicht komplett ausgeblendet (analog Kond-Gruppen in der Firmensprache-Ansicht).
- **KLEIN/WARTUNG:** `_uebersetzen_zeile` sendet jetzt `system_marker=True` (wie der Haupt-Button — sonst gingen unersetzte `{Sprache …}`-Marker ans LLM); `_connect_dirty` verbindet Widgets nur noch einmal (Guard-Flag gegen Mehrfach-Signale bei wiederholtem `load()`).
- **i18n:** neue Keys `druck.default.zins_stufe`, `firma.druck.j_anzahl`, `firma.druck.zins_stufe`, `firma.druck.zinssatz_wert`, `firma.druck.keine_firmensprache` (de+en; ts/h stempelt der Generator).
- **Dateien:** `app/uebersetzung.py`, `app/druck_journal.py`, `app/druck_beleg.py`, `app/db/db_firma.py`, `app/db/db_belege.py`, `app/mod_firma_tabs/mod_firma_drucktexte.py`, `app/language.json`, `DOKU-TODO.md`.
- **Verifikation:** `ruff check app` ✓; `py_compile` ✓; `audit_firma_id.py` ✓ (neue UNION-Queries firma-gefiltert, keine neuen Warnungen); Import-Smoke `druck`+`main` ✓; **Headless-Smokes Firma 990** (Scratchpad, Testwerte danach zurückgesetzt): Overlay ohne KI in `bereite_firmensprache`/`uebersetze_beleg`/`firma_mit_drucktexten` ✓, Rechnungsbuch-PDF druckt `firma_drucktexte`-Werte inkl. `txt_journal_anzahl` ✓, Reiter offscreen: neue Zeilen da/`txt_pos_mwst` weg, Waise `kond_zk:WAISE-TEST` + eingefrorenes `kond_mwst:Voller Satz` erscheinen, Journal-Gruppen in Zielsprache ausgeblendet, Schritt 0 lässt manuell abweichenden Text stehen und füllt leere aus-Felder 1:1 ✓. In-App-Abnahme durch Walter (Firma 990: Reiter, Journal-Druck, Beleg + Kundenkopie) steht aus.

## 2026-07-11 10:11 — Buchungsexport (FiBu-Übergabe): 8 Fehler behoben + 3 Optimierungen (Review)

- **Anlass (Walter):** Review des Programmzweigs „Erstellung von Buchungen (Übergabe an die FiBu)" — 4 kritische Fehler (verfälschte FiBu-Zahlen bei Stornos/Mahnstufen), 4 mittlere, 3 Optimierungen. Plan komplett umgesetzt; Rundungs-Entscheidung Walters: Cent-Ausgleich an den gedruckten Rechnungsbetrag. Keine DB-Schema-Änderung.
- **KRITISCH 1 — EXTF-Storno-Vorzeichen:** `app/datev/extf.py::_buchungszeile` — Soll/Haben-Kennzeichen war fest `"S"` bei `abs(betrag)`; Stornorechnungen (negative Buchungen) wurden als normale Soll-Buchung exportiert (Forderung/Erlös verdoppelt statt storniert). Jetzt: negativ → Kennzeichen `"H"`, Umsatz positiv.
- **KRITISCH 2 — RDS-Storno-Vorzeichen:** `app/datev/rds.py::_ledger_xml_bytes` — `amount` war `abs(...)`, `consolidatedAmount` signiert (inkonsistent, Storno positiv). Jetzt `amount` vorzeichenbehaftet (XSD-Typ `p7` sieht das ausdrücklich vor).
- **KRITISCH 3 — Mahngebühren-Doppelbuchung:** `app/buchungsexport_gen.py::_buchung_mahnung` — Gebühr-Filter `startswith("Mahngebühr")` erfasste auch die auf Folgemahnungen mitgeführten Gebühren früherer Stufen (bereits mit ihrer eigenen Mahnung gebucht). Jetzt `startswith(f"Mahngebühr {stufe_bez}")` analog zum Zins-Filter.
- **KRITISCH 4 — Storno-Mahnungen:** (a) `_gruppe_brutto` ignorierte die Menge — `storniere_mahnung` negiert aber die Menge, die Storno-Mahnung wäre positiv gebucht worden; jetzt `menge * einzelpreis`. (b) `db_buchungsexport.py`: Mahnungs-Selektion verlangte `erstellungsdatum!=''` — Storno-Mahnungen (festgeschrieben, aber ungedruckt) erreichten den Export nie; neues Kriterium `_MAHNUNG_FINAL` = `(erstellungsdatum!='' OR festgeschrieben=1)`.
- **Konsistenz + N+1-Optimierung:** `_mahnung_hat_buchung` prüft jetzt stufengerecht (nur eigene Stufe, wie die Buchung selbst) und ist als SQL-Fragment `_MAHNPOS_EXISTS` (mit `_STUFE_BEZ_SQL`-CASE) direkt in die Queries von `unexportierte_belege`/`unexportiert_pro_periode`/`unexportiert_pro_periode_alle` integriert — statt 1 Query pro Mahnung bei jedem Fenster-Refresh.
- **Cent-Ausgleich Rundung:** `_buchung_rechnung` gleicht die je Steuerschlüssel-Gruppe gerundeten Bruttobeträge an den gedruckten Rechnungsbetrag an (`round(berechne_positionen(pos)[2], 2)`, gleiche Summenbildung wie der PDF-Druck); Differenz geht in die betragsgrößte Gruppe. Debitor-Soll == Rechnungsbetrag → offener Posten geht beim Zahlungsausgleich auf.
- **„Wiederholen" pflegt Protokoll:** neuer Helfer `db_buchungsexport.py::update_buchungsexport_datei`; `mod_buchungsexport.py::_wiederholen` löscht die alte Datei (best effort), stellt Protokoll auf die neue um, refresht die Tabelle. Vorher zeigte die Liste den alten Dateinamen und Undo/Storno löschte die falsche Datei (neue blieb als Duplikat liegen).
- **Undo erhält Storno-Festschreibung:** `delete_buchungsexport` setzt `festgeschrieben=0` nicht mehr für Storno-Mahnungen (per Design ab Anlage festgeschrieben) — SQL-CASE über `storno_von_mahnung_id`.
- **Parallel-Export-Schutz:** `save_buchungsexport` markiert Belege nur noch mit `... AND buchungsexport_id IS NULL` + `rowcount`-Prüfung; bei Konflikt (zweiter Benutzer exportiert dieselbe Periode) RuntimeError + Rollback der ganzen Transaktion. Abweichung vom Plan: Meldung deutsch in der DB-Schicht (Bestandsmuster wie die Storno-RuntimeErrors in `db_belege.py`; kein i18n-Import in db/), UI zeigt sie via bestehendem Fehler-Handler.
- **Weitere Optimierungen:** `rds.py::schreibe_rds` berechnet die Buchungssätze einmal für alle Belege (Gruppierung per Belegnummer) statt je Beleg die komplette Stammdaten-Ladung; `next_export_nr` per `MAX(CAST(substr(...)))` statt Probier-Schleife.
- **Altdaten-Prüfung (read-only, Scratchpad):** Betroffen ausschließlich **Testfirma 990** (1 RDS-Export mit Stornorechnung, 1 Mahnung Stufe 3 mit Fremd-Gebühr in Export BX2026-0002); keine Storno-Mahnungen ohne Export; keine Echtdaten betroffen → keine Korrektur an Produktivdaten nötig.
- **Dateien:** `app/buchungsexport_gen.py`, `app/db/db_buchungsexport.py`, `app/modul/mod_buchungsexport.py`, `app/datev/extf.py`, `app/datev/rds.py`, `DOKU-TODO.md`.
- **Verifikation:** `ruff check app` ✓; `py_compile` ✓; `audit_firma_id.py` ✓ (keine Fehler, nur bekannte Warnungen); Import-Smoke ✓; **Smoke-Skript** (17 Checks, synthetische Daten): Normalrechnung, 3 Rundungsfälle (Buchungssumme == PDF-Brutto), Stornorechnung negativ + EXTF „H"/positiver Umsatz, Mahnung Stufe 2 bucht nur eigene Gebühr/Zins, Storno-Mahnung negativ, RDS-Storno-XML XSD-valide mit signiertem amount/consolidatedAmount — alle ✓. In-App-Echtlauf in Firma 990 (Storno → Export → Datei prüfen) steht bei Walter aus.
- **Nacharbeit (gleicher Tag):** `update_buchungsexport_datei` führt beim „Wiederholen" optional auch `summe_soll`/`summe_haben` nach (sonst zeigte das Protokoll nach einer Berechnungsänderung alte Summen); `_wiederholen` übergibt die neuen Summen. Beide Firma-990-Exporte per Headless-„Wiederholen" (Scratchpad, gleiche Mängel-Prüfungen wie die App) mit korrigierter Logik neu erzeugt: BX2026-0002 Summe 298,73 → 293,73 (genau die doppelt gebuchte 5,00-€-Zahlungserinnerungs-Gebühr entfällt), BX2026-0001 RDS-ZIP jetzt mit konsistent negativem Storno (`amount` == `consolidatedAmount` == −20000,00; vorher amount +20000 bei consolidatedAmount −20000); alte ZIPs ersetzt, Drucklisten neu erzeugt, Protokoll aktualisiert.

## 2026-07-10 23:15 — KI-Prompts verschlankt: Mini-System-Prompt, Regeln in die Task-Prompts (DB v67)

- **Anlass (Plan `PLAN-KI-Prompts-verschlanken.md`, Walters Entscheidung):** Der Übersetzungs-System-Prompt (Firma 990: 1126 Zeichen) war größtenteils redundant zu den Task-Prompts und zu klein für Anthropic-Prompt-Caching. Er schrumpft auf einen Rollen-Satz („Du bist ein Fachübersetzer für kaufmännische Dokumente …"); die zwei Regeln, die NUR dort standen, wandern zwingend in die drei Übersetzungs-Prompts: die **`@@FEHLER@@`-Anweisung** (daran hängt `_ist_uebersetzung_unmoeglich` + Retry) und die **Unveränderlich-Liste** (USt-IdNr., IBAN, BIC, Leitweg-IDs, E-Mail-Adressen, URLs, Firmen-/Personennamen). Zusätzlich wandert der Terminologie-Satz („kaufmännische Fachterminologie der Zielsprache") als eine Regel in die Task-Prompts; Redundantes entfällt ersatzlos (Nur-Übersetzung-ausgeben, Struktur-erhalten, obsolete `{so_wie_dieser}`-Regel).
- **`app/ki_client.py`:** Konstanten `SYSTEM_PROMPT`, `UEBERSETZUNG_PROMPT`, `RUECKUEBERSETZUNG_PROMPT`, `MASSEN_UEBERSETZUNG_PROMPT` neu gefasst (Default-Struktur `## Was du nicht machen darfst!` und `#Nummer:`-Batch-Format beibehalten; Massen-Prompt: `@@FEHLER@@` je Item format-konform als `#Nummer: @@FEHLER@@`). Versehentlich doppelte Abkürzungs-Zeile im Rück-Prompt dabei bereinigt.
- **`app/DB-Pflege.py`:** `CURRENT_VERSION = 67`, `_to_v67` nach v64/v65-Muster — hebt Bestandsfirmen **nur bei exaktem Treffer** bekannter Alt-Fassungen (v45-/ki_client-Defaults + Firma-990-Fassungen, als Snapshots eingebettet); individuell angepasste Prompts bleiben unangetastet und funktionieren weiter (beide Fehler-Signale werden erkannt). Keine Schema-Änderung (`db_schema.py` unverändert, Spalten `DEFAULT ''`, `create_firma` seedet aus den ki_client-Konstanten). `ki_prompt_uebersetzung_retry` ist im Code ungenutzt (nur noch DB-Spalte) → unangetastet. Kein Eingriff in `uebersetzung.py` — die `{Sprache …}`-Ersetzung im `system_marker`-Pfad ist beim markerlosen Mini-Prompt ein No-op.
- **Token-Bilanz (Firma 990, System+Task je Aufruf):** Vorwärts/Rück −716 Zeichen, Massen −702 Zeichen ≈ **−200 Input-Tokens je Übersetzungs-Aufruf**. Die ki_client-Defaults wachsen netto (+390…+480), weil sie FEHLER-/Unveränderlich-Regeln bisher gar nicht enthielten (gewollter Funktionsumzug).
- **Dateien:** `app/ki_client.py`, `app/DB-Pflege.py`, `DOKU-TODO.md`.
- **Verifikation:** `ruff check app` ✓; `py_compile` ✓; **Migrations-Smoke auf DB-Kopie im Scratchpad** (30 Checks): Firma 990 → Mini-System-Prompt + `@@FEHLER@@` + IBAN/Namen-Liste + Terminologie-Satz in allen drei Task-Prompts, alle `{…}`-Marker erhalten; simulierte Firma mit individuellem Prompt unverändert; Firma mit altem ki_client-Default gehoben; 2. Lauf idempotent ✓. Headless-Prompt-Smoke: `baue_prompt` ohne verwaiste Marker, System-Prompt-Marker-Ersetzung No-op, Leerer-Kontext-Verhalten unverändert ✓. `audit_firma_id.py` ✓. Echt-DB migriert regulär beim nächsten App-Start; In-App-Prüfung (Übersetzungstest Firma 990, `@@FEHLER@@`-Pfad, Sprach-Generator, Token-Verbrauch vorher/nachher) steht aus.
...
- **Schritt 1 — Festschreiben erst NACH erfolgreichem PDF-Bau:** `druck_beleg.py::_drucke_beleg_intern` — `besterstand`/`erstellungszeitpunkt` werden weiterhin vor dem Rendern berechnet (Wert steht im PDF), aber alle DB-Schreibvorgänge (`save_erstellungsdatum`, `beleg_entwurf_bestaetigen`, `save_festgeschrieben`, `save_mahnung_snapshot`, `save_kopf_snapshot`) laufen erst nach erfolgreichem `_merge_pdfs` (direkt vor `_save_beleg_snapshot`). Schlägt der PDF-Bau fehl, bleibt der Beleg unfestgeschrieben/Entwurf; der nächste Druck ist wieder regulärer „erster Echtdruck" (E-Rechnung/Snapshots laufen dann). Bisher: festgeschriebene Rechnung ohne PDF, E-Rechnung entfiel dauerhaft.
- **Schritt 2 — Temp-Dateien ins Zielverzeichnis:** `druck_pdf_utils.py` — in `_testdruck_watermark`, `_overlay_lieferanschrift`, `_fix_page_numbers`, `_draw_folgeseite_hint` jetzt `tempfile.mkstemp(…, dir=os.path.dirname(pfad) or None)` + Aufräumen im `finally` (Muster wie `pdf_signatur.signiere_pdf`). Vorher scheiterte `os.replace` unter Windows, sobald der Ausdrucke-Pfad auf einem anderen Laufwerk/Netzlaufwerk lag.
- **Schritt 3 — igL-Prüfung + Steuerhinweise über den Steuerschlüssel:** `druck_daten.py` neu `_mwst_klassen_map` (lokale Kopie von `e_rechnung._klassen_info`; Druck bleibt unabhängig): Mapping `steuerschluessel → {igl, hinweis_text}` inkl. gelöschter Klassen/Sätze (Altbelege, aktive haben Vorrang) + Bezeichnungs-Map der aktiven Klassen als Zusatz-ODER. `_pruefe_igl_voraussetzungen` und `_sammle_steuerhinweise` matchen primär über den in den Positionen eingefrorenen `steuerschluessel` — eine umbenannte igL-Klasse hebelt die Druck-Blockade nicht mehr aus, der Pflicht-Steuerhinweis bleibt erhalten.
- **Schritt 4 — Robustheit + Logo-Fallback-Tracking:** `druck_daten.py::_lade_beleg_daten` sichert `get_artikel_by_id` gegen `None` (verwaiste `artikel_id` → wie „ohne Artikel" weiterdrucken statt TypeError). Logo nach Fallback-Tracking-Regel: fehlende Datei (`druck_basis.py::_get_logo_path`) bzw. nicht ladbares Logo (`druck_beleg.py::_header_firma`) → `fallback_log.melde(modul="Druck/Logo", …)` (ERROR.DB, firmennr-bezogen) statt nur stderr; Druck läuft ohne Logo weiter, keine Gelb-Markierung (Logo entfällt ersatzlos). **Zusatzbefund (gemessen):** `RLImage` mit festen width/height liest die Datei erst beim `doc.build` — der except-Zweig (auch der alte stderr-Print) wurde bei kaputter Datei nie erreicht, der Druck crashte im Build; jetzt sofortige Validierung via `ImageReader(logo_path)`, damit der Fallback-Zweig wirklich greift.
- **Schritt 5 — Wartung/Dedup:** `_build_pdf` auf Ein-Build + `_fix_page_numbers` vereinfacht — der `_afterBuild`-Zweig war auf Standard-ReportLab toter Code (Kwarg wirft immer TypeError); dasselbe tote try/except-Muster auch in `druck_journal.py` (3×) und `dsgvo_export.py` (1×) bereinigt; `_after_build` aus `druck_pdf_utils.py` + `druck.py`-Fassade entfernt. `_erstelle_pdf`: stilles `**extra` entfernt (Tippfehler-Kwargs knallen; alle `_BELEG_CFG`-extra_kwargs sind echte Parameter). `_header_firma`: ungenutzte Parameter (`belegtyp`, `belegnr`, `datum`, `lieferdatum`, `erstellungszeitpunkt`) + ungenutztes `ST` bereinigt, Aufrufstelle angepasst. `pdf_signatur.signiere_pdf`: Seitenzahl aus dem Page-Tree des pyHanko-Writers (`/Pages`→`/Count`) statt zusätzlichem fitz-Import.
- **Dateien:** `app/druck_beleg.py`, `app/druck_pdf_utils.py`, `app/druck_daten.py`, `app/druck_basis.py`, `app/druck_journal.py`, `app/dsgvo_export.py`, `app/druck.py`, `app/pdf_signatur.py`, `PLAN-PDF-Druck-Signatur.md`, `DOKU-TODO.md`.
- **Verifikation:** je Schritt `ruff check app` ✓ + `py_compile` ✓; `audit_firma_id.py` ✓ (unverändert, nur bekannte Warnungen). Headless-Smokes in Firma 990 (nur Lesezugriff, RecorderDB blockiert Schreibzugriffe): `_merge_pdfs`-Monkeypatch wirft → Fehler propagiert, kein `save_*` vorher; Render-Smoke 1-seitig + 17-seitig (Overlay/Watermark/Seitenzahl-Marker „17 - 17"/Folgeseiten-Hinweis) ✓, keine tmp-Reste; igL-Klasse mit verfälschter Bezeichnung → Hinweis + Blockade via Steuerschlüssel, Nicht-igL unblockiert, Bezeichnungs-Zusatz-ODER greift; `get_artikel_by_id`→None ohne TypeError; Logo fehlend/kaputt → je genau eine melde-Meldung + Weiterdruck; Tippfehler-Kwarg wirft TypeError; `/Count`==fitz-Seitenzahl; komplette Signatur mit echtem Firma-990-Zertifikat ohne fitz ✓. In-App-Tests (Echt-/Testdruck, Journal) bei Walter.

## 2026-07-10 12:50 — E-Rechnung: 6 Korrektheits-Fixes + Wartung (Review Erstellungspfad)

- **Anlass (Walter):** Review des E-Rechnungs-Erstellungspfads auf Fehler und Optimierung (Plan `PLAN-E-Rechnung.md`, 7 Schritte, schrittweise abgenommen). Keine DB-Schema-Änderung.
- **Schritt 1 — Rundungskonforme Summen (BR-CO-10/13/17):** `ubl_2_1.py` neu `_zeilen_netto` + `_summen` — jede Zeile wird auf 2 Nachkommastellen gerundet, derselbe Wert steht in `LineExtensionAmount`/`LineTotalAmount` **und** in der Summenbildung; MwSt je Satz-Gruppe aus der gerundeten Bemessungsgrundlage. Beide Generatoren ersetzen `helpers.berechne_positionen` (bleibt app-weit unverändert für PDF/Anzeige). Vorher konnten Zeilensumme und Gesamt-Netto um Cent differieren (invalide XML); dafür kann die XML jetzt in seltenen Fällen um 1 Cent vom PDF abweichen (EN 16931 hat Vorrang, bewusste Entscheidung).
- **Schritt 2 — igL über den Steuerschlüssel:** `ubl_2_1.py` neu `_klassen_info` (Mapping `steuerschluessel → {igl, hinweis_text, bezeichnung}`, gelöschte Klassen/Sätze für Altbelege mit aufgenommen) + `_ist_igl` (primär eingefrorener Steuerschlüssel der Position, Bezeichnungs-Gleichheit nur noch als Zusatz-ODER für Altbestand). Ersetzt die `igl_bez`-Sets in beiden Generatoren — eine **umbenannte** igL-Klasse verliert die Kategorie „K" nicht mehr (Konvention „Steuerschlüssel = stabiler Schlüssel", vgl. Buchungsexport).
- **Schritt 3 — Befreiungsgrund für Kategorie „E" (BR-E-10):** 0 %-Gruppen ohne igL erhalten `TaxExemptionReason` (UBL) bzw. `ram:ExemptionReason` vor `BasisAmount` (CII); Text = Hinweistext der Klasse, leer → `EXEMPT_REASON_DEFAULT` („Steuerbefreite Leistung"). Neuer Prüfpunkt in `__init__.py::_pruefe_und_protokolliere_fallbacks`: 0 %-Klasse ohne Hinweistext → ERROR.DB + Sidecar (gelbe Spool-Zeile), je Steuerschlüssel einmal.
- **Schritt 4 — Fälligkeit aus dem `kopf_snapshot`:** `_faelligkeit` liest zuerst den beim Festschreiben eingefrorenen Wert (`falligkeit`, ISO; Format-Absicherung via neuem `_faellig_zu_iso`, TT.MM.JJJJ wird konvertiert) — `DueDate` bleibt nach nachträglicher Änderung der Zahlungskondition beim festgeschriebenen Wert (Belegkonstanz); ohne Snapshot Live-Berechnung wie bisher. CII via `_u._faelligkeit` automatisch mit abgedeckt.
- **Schritt 5 — Storno: 381 + positive Beträge; CII-Referenz:** Neu `_storno_positiv` (dreht die negiert gespeicherten Storno-Mengen fürs XML zurück, DB unangetastet) — alle Beträge positiv, Gutschrift allein über TypeCode 381 (**Walter-Entscheidung**, bewusste Abweichung vom gedruckten Negativ-PDF). CII: Storno-Referenz vom falschen `ram:BuyerOrderReferencedDocument` (BT-13) ins `ram:InvoiceReferencedDocument` (BG-3) im Settlement nach der MonetarySummation verschoben, jetzt inkl. `FormattedIssueDateTime` (Format 102, neuer `qdt`-Namespace). UBL-`BillingReference` unverändert.
- **Schritt 6 — Validator + Pflichtfelder:** `validator.py::_bestimme_validation_type` repariert (CreditNote-Erkennung prüfte nur die XML-Deklaration; jetzt `crossindustryinvoice`→cii, `<creditnote`→credit, sonst ubl). Drei neue Fallback-Prüfpunkte: Firmenname leer (BT-27), USt-IdNr **und** Steuernummer leer (BR-CO-26), XRechnung ohne Telefon (BG-6) — je ERROR.DB + gelbe Spool-Zeile.
- **Schritt 7 — Wartung/Dedup:** `__init__.py` neu `spool_basis(firma)` (Pfad-Auflösung Ablage+Suche+Spool-Ansicht aus EINER Funktion; genutzt von `erzeuge`, `finde_vorhandene`, `mod_e_spool._spool_dir`); `erzeuge()` nutzt `_ist_aktiv_fuer_kunde()` statt Inline-Duplikat; toter Parameter `waehrung` aus `cii_d16b._add_line_item` entfernt; veraltete Docstrings (`erzeuge`-Raises, `ubl_2_1`-Modulkopf) korrigiert; verwaisten `settings`-Import in `mod_e_spool.py` entfernt.
- **Dateien:** `app/e_rechnung/__init__.py`, `app/e_rechnung/ubl_2_1.py`, `app/e_rechnung/cii_d16b.py`, `app/e_rechnung/validator.py`, `app/modul/mod_e_spool.py`, `PLAN-E-Rechnung.md`, `DOKU-TODO.md`.
- **Verifikation:** je Schritt `ruff check app` ✓ + `py_compile` ✓; `audit_firma_id.py` ✓; Headless-XML-Smokes in Firma 990 (nur Lesezugriff): Rundungsfall 3×10,005 (Zeilen==Summe), igL trotz umbenannter Klasse + Regression Nicht-igL nur „S", Kategorie-E-Reason (Default + Fallback-Label) inkl. igL-/19 %-Gegenproben, DueDate aus Snapshot (ISO/TT.MM.JJJJ/leer→live), echter Storno RE2026-0006 (alle Beträge positiv, 381, Referenz RE2026-0001 mit Datum im Settlement), Validator-Heuristik (ubl/cii/credit/381), `spool_basis`==alte Inline-Logik, Pflichtfeld-Checks positiv+negativ. Manuelle Tests (ITB-Validierung, gelbe Spool-Zeilen, Storno-XML) je Schritt bei Walter.
...
- **Schritt 1 — `hat_fallback` als DB-Spalte (DB v66):** `email_versand.hat_fallback INTEGER DEFAULT 0` in `app/db/db_schema.py` + `app/DB-Pflege.py` (`_to_v66`, CURRENT_VERSION=66) inkl. **Backfill** aus `meta._fallback` der vorhandenen JSON-Dateien (fehlende/defekte Dateien bleiben 0). `app/email_gen.py` schreibt das Flag beim Anlegen; `app/modul/mod_emails.py::_email_hat_fallback` liest nur noch die Spalte — der Postausgang-Refresh öffnet **keine JSON-Datei je Zeile** mehr.
- **Schritt 2 — Query-Optimierungen:** `app/db/db_emails.py` neu: `hat_email_fehler(firma_id)` (`SELECT EXISTS`), Einzel-Getter `get_email_versand(id_)` (kunde_name-JOIN, `id=? AND firma_id=?`), `get_email_versand_liste(…, beleg_typ=None)` (Typ-Filter in SQL). `mod_emails.py`: `_refresh` ohne Voll-Query fürs Fehler-Label und ohne Python-Typ-Filter; `_sel_row` per Einzel-Getter statt Liste + Linearsuche. Bewusster Randeffekt: „Öffnen" funktioniert jetzt auch im „Gelöscht"-Filter (Einzel-Getter liefert auch soft-gelöschte Zeilen); Versand gelöschter Einträge bleibt durch die `geloescht`-Prüfung im neuen Versand-Kontext (Schritt 3) unterbunden.
- **Schritt 3 — Sender-Dedup (`app/modul/email_provider_mixin.py`, 714→630 Zeilen):** gemeinsame Helfer `_versand_kontext` (Row via `get_email_versand` + JSON laden + `geloescht`-Guard; Lesefehler → Status „fehler"), Modul-Funktion `_empfaenger_betreff` (5× duplizierter Betreff-Default), `_versand_erfolg`/`_versand_fehlgeschlagen` (Status+JSON zentral; Dialoge bleiben provider-spezifisch an den Aufrufstellen). Gmail und generisches SMTP teilen sich den neuen `_smtp_kern` (MIME-Aufbau, TLS-Modi, Login, Statuspflege); `_gmail_senden`/`_smtp_senden` sind nur noch Konfig-Prüfung + Kern-Aufruf mit eigenen i18n-Keys. Verhaltenserhaltend (gleiche Meldungen/Keys).
- **Schritt 4 — Kleinigkeiten:** `_brevo_senden` liest die API-Antwort und speichert `messageId` als `brevo_message_id` in der E-Mail-JSON (`_json_status_setzen`/`_versand_erfolg` um `extra=` erweitert, ein Schreibvorgang); toter `"lieferschein"`-Eintrag aus `_NR_FELD` in `email_gen.py` entfernt.
- **Verifikation:** je Schritt `ruff check app` ✓ + `py_compile` ✓; `audit_firma_id.py` ✓ (neue Queries firma-gefiltert); Import-Smoke Mixin ✓ (Betreff-Default „Rechnung RE-1"). Manuelle Tests (Migration auf v66, gelbe Zeilen nach Backfill, Filter, Versand je Client, `brevo_message_id`) je Schritt bei Walter. Kein DOKU-TODO-Punkt (rein interne Optimierung).

## 2026-07-10 06:48 — E-Mail-Pfad: 5 Fehler behoben (Review Erstellung/Versand)

- **Anlass (Walter):** Review des E-Mail-Erstellungs-/Versandpfads auf Fehler und Optimierung; die 5 gefundenen Fehler wurden schrittweise behoben (Optimierungen bewusst zurückgestellt).
- **Fix 1 — „Alle senden" bei fehlendem Anhang (`app/modul/email_provider_mixin.py`):** Bisher gingen E-Mails beim Sammel-Versand bei fehlendem Anhang **ohne PDF** raus (Gmail/SMTP/Outlook/New Outlook: Dict-Marker per `continue` übersprungen, Status „gesendet") bzw. mit ungültigem Base64-Anhang (`<fehlt: …>`-Marker im Brevo-Request). Neuer Helfer `_fehlende_anhaenge_behandeln` (Status „fehler" + Meldung „Anhang fehlt: <Name>" in DB und JSON, Versand abgebrochen), Aufruf in allen 5 Sendern nach `_resolve_anhang_pfade`. Anhang-Auflösung aus `_build_brevo_body` in die Aufrufer verlagert (neuer Parameter `anhang_pfade`); die Diagnose-Anzeige „Body anzeigen" behält die `<fehlt: …>`-Marker bewusst.
- **Fix 2 — Marker-Ersetzung protokolliert (`app/email_gen.py`):** Schlägt `ersetze_markern` fehl, ging die rohe Vorlage mit unersetzten `{…}`-Markern still an den Kunden. Jetzt: `fallback_log.melde` (Modul „E-Mail-Erstellung") + `"marker"` in `meta._fallback` → gelbe Zeile im Postausgang.
- **Fix 3 — E-Rechnung-Anhang bei „E-Mail neu erzeugen" (`app/e_rechnung/__init__.py`, `app/modul/beleg_liste.py`, `app/email_gen.py`):** Neuer Helfer `e_rechnung.finde_vorhandene(db, rechnung_id)` (deterministischer Dateiname via `vorhersage_dateiname`, rekursive Suche unter `{Spool}/{Firmennr}`, neuester Treffer; konventionsbasiert, kein Pfad in der DB). `_email_neu_erzeugen_aktion` reicht ihn bei Rechnungen als `e_rechnung_pfad` durch — bei Versandart 2/3 bleibt die XML im Anhang (ging bisher still verloren). Zusätzlich in `erzeuge_email`: Versandart 2/3 ohne vorliegende E-Rechnung → Fallback-Protokoll + gelbe Zeile (deckt auch den Druckpfad ab, z. B. Versandart E-Rechnung bei inaktiver Kunden-E-Rechnung).
- **Fix 4 — Hartes Löschen statt Soft-Delete (`app/db/db_emails.py`, `app/email_gen.py`):** Neue Methode `delete_email_versand_hart` (`DELETE … WHERE id=? AND firma_id=?`). Verwendet beim Ersetzen alter ausstehender/fehlerhafter E-Mails desselben Belegs und beim Rollback nach JSON-Schreibfehler — bisher blieben Soft-Delete-Datenleichen im „Gelöscht"-Filter. Benutzer-Löschungen bleiben Soft-Delete.
- **Fix 5 — „Erneut senden" (`app/modul/mod_emails.py`):** Vorab-Reset auf „ausstehend" (DB+JSON) entfernt — bei Abbruch des Versands bleiben bisheriger Status und `fehler_meldung` erhalten; `_email_versenden` setzt den Endstatus selbst. Verwaisten Import `_json_status_setzen` mitentfernt.
- **Verifikation:** `ruff check app` ✓ (nach jedem Fix); `py_compile` aller 6 geänderten Dateien ✓; `audit_firma_id.py` ✓ (neue DELETE-Query firma-gefiltert, keine Fehler). Manuelle Tests in Firma 990 je Fix bei Walter (Testanweisungen je Schritt übergeben).
...
- **Wahre Ursache (durch Daten belegt, kein Datenverlust):** Alle `language.<code>.json` haben den Key mit **4 `\n`** wie der Master — Speichern/Übersetzen war immer korrekt. Der Verlust war rein kosmetisch: die Übersetzungs-Spalte (`COL_UEB`) nutzte den Delegate `_MarkerHighlightDelegate`, der den Text als **HTML** (`QTextDocument.setHtml`) rendert, um fehlerhafte Marker invers-rot zu markieren — **HTML kollabiert `\n` zu Leerzeichen**. Die Rück-Spalte (Standard-Klartext) zeigte die Umbrüche, daher der irreführende Unterschied. (Headless bestätigt: HTML-`\n` → 0 Umbrüche/Höhe 22; Klartext → 4/Höhe 78.)
- **Vorgeschichte:** Zwei falsche Fix-Versuche (Parser-Leerzeilen, `lang_tools.uebernimm_layout`) wurden zuvor per `git revert` (`2a3a046`) zurückgenommen — die Diagnose (LLM-Treue / Speicher-Pfad) war falsch.
- **Fix (Walter-Entscheidung):** Da die Platzhalter über die ⟦N⟧-Maskierung zuverlässig geschützt sind, ist die Inline-Marker-Hervorhebung (und der HTML-Umweg) überflüssig. `_MarkerHighlightDelegate` **ganz entfernt** (`app/modul/sprachdatei_dialoge.py`, inkl. Docstring + ungenutzte Importe); Verdrahtung `setItemDelegateForColumn(COL_UEB, …)` in `app/modul/mod_sprachdatei.py` entfernt → die Zelle rendert wieder Klartext und zeigt `\n` mehrzeilig (wie die Rück-Spalte). Platzhalter-Probleme werden jetzt über einen **gelben Zellhintergrund** signalisiert (`theme.fallback_qcolor()`) plus dem bestehenden Tooltip; die Logik „Marker-Fehler ⇒ unstimmig/rot" bleibt.
- **Verifikation:** `ruff check app` ✓ (fing die ungenutzten Delegate-Importe); `py_compile` ✓; Import-Smoke ✓ (Delegate weg); Daten-Gegenprobe: Datei-`\n`-Zählung unverändert 4. App-Smoke (Layout in der Übersetzungs-Spalte, gelbe Marker-Zelle) steht bei Walter aus.
...
- **Neuer Prüf-Prompt (`app/ki_client.py::AEHNLICHKEIT_PROMPT`):** von 9 auf 4 Zeilen reduziert; nur noch zwei Prüfschritte (1. Übersetzung sprachlich, 2. Genauigkeit) mit dem Zusatz „Den Ausgangstext nicht bewerten oder korrigieren — er gilt als verbindlich."; Few-Shots gekürzt; Eingabeblock (`@@AUSGANGSTEXT_ANFANG@@`/`@@UEBERSETZUNG_ANFANG@@`) unverändert (Ausgangstext wird weiter zur Genauigkeitsbewertung mitgeschickt), ⟦N⟧-Regel bleibt.
- **Parser (`app/pruefung_parser.py`):** `FELDER` auf die 4 neuen; STATUS-Felder + `widerspruch` entfallen; `parse` gibt `(felder, fehler)` zurück; Plausibilität jetzt auf `@@KORREKTUR` == Eingabe (Echo-Fehler → Retry); `auf_tupel` liefert **3-Tupel** `(stufe, begruendung, korrektur)` — bei vorhandener Korrektur wird der sprachliche Befund der Begründung vorangestellt.
- **Integration (`app/uebersetzung.py::bewerte_und_korrigiere`):** Format-Erkennung am Template auf `@@KORREKTUR` umgestellt (die alten 9-Zeilen-/`<(KQ)>`-Prompts enthalten das nicht → laufen weiter über den Legacy-Parser); Rückgabe **4-Tupel** `(stufe, begruendung, korrektur, antwort)`; ein Temp-0-Retry unverändert. `_parse_bewertung_korrektur` bleibt als Fallback, dessen Quellsprache-Korrektur wird verworfen.
- **Konsumenten (`app/modul/sprachdatei_lauf.py`):** alle 6 Aufrufstellen auf 4-Tupel; `uebernehme_grammatik_quelle` gelöscht; `grammatik == "quelle"`-Pfade in `lauf`/`phase3_kern`/`neu_uebersetze_zeile` entfernt (`neu_uebersetze_zeile` jetzt Ein-Durchlauf → 8-Tupel ohne `quelle_geaendert`); `LaufUmgebung` ohne `frage_quelle_korrektur`/`zweitcode`/`zweitlabel`; `TEMPERATUR_SCHRITT` entfernt (kein Mehrfach-Durchlauf mehr). `quelltext_uebernehmen` (manuelle Entwickler-Quelltext-Änderung) bleibt erhalten.
- **UI (`app/modul/mod_sprachdatei.py`, `app/modul/sprachdatei_dialoge.py`):** `_frage_quelle_korrektur` + Verdrahtung entfernt; `quelle_geaendert`-Parameter/Spaltenhervorhebung in `_set_row` und `_retranslate_row` entfernt; verwaiste `_AntwortDialog`-Klasse (nur vom gelöschten Dialog genutzt) samt Import gestrichen.
- **i18n (`app/language.json`):** Schlüssel `dlg.sprachdatei.quelle_korrektur_frage` entfernt (übersetzte Kopien bleiben als harmlose Waisen).
- **Migration (`app/DB-Pflege.py::_to_v65`, CURRENT_VERSION 64→65, keine Schema-Änderung):** hebt `ki_prompt_aehnlichkeit` **nur bei exaktem Treffer** der v64-9-Zeilen-Fassung auf das neue 4-Marker-Formular; eigene Anpassungen bleiben.
- **Tests (`app/test_pruefung_parser.py`):** auf 4 Felder umgeschrieben (STATUS-/AUSGANGSTEXT-/BESTE_UEBERSETZUNG-Fälle raus; `@@KORREKTUR`, 3-Tupel-`auf_tupel`, Echo-Fehler, `(felder, fehler)`-Signatur; ⟦N⟧-Maskierung unverändert).
- **Verifikation:** `ruff check app` ✓; `py_compile` der 7 geänderten Dateien ✓; 17/17 Parser-Tests ✓; `audit_firma_id.py` Exit 0 ✓; Migrations-Kette v64→v65 per AST + In-Memory-Lauf ✓ (v65.alt == v64-Zielfassung, v65.neu == `AEHNLICHKEIT_PROMPT`, 9-Zeilen→4-Marker, eigene Anpassung unangetastet); Headless-Integration `bewerte_und_korrigiere` mit gemocktem LLM ✓ (4-Tupel, `@@KORREKTUR`-Prompt gesendet, `stufe=gut`, Korrektur demaskiert `{Nr}`, Befund vorangestellt). Offen bei Walter: App-Start (Migration v65) + Live-Prüflauf in Firma 990.

## 2026-07-09 09:35 — LLM-Übersetzungspipeline gehärtet: Prüf-Prompt-Formular, strikter Parser, ⟦N⟧-Nachzug (DB v64)

- **Anlass (Walter, aus Diskussions-Auftrag):** Der Bewertungs-/Prüf-Prompt (`ki_prompt_aehnlichkeit`, Firma 990) hatte drei Designfehler, die beim lokalen Modell zu Slot-Verwechslungen führten: bedingter Kontrollfluss („Führe Schritt 2 nicht durch, gib trotzdem alle drei Zeilen aus"), verwechselbare Kürzel-Delimiter (`<(KQ[…]QK)>`/`<(KZ[…]ZK)>`) und Urteil vor Begründung. Zusatzbefund der Ist-Analyse: **kein** DB-Prompt (990 und Bestands-Defaults v45/v50/v55) enthielt die ⟦N⟧-Masken-Regel, obwohl die Pipeline seit der Platzhalter-Maskierung (2026-07-08) ⟦N⟧-Token sendet.
- **Neuer Prüf-Prompt (`app/ki_client.py::AEHNLICHKEIT_PROMPT`):** festes 9-Zeilen-Formular `@@AUSGANGSTEXT_BEFUND:` … `@@BESTE_UEBERSETZUNG:` — immer alle Felder (keine Verzweigung im Prompt), sprechende Feldnamen (eine `@@`-Marker-Familie), Befund **vor** Status/Genauigkeit (erst analysieren, dann klassifizieren), Literal-Blöcke in `~~~`-Fences, ⟦N⟧-Regel im Regelteil **und** wiederholt direkt vor den Eingabedaten, Eingabe in `@@AUSGANGSTEXT_ANFANG@@`/`@@UEBERSETZUNG_ANFANG@@`-Blöcken.
- **Neuer Parser (`app/pruefung_parser.py`, Qt-/LLM-frei):** Fence-Strip (```` ``` ````/`~~~`), strikte 9-Feld-Prüfung (fehlend/doppelt/unbekanntes `@@`-Konstrukt = Fehler), Wertevalidierung (STATUS nur KORREKT/FEHLERHAFT, GENAUIGKEIT nur IDENTISCH/SEHRGUT/GUT/SCHLECHT), „-"-Normalisierung, mehrzeilige Feldwerte; Plausibilität: (a) FEHLERHAFT + Korrektur == Eingabe → Fehler (Kopier-Fehler), (b) KORREKT + SCHLECHT → Widerspruchs-Flag mit Sichtungs-Hinweis in der Begründung. `auf_tupel` mappt aufs bestehende Bewertungs-Tupel (quelle gewinnt bei beidseitigem Grammatikfehler; Ziel-Korrekturen fließen über `korrektur`).
- **Integration (`app/uebersetzung.py::bewerte_und_korrigiere`):** Format-Erkennung am Template (`@@AUSGANGSTEXT_BEFUND` enthalten → neuer Parser); bei Parse-/Validierungsfehler **genau ein** deterministischer Retry (Temperatur 0), danach `stufe=None` mit Fehlerliste in der Begründung. Alter Parser (`_parse_bewertung_korrektur`) bleibt unverändert als Fallback für individuell gepflegte Alt-Prompts. 6-Tupel-Schnittstelle der Konsumenten (`sprachdatei_lauf.py`) unverändert. `_MASKE_RETRY_TEMPERATUR` 0.2 → 0.0 (deterministischer Masken-Retry).
- **Migration (`app/DB-Pflege.py::_to_v64`, CURRENT_VERSION 63→64, keine Schema-Änderung):** hebt `ki_prompt_aehnlichkeit`/`massen`/`uebersetzung`/`rueckuebersetzung` **nur bei exaktem Treffer** einer bekannten Alt-Fassung (je Feld 3: v45/v50/v55-Defaults, Vor-⟦N⟧-ki_client-Fassung, Firma-990-Ist-Fassung; per Skript byte-exakt aus Git/DB extrahiert, dedupliziert). 990s `massen`/`uebersetzung`/`rueck` erhalten dabei die ⟦N⟧-Regel statt der `{…}`-Klammer-Regel, Few-Shots auf `⟦0⟧`/`⟦1⟧` umgestellt, WICHTIG-Wiederholung vor den Eingabedaten; `aehnlichkeit` wird überall durch das neue Formular ersetzt. Eigene Anpassungen fremder Installationen bleiben unberührt (alter Parser deckt sie weiter ab).
- **Tests (`app/test_pruefung_parser.py`, neu):** 20 Tests (standalone + pytest-kompatibel) — Fences, unbekanntes/fehlendes/doppeltes Feld, ungültige Werte, Kopier-Fehler, Widerspruch, mehrzeilige Korrektur, Freitext-Präambel, ⟦N⟧-Roundtrip/Token-fehlt/-verdoppelt/No-op/`maskiere_gemeinsam`.
- **Verifikation:** 20/20 Tests ✓; `ruff check app` ✓; `audit_firma_id.py` Exit 0 ✓; `_to_v64`-Dry-Run auf DB-Kopie ✓ (alle 4 990-Prompts gehoben, idempotent); End-to-End-Smoke mit gemocktem LLM ✓ (Format-Erkennung, Retry `[None, 0.0]`, Mapping, Demaskierung, Alt-Parser-Fallback). Offen bei Walter: App-Start (Migration v64 via DB-Pflege) + Live-Bewertung eines Items in Firma 990 im Übersetzungstest-Modus.
- **Bewusst nicht umgesetzt:** kein neues Maskierungs-Modul (existiert vollständig in `lang_tools.py`), keine `prompts/*.md`-Pflegedateien (Walter-Entscheidung: Konstante in `ki_client.py` bleibt single source), Gold-Standard-Testsatz pro Sprache = späterer Schritt.
- **Dateien:** `app/ki_client.py`, `app/pruefung_parser.py` (neu), `app/uebersetzung.py`, `app/DB-Pflege.py`, `app/test_pruefung_parser.py` (neu), `DOKU-TODO.md`.
...
- **Lösung (wie professionelle MT):** Platzhalter **vor** dem LLM-Aufruf durch semantisch leere Token `⟦N⟧` (U+27E6/27E7) maskieren und **nach** der Antwort wiederherstellen — deterministische Treue statt Prompt-Hoffnung. Nebeneffekt: die Doppelnutzung geschweifter Klammern (Template-Marker `{Text}` vs. Content-Platzhalter) löst sich auf, da im Modell-Input keine `{}` mehr ankommen.
- **Maskier-Helfer (`app/lang_tools.py`):** `maskiere` (jede Fundstelle → eigenes Token), `maskiere_gemeinsam` (gleicher Platzhalter-Wert → gleiches Token über mehrere Texte, für die Bewertung), `demaskiere`, `maske_intakt` (`count==1`-Check). Qt-frei, wiederverwendet `_MARKER_RE`. Bei markerfreiem Text alles ein No-op.
- **Integration (`app/uebersetzung.py`), ganze Pipeline:** `_uebersetze_text` (Vorwärts einzeln — inkl. **einem** Retry mit gesenkter Temperatur `⟦N⟧`-Verlust; `_uebersetze_schritt` erhielt dafür einen optionalen `temperatur`-Parameter), `uebersetze_batch` (vor + rück, je Item eigenes Mapping), `uebersetze_rueck`, `bewerte_und_korrigiere` (Ausgangstext + Übersetzung mit gemeinsamem Mapping; `korrektur`/`grammatik_korrektur`/`begründung`/Rohantwort demaskiert). `import lang_tools` ergänzt. Der Beleg-Druck-Split-Pfad (`_translate` ohne `kein_split`) bleibt unberührt (No-op, Literale ohne `{}`). Kaputte Batch-Items fängt weiterhin `marker_stimmig` ab.
- **Default-Prompts (`app/ki_client.py`):** In `UEBERSETZUNG_PROMPT`, `MASSEN_UEBERSETZUNG_PROMPT`, `RUECKUEBERSETZUNG_PROMPT`, `AEHNLICHKEIT_PROMPT` die `{}`-Regel durch eine `⟦N⟧`-Regel ersetzt + kritische Betonung kurz vor den Eingabedaten. Betrifft nur **neue** Firmen (create_firma/_to_v28); Firma 990 behält ihre DB-Prompts (funktionieren mit Maskierung, `{}`-Regel wird No-op) — Walter passt sie bei Bedarf selbst an. `RECHTSCHREIBUNG_PROMPT` unberührt (Pfad nicht maskiert).
- **Verifikation:** `ruff check app` ✓, `py_compile` ✓; Headless-Roundtrip (maskiere/demaskiere/maske_intakt/maskiere_gemeinsam inkl. Wiederholung/leer) ✓; Pipeline-Integrationstest mit gemocktem LLM (`uebersetze_batch`: Prompt maskiert, Ergebnis behält `{…}`; `bewerte_und_korrigiere`: gemeinsames Token in beiden Texten, Korrektur demaskiert) ✓; Import-Smoke ✓. Realtest in Firma 990 (Live-LLM) steht als manueller Folgeschritt aus.
- **Dateien:** `app/lang_tools.py`, `app/uebersetzung.py`, `app/ki_client.py`.

## 2026-07-08 13:51 — Übersetzungs-Code an @@-Prompts angepasst (Massen, Fehlersignal, <text>-Tags, Rechtschreibung)

- **Anlass (Walter):** Prüfen, dass in keinem KI-Prompt der Firma 990 noch `#`/`##`-Marker vorkommen. Ergebnis: Die Prompts wurden erneut überarbeitet (alle Ausgabe-Marker jetzt `@@…@@` bzw. `<(…)>`); nur der **ungenutzte** `ki_prompt_uebersetzung_retry` enthält noch 6 `##`-**Markdown-Überschriften** (kein Code-Pfad liest dieses Feld — unverändert gelassen). Dabei traten vier neue Code-Inkompatibilitäten zutage, die Walter alle beheben ließ (rückwärtskompatibel zu den alten `#`-Formaten).
- **Fix A — Massen-Übersetzung `@@N@@` + `@@ENDE@@` (`uebersetzung.py`):** `_BATCH_MARKER_RE` erkennt jetzt `@@N@@` (Gruppe 1) **und** `#N#`/`#N:` (Gruppe 2); neues `_ENDE_MARKER_RE` verwirft die `@@ENDE@@`-Abschlusszeile vor dem Zerlegen; `_baue_nummerierten_block(texte, at_format)` baut Items je nach Prompt als `@@N@@` oder `#N#`; in `uebersetze_batch` wird das Format aus dem Template abgeleitet (`"@@" in template`) und der Items-Block per `{Text}`-Marker **eingesetzt** statt angehängt (der neue Prompt hat nach `{Text}` noch `@@ENDE@@` + Schlussanweisung — Anhängen hätte den Block an die falsche Stelle gesetzt). Ohne den Fix scheiterte jeder Batch und fiel auf die langsame Einzelübersetzung zurück.
- **Fix B — Fehlersignal `@@FEHLER@@` (`_ist_uebersetzung_unmoeglich`):** erkennt jetzt zusätzlich `@@FEHLER@@` (aktueller System-/Übersetzungs-Prompt) neben dem alten `"ÜBERSETZUNG NICHT MÖGLICH!"`. Ohne den Fix wäre `@@FEHLER@@` als gültige Übersetzung in Text/Beleg gelangt.
- **Fix C — `<text>`-Tags (`_bereinige_uebersetzung`):** entfernt umschließende `<text>…</text>`-Tags aus der Antwort (der aktuelle Übersetzungs-/Rückübersetzungs-Prompt rahmt den Eingabetext damit ein und verlangt die Ausgabe „ohne Tags", manche LLMs geben sie dennoch mit). Zentral in der Bereinigung → deckt Vorwärts-, Rück- und Batch-Übersetzung ab.
- **Fix D — Rechtschreib-Marker (`ki_client.parse_rechtschreib_antwort` + `mod_artikel.py::_ki_korrektur`):** neue rückwärtskompatible Parse-Funktion erkennt `@@OK@@`/`@@KORREKTUR@@`/`@@KOMMENTAR@@`/`@@FEHLER@@` **und** die alten `##KORREKTUR:`/`##KOMMENTAR:`/`##Nicht prüfbar!`. `_ki_korrektur` wertet den Status aus: `ok` → Meldung „keine Fehler gefunden", `fehler` → „nicht prüfbar", `korrektur` → Korrektur-Dialog mit **markerfreiem** Text (`text`-Fallback = rohe Antwort wie bisher). Zwei neue i18n-Keys (`artikel.ki.msg.keine_fehler`, `artikel.ki.msg.nicht_pruefbar`). Vorher landeten die `@@`-Marker roh im Korrektur-Feld.
- **Verifikation:** 16/16 Unit-Checks (Massen neu/alt + Block-Bau, `@@FEHLER@@`/alt/normal, `<text>`-Strip, Rechtschreibung alle 6 Fälle); Integrationscheck des echten Firma-990-Massen-Prompts (Block korrekt in `{Text}` eingesetzt, kein Rest-Literal, Item-Platzhalter erhalten); `python -m py_compile` + `python -m ruff check app` ✓. Kein DB-Schema betroffen; DB-Prompts nicht verändert (Walter pflegt sie über die UI).
- **Offen/bewusst:** `ki_prompt_uebersetzung_retry` (toter Prompt) nicht angefasst; `ki_client.uebersetze` (separater Beleg-Druck-Pfad) nutzt noch die alten `{Sprache Firma}`/`{Sprache Kunde}`-Marker — nicht Teil dieses Auftrags, bei Bedarf separat.
- **Dateien:** `app/uebersetzung.py`, `app/ki_client.py`, `app/modul/mod_artikel.py`, `app/language.json`.

## 2026-07-08 09:58 — Übersetzungs-Prompts (Firma 990) an Code angepasst + App-Sprache-Fenster in Taskleiste

- **Anlass (Walter):** (1) Das Fenster „App-Sprache" war beim Minimieren nicht in der Taskleiste sichtbar; (2) die KI-Übersetzungs-Prompts der Firma 990 hatten sich in der DB geändert und passten nicht mehr zum Code.
- **Fenster-Fix (`app/modul/mod_sprachdatei.py`):** `SprachdateiDialog` ist ein `QDialog` mit Parent (Typ `Qt.Dialog`) → bekommt unter Windows kein eigenes Taskleisten-Symbol; mit aktiviertem Minimieren-Knopf verschwand das Fenster beim Minimieren spurlos. Flag `Qt.WindowType.Window` ergänzt → eigenes Taskleisten-Symbol, Modalität (`exec()`) unverändert.
- **Prompt-Analyse:** Die Firma-990-Prompts in der DB (`ki_prompt_*`) wurden stark überarbeitet und weichen von den Code-Defaults in `ki_client.py` ab. Zwei Code-Inkompatibilitäten empirisch nachgewiesen (Parser-Test + Marker-Ersetzung): (a) `ki_prompt_aehnlichkeit` nutzt jetzt `@@`-Marker statt `#`/`##` und stellt die Bewertungsstufe in Zeile 2 (hinter die Grammatik-Zeile) → `_parse_bewertung_korrektur` lieferte durchgängig `stufe=None`, Korrektur/Grammatik-Korrektur wurden verworfen; (b) `ki_prompt_uebersetzung` nutzt `{Quellsprache}`/`{Zielsprache}`, der Einzelübersetzungs-Pfad `_uebersetze_schritt` befüllte aber nur `{Sprache Firma}`/`{Sprache Kunde}` → Sprachrichtung-Platzhalter blieben wörtlich stehen. Entscheidung Walter: Code an die neuen Prompts anpassen (rückwärtskompatibel), Code-Defaults in `ki_client.py` **nicht** nachziehen.
- **Fix 1 — Ähnlichkeits-Parser (`app/uebersetzung.py`), rückwärtskompatibel:** `_STUFE_MARKER_RE` von `#+` auf `[#@]+` erweitert (erkennt `@@IDENTISCH`/`@@GUT` …; die positionsunabhängige `.search()` löst zugleich das „Stufe in Zeile 2"-Problem). Neue Regexes `_GQ_KORR_RE` (`<(KQ…QK)>` Quelle) / `_GZ_KORR_RE` (`<(KZ…ZK)>` Ziel) speisen richtungsabhängig `grammatik_korrektur`, `<(G…G)>` bleibt Rückfall; `_GRAMMATIK_BEGR_RE` (`<(BQ…QB)>`/`<(BZ…ZB)>`) entfernt die Grammatik-Begründungen aus dem Rest, damit sie nicht in die gesammelte Genauigkeits-Begründung lecken.
- **Fix 2 — Einzelübersetzungs-Pfad (`app/uebersetzung.py::_uebersetze_schritt`):** zusätzlich `MARKER_QUELLSPRACHE`/`MARKER_ZIELSPRACHE` befüllt (alte `{Sprache Firma}`/`{Sprache Kunde}` bleiben für Rückwärtskompat). Massen-, Rück- und Ähnlichkeits-Pfad brauchten keine Änderung (Marker passten bereits).
- **Verifikation:** Parser-Test 9/9 (5× neues `@@`-Format inkl. `NICHT_GEPRUEFT`/`SEHRGUT`/`SCHLECHT`, 4× altes `#`-Format mit `##VORSCHLAG`/`##KEINVORSCHLAG`/`<(K…K)>`) — beide Formate korrekt; Marker-Ersetzung des echten Firma-990-Übersetzungsprompts: keine `{Quellsprache}`/`{Zielsprache}`-Literale mehr; `python -m py_compile` ✓; `python -m ruff check app` ✓. Kein DB-Schema betroffen.
- **Dateien:** `app/modul/mod_sprachdatei.py`, `app/uebersetzung.py`.

## 2026-07-08 08:38 — Belegerstellung: Crash Rechnung→Mahnung, Buchungsexport-Lücken bei Storno/Kopie, Snapshot-Vererbung, Transaktionen, Nummern-Race

- **Anlass (Walter):** Belegerstellung auf Fehler und Optimierungen prüfen, Befunde umsetzen.
- **KRITISCH 1 — `rechnung_zu_mahnung` crashte** seit v61 (04.07.) mit `table mahnungen has no column named kopf_snapshot`: das Rechnungs-Dict wurde inkl. `kopf_snapshot` in die `mahnungen`-Tabelle eingefügt, die diese Spalte nicht hat (Mahnungen nutzen `mahnung_snapshot`). Empirisch auf DB-Kopie reproduziert. Fix: `kopf_snapshot` (+ `kunde_snapshot`, s. u.) in die pop-Liste (`db_belege.py`).
- **KRITISCH 2 — Storno/Kopie erbten `buchungsexport_id`:** `rechnung_stornieren` und `rechnung_kopieren` übernahmen die Export-Markierung des Originals — bereits exportierte Originale hätten ihren Storno nie in den Buchungsexport gebracht (stille FiBu-Differenz). Fix analog `storniere_mahnung` (dort war es korrekt): Feld poppen bzw. auf None setzen. Altdaten-Prüfung (read-only): kein betroffener Bestandsfall (der einzige Storno mit Export-ID war regulär exportiert, Export-ID ≠ Original) → keine Reparatur-Migration nötig.
- **HOCH — Snapshot-Vererbung an Folgebelege beendet:** `angebot_zu_auftrag`, `auftrag_zu_lieferschein`, `auftrag_zu_rechnung`, `lieferschein_zu_rechnung` poppen jetzt `kunde_snapshot`/`kopf_snapshot`; `rechnung_zu_mahnung` zusätzlich `kunde_snapshot`; `mahnung_zu_naechste_stufe` poppt `mahnung_snapshot`/`kunde_snapshot` (sonst druckte Stufe N+1 die eingefrorene Stufenbezeichnung/Fälligkeit/Zinsen der Vorstufe); `rechnung_kopieren` leert beide. Folgebelege frieren damit beim EIGENEN Erstdruck ein (Konzept „Änderungen erst im Nachfolgebeleg"). Stornos (Rechnung + Mahnung) erben die Snapshots weiterhin bewusst (Spiegel des Originals, kommentiert). Altdaten: keine geerbten Snapshots in nie gedruckten Belegen vorhanden.
- **HOCH — Echte Transaktionen:** `_save_beleg`/`beleg_zahl_erhoehen`/`_set_beleg_zahl`/`storniere_mahnung` haben jetzt einen `commit`-Parameter (Default True, alle Alt-Aufrufer unverändert). Alle Konvertierungen, `_save_mahnung`, `rechnung_stornieren`/`rechnung_kopieren` bündeln ihre Teilschritte mit `commit=False` + einem `conn.commit()` am Ende + rollback im except — vorher committete `_save_beleg` intern sofort, der bestehende rollback war wirkungslos (z. B. Storno angelegt, Original nicht markiert → Doppelstorno möglich).
- **HOCH — Nummern-Race beim Neuanlegen entschärft** (`beleg_edit.py::_speichern`): Belegnummer wird nicht mehr von der beim Dialog-Öffnen gezogenen Vorschau übernommen, sondern unmittelbar vor dem Speichern frisch gezogen; `_save` in try/except — bei Fehler (z. B. UNIQUE-Kollision durch zweiten Benutzer) Meldung (`msg.fehler`) statt Crash, Dialog und Eingaben bleiben erhalten.
- **MITTEL — Mandanten-Isolation:** Nummern-Kollisionsprüfung `_next_nr_vorschau` jetzt mit `AND firma_id=?` (fremde Firmen erzeugen keine Lücken im eigenen Nummernkreis mehr); `_save_beleg`-UPDATE-Zweig mit `AND firma_id=?` + rowcount-Guard (RuntimeError statt Positionen an fremden Beleg hängen); `save_pdf_pfad`/`save_erstellungsdatum` über `_update_firma`.
- **Kleinkram:** doppelte `get_kunde`/`get_auftrag`/`get_rechnung`-Aufrufe in Konvertierungen/Deletes zusammengefasst; `lieferschein_zu_rechnung` aktualisiert den Auftrag nur noch bei vorhandener `auftrag_id`; Docstring `rechnung_kopieren` (status `entwurf` statt `offen`).
- **Verifikation:** Smoke-Test auf DB-Kopie mit 20 Checks (Mahnung aus Rechnung inkl. Folgestufe, Snapshot-Freiheit aller Folgebelege, Storno/Kopie ohne Export-ID, Storno-Snapshot-Spiegel, firma-isolierte Nummernvorschau, Fremd-UPDATE abgewiesen) — 20/20 OK; `python -m ruff check app` ✓; `audit_firma_id.py` FEHLER: keine ✓; Import-Smoke der geänderten Module ✓. Kein DB-Schema betroffen, keine Migration.
- **Offen/bewusst nicht angefasst:** `_save_record` (Nicht-Beleg-Tabellen) weiterhin ohne firma_id im UPDATE; `get_jahre` liefert Jahre über alle Firmen; Zähler läuft nach Kollisionssprüngen der Nummer hinterher (funktional durch while-Schleife abgedeckt).
- **Dateien:** `app/db/db_belege.py`, `app/db/db_core.py`, `app/db/db_belegzaehler.py`, `app/modul/beleg_edit.py`.

## 2026-07-07 21:03 — Anwenderdoku: App-Sprachübersetzung (mit Blockschaltbild + Flussdiagramm) und KI-Anbindung komplett nachgezogen (DE + EN)

- **Anlass (Walter):** Ausführliche Doku zur App-Sprachübersetzung mit Blockschaltbild/Flussdiagramm schreiben und in die App-Doku (F1-Hilfe) integrieren; die Anbindung-KI mit einbeziehen. Entscheidung: `doku.en.html` im selben Zug synchron nachziehen. Damit wurden 13 offene `DOKU-TODO`-Punkte auf einmal erledigt.
- **`app/doku.de.html` — Kapitel „KI-Anbindung einrichten" (id `firma-ki`) modernisiert:** LLM-Bezeichnungen auf „LLM 1 — einfache / LLM 2 — intensive Denkprozesse" aktualisiert (Belegverarbeitung immer LLM 1); neue Tabellenzeile „Denkprozess (Reasoning)" (OpenRouter je LLM, lokal je Server-Profil inkl. Token-Budget, Anthropic über Effort); neue Unterüberschrift „App-Übersetzung: LLM-Zuordnung" (id `firma-ki-llm-zuordnung`) mit Aufgaben-Tabelle (Übersetzung/Rückübersetzung/Bewertung, Standard-Zuordnung) und dem Feld „Denken (Anthropic)" (Adaptiv/Niedrig/Mittel/Hoch/Sehr hoch/Maximal); Prompt-Abschnitt vervollständigt (zweizeilige Klick-Felder → Markdown-Editor mit Live-Vorschau + Marker-Buttons; alle 7 Prompts inkl. „Massen-/Batchübersetzung" mit `{Quellsprache}`/`{Zielsprache}`/`{Anzahl}` und „Übereinstimmungsprüfung & Korrektur" mit `{Ausgangstext}`/`{Übersetzung}`; Hinweis System-Prompt bei Bewertung weggelassen, Rechtschreib-Prompt nicht für den Generator).
- **Kapitel „Zusätzliche App-Sprachen erstellen" (id `app-sprachen`) komplett neu geschrieben** (vorher veraltet — beschrieb u. a. die entfernte „Durchläufe"-Einstellung und nur 3 Bewertungsstufen): Überblick; **Blockschaltbild (SVG)** language.json → Generator ⇄ KI → language.<code>.json/.review.json → App-Oberfläche/installed_languages.txt inkl. gestricheltem Rückpfad „bestätigte Quelltext-Korrektur"; Zielsprache/Quelle/Sprachbeherrschungs-Prüfung (Ablehnung > 6); alle Steuerfelder und Schaltflächen (Batch-Größe, Nur fehlende, Sinngemäße Prüfung, Schlecht/Gut neu übersetzen, Alle anzeigen, Sofort-Abbruch); **Flussdiagramm (SVG)** der 3-Phasen-Batch-Pipeline mit ja/nein-Verzweigung, 4 Bewertungsstufen, automatischer Korrektur-Übernahme und dem Grammatik-Rückfrage-Seitenpfad (Marker-Guard, en-Nachzug, frische Neuübersetzung); vollständige Farberklärung (rot/grün/normal, hellgrau veraltet, kursiv-fett KI-geändert, invers-rote Platzhalter, Stern-Ampel mit Tooltip); Zeilen-Aktionen (Häkchen, Doppelklick-Bearbeitung, Neu übersetzen, Bewertung); eigener Unterabschnitt „Grammatikfehler im Ausgangstext (Rückfrage)"; Token-Verbrauch-Rahmen mit „Aktuell:"-Zeile; Massenaktualisierung; Hinweise (Noto-Schriftsysteme, Wörterbuch-Installer, ausgenommene kundengerichtete Vorlagen, Entwicklermodus). Beide SVGs nutzen die bestehenden theme-aware `d-*`-Klassen.
- **Neues Kapitel „Token-Verbrauch" (id `token-verbrauch`, passend zum `HELP_ANCHOR` von `mod_token_verbrauch.py`)** zwischen Fehler-Nachverfolgung und DSGVO + TOC-Link: firmenbezogene Tokenzählung je Anbieter/Modell/Aufgabe, „Zurücksetzen" mit Rückfrage, Abgrenzung zum Sitzungs-Zähler des Generators.
- **`app/doku.en.html`:** alle drei Bereiche synchron auf Englisch nachgezogen (gleiche Anker, gleiche SVG-Struktur mit übersetzten Beschriftungen, TOC-Link „Token Consumption").
- **`DOKU-TODO.md`:** 13 erledigte Punkte entfernt (Massenaktualisierung, „Aktuell:"-Zeile, veraltet-hellgrau, kursiv-fett, Token-Verbrauch, Grammatik-/Stil-Rückfrage, Anthropic-Effort, Reasoning/Budget, Sprachbeherrschung, LLM-Zuordnung, Batch-Pipeline, Bewertung-Button, Schriftsysteme, Marker-Prüfung, kombinierter Bewertungs-/Korrektur-Prompt, Markdown-Prompt-Editor, Entwicklermodus); Wörterbuch-Punkt bleibt (betrifft nur noch `Readme.admin.de.md`), ebenso Beleg-Archiv/Freeze/Marker-„(—)". Schlussnotiz auf 2026-07-07 (DE+EN synchron) aktualisiert.
- **Verifikation:** Beide HTML-Dateien per Parser geprüft — wohlgeformt, keine doppelten ids, keine toten `#`-Links, Anker `app-sprachen`/`token-verbrauch`/`firma-ki`/`firma-ki-llm-zuordnung` vorhanden, kein Mojibake, je 2 SVGs im Kapitel; `HELP_ANCHOR`-Abgleich (einziger fehlender Anker bleibt `beleg-archiv` — vorbestehender offener DOKU-TODO-Punkt). Beide SVGs per QtSvg zu PNG gerendert und gesichtet (Label „Quelltexte" nach Sichtprüfung verschoben — kollidierte mit dem Pfeil); alle Inhaltsangaben gegen `mod_firma_ki.py`, `mod_sprachdatei.py`, `sprachdatei_lauf.py`, `ki_client.py` (Marker-Konstanten) und die `dlg.sprachdatei.*`/`firma.ki.*`-Labels aus `language.json` verifiziert. `python -m ruff check app` ✓.
- **Dateien:** `app/doku.de.html`, `app/doku.en.html`, `DOKU-TODO.md`.

## 2026-07-07 20:45 — App-Sprach-Übersetzung: Quelltext-Schäden repariert + Lauf gegen kaputte Quelltext-Korrekturen gehärtet

- **Anlass (Walter):** „App-Sprach-Übersetzung ansehen und optimieren." Die Analyse ergab: Beim Schweden-Lauf (SE) am 2026-07-07 hat die Grammatikprüfung des Ausgangstexts (`GRAMMATIK_QUELLE`, erweiterter Firma-990-Prompt) mehrere **deutsche Quelltexte in `language.json` verschlechtert oder zerstört** — bis mittags noch ohne Rückfrage (der Bestätigungs-Dialog `frage_quelle_korrektur` kam erst nachmittags hinzu, Checkpoint `f29f3c0`). Schlimmster Fall: `field.kunde.email_versand_angebot` = `{E-Mail-Versand-Angebot:}` — geschweifte Klammern sind Platzhalter-Syntax, die deutsche UI zeigte das wörtlich an, und `language.se.json` hatte den kaputten Text unübersetzt übernommen und als `ok:true` bestätigt. Plan: `PLAN-Sprachdatei-Optimierung.md`.
- **Datenreparatur `app/language.json`** (über `lang_tools.load_main`/`stamp_main`/`schreibe_main`, 6 Keys neu gestempelt → gelten in allen Zusatzsprachen als veraltet und werden nachübersetzt): Die 4 E-Mail-Versand-Felder des Kundenstamms vereinheitlicht auf »E-Mail-Versand von Rechnungen/Angeboten/Aufträgen/Mahnungen:« (en analog »Sending … by email:«); `dlg.sprachdatei.btn_aehnlichkeit_tt` (sinnentstellt), `dlg.sprachdatei.legende_veraltet` („Hellgrau Hintergrund" statt „Hellgrau hinterlegt") und `col.steuerschluessel` („Tax Key" statt „Tax Code") auf den Stand von `23f91da` zurückgesetzt; `dlg.dsgvo.anonymisieren_frage` behalten (echte Verbesserung). `language.se.json` + `.review.json`: kaputten Eintrag `field.kunde.email_versand_angebot` entfernt (bis zur Nachübersetzung greift der Quellsprachen-Fallback).
- **Marker-Guard (`app/modul/sprachdatei_lauf.py::uebernehme_grammatik_quelle`):** Leere Vorschläge sowie Vorschläge, die die `{…}`-Format-Platzhalter des bisherigen Texts verändern (`lang_tools.marker_stimmig`), werden jetzt **ohne Rückfrage verworfen** — genau dieser Guard hätte den Vorfall verhindert.
- **Zweite Quellsprache nachziehen:** Bei bestätigter Quelltext-Korrektur wird die zweite Basissprache (de↔en) per LLM an den neuen Text angepasst (wie in `quelltext_uebernehmen`), damit die Quellsprachen nicht auseinanderdriften. `LaufUmgebung` dafür um `zweitcode`/`zweitlabel` erweitert (befüllt in `mod_sprachdatei.py::_lauf_umgebung` über das vorhandene `_zweite_quelle()`).
- **Neuübersetzung statt Rückprüfung:** In `lauf()` (Phase 3) und `phase3_kern()` blieb nach einer bestätigten Quelltext-Korrektur die alte Übersetzung stehen (nur die Rückübersetzung wurde neu berechnet); ein mitgelieferter Korrekturvorschlag bezog sich ebenfalls auf den alten Quelltext. Jetzt: bei `quelle_geaendert` wird die alte Korrektur verworfen und der neue Quelltext **frisch vorwärts übersetzt** (danach wie bisher Rückübersetzung + ok-Prüfung); Bewertung/Begründung der verworfenen Übersetzung werden geleert. `neu_uebersetze_zeile` hatte diese Logik bereits (Neustart).
- **Verifikation:** `python -m ruff check app` ✓, `py_compile` ✓; 30 Headless-Stub-Tests ✓ (Marker-Guard verwirft fremde/verlorene Marker und leere Vorschläge ohne Rückfrage; Zustimmung aktualisiert de+en+`quellwerte`; Ablehnung schreibt nichts; `lauf()`/`phase3_kern` nutzen nach Quellkorrektur die frische Übersetzung mit `bewertung=None`/`ok=True`/`quelle_geaendert=True`; Ablehnungs-Pfad behält den alten Korrektur-Ablauf). Datenprüfung ✓: keine `{…}`-Marker in den 7 reparierten Keys, SE-Eintrag entfernt, `ist_veraltet` meldet die 6 neu gestempelten Keys für NL als veraltet.
- **Dateien:** `app/language.json`, `app/language.se.json`, `app/language.se.review.json`, `app/modul/sprachdatei_lauf.py`, `app/modul/mod_sprachdatei.py`, `PLAN-Sprachdatei-Optimierung.md`, `DOKU-TODO.md`.

## 2026-07-06 18:11 — Bewertungs-/Korrektur-Prompt: Begründung über <(B[...]B)>-Block

- **Anlass (Walter):** Die Firma-990-Prompt-Vorlage (DB-Konfiguration) wurde um einen dritten Klammer-Marker `<(B[{Bewertung}]B)>` ergänzt, der die kurze Begründung (max. drei Sätze) eindeutig umschließt — analog zu `<(G[...]G)>`/`<(K[...]K)>`. Zur Klärung: „B" bezeichnet dabei EIN Klammer-Block, der Stufenwort **und** Begründungstext zusammen enthält (Stufe bleibt zusätzlich als eigener Rauten-Marker in Zeile 2, `<(B[...]B)>` liefert nur den Begründungstext separat in Zeile 3 der aktuellen Prompt-Fassung).
- **`app/uebersetzung.py::_parse_bewertung_korrektur`:** Neue Konstante `_B_KORR_RE` (`<\(\s*B\s*\[(.*?)\]\s*B\s*\)>`, wie G/K positionsunabhängig per `.search()` über den gesamten Text). Die Begründung wird jetzt bevorzugt direkt aus dem B-Block gelesen; ohne Treffer (Standard-Prompt ohne dieses Format) greift weiterhin der bisherige Rückfall auf `_sammle_begruendung` (übrig gebliebene Zeilen nach Entfernen aller erkannten Bausteine).
- **Verifikation:** 6 Szenarien gegen das reale Firma-990-Format getestet (inkl. vertauschter Reihenfolge B vor/nach GUT+K, IDENTISCH/SEHRGUT/GUT mit B-Block, GRAMMATIK_QUELLE+G+GUT+K+B kombiniert) sowie alle bisherigen Regressionstests (Standard-Prompt-Rückfall, False-Positive-Schutz, alte `##BESSER`-Marker) erneut ohne Abweichung durchlaufen; `python -m ruff check app` ✓.
- **Dateien:** `app/uebersetzung.py`.

## 2026-07-06 15:46 — Bewertungs-/Korrektur-Parser positionsunabhängig gemacht + „Neue Bewertung" entfernt und in „Neu übersetzen" integriert

- **Anlass (Walter):** Nach dem Parser-Fix von 15:10 blieb ein Fall bestehen, in dem eine gelieferte `<(K[...]K)>`-Korrektur trotz `##GUT`-Bewertung nicht übernommen wurde (Leerzeile zwischen `##GRAMMATIK_OK` und `##GUT`-Zeile). Walter wies zusätzlich darauf hin, dass die Position der Marker/Klammer-Blöcke im Antworttext bei unterschiedlichen LLMs ohnehin nicht zuverlässig steuerbar ist — die Suche solle grundsätzlich nur nach dem Klammerinhalt suchen, unabhängig von Reihenfolge/Umgebung.
- **`app/uebersetzung.py::_parse_bewertung_korrektur` überarbeitet:** Grammatik-Ergebnis, `<(G[...]G)>`-/`<(K[...]K)>`-Block sowie eine rautenmarkierte Bewertungsstufe (`##GUT` etc., erweiterter Prompt wie Firma 990) werden jetzt per Regex im **gesamten** Antworttext gesucht statt zeilengebunden — Reihenfolge und Position spielen keine Rolle mehr; Begründungstext vor **und** nach den Blöcken wird erfasst (vorher ging Text nach dem K-Block verloren). Nur die Stufe des **Standard-Prompts** (blankes Wort ohne Raute, `ki_client.py::AEHNLICHKEIT_PROMPT`) bleibt bewusst an Zeile 1 gebunden, da sich „gut"/„schlecht" ohne Raute sonst nicht von ihrem normalen Vorkommen in einer Begründung unterscheiden ließe (False-Positive-Schutz getestet). Neue Helfer: `_finde_stufe` (zweistufige Suche), `_grammatik_aus_text`, `_sammle_begruendung`; `_stufe_aus_zeile` → `_stufe_aus_marker` umbenannt.
- **Automatische Korrektur-Übernahme statt reiner Anzeige:** In `app/modul/mod_sprachdatei.py::_bewerte_row` (Zeilen-Button „Bewerten") wurde ein gelieferter Verbesserungsvorschlag bisher nur zur Anzeige gespeichert, nicht übernommen — jetzt automatisch übernommen samt frischer Rückübersetzung (analog zum Batch-Lauf). `app/modul/sprachdatei_lauf.py::neu_uebersetze_zeile` und `quelltext_uebernehmen` nutzten dafür `uebersetzung.bewerte_aehnlichkeit`, das laut eigener Doku die Korrektur verwirft — beide auf `bewerte_und_korrigiere` direkt umgestellt; `bewerte_aehnlichkeit` dadurch ohne verbleibende Aufrufer entfernt.
- **„Neue Bewertung" entfernt, Logik in „Neu übersetzen" integriert** (zur Übersichtlichkeit, zwei ähnliche Aktionen waren verwirrend): `neu_uebersetze_zeile` komplett neu aufgebaut — (1) vorwärts übersetzen, (2) rückübersetzen, (3) bei Übereinstimmung fertig, (4) sonst sinngemäße Bewertung, (5) eine Übersetzungs-Korrektur wird übernommen + erneut rückübersetzt/geprüft, (6) ein gemeldeter Grammatikfehler im Ausgangstext wird übernommen (`uebernehme_grammatik_quelle`, wie in `lauf()`/`phase3_kern`) und der komplette Ablauf beginnt **einmalig** von vorn (maximal eine Wiederholung; Schritt 5 wird übersprungen, wenn Schritt 6 ohnehin einen Neustart auslöst — sonst würde eine Korrektur-Rückübersetzung sofort wieder verworfen). `ki_geaendert` wird per Diff (`ueb != ueb_vorher` des letzten Durchlaufs) statt als durchgereichtes Flag berechnet, um eine durch den Neustart verworfene Korrektur aus Durchlauf 1 nicht fälschlich als „KI-geändert" zu markieren. `_bewerte_row` sowie der zugehörige Button „Neue Bewertung" in `_set_row` entfernt; `_retranslate_row` übernimmt jetzt Key/`src_ts`, gibt sie weiter und übernimmt die neue `ok`-Berechnung nach Batch-Muster (`ok = (not ist_unstimmig) if angewendet else bewertung in BEWERTUNG_OK`).
- **i18n-Aufräumung:** Verwaiste Keys `dlg.sprachdatei.btn_neu_bewertung`/`_tt` per `lang_tools.unused_keys()`/`entferne_keys()` aus `language.json` und allen 16 `language.<code>.json`/`.review.json` entfernt (Tooling statt manueller Mehrfach-Bearbeitung).
- **Verifikation:** `python -m py_compile`/`python -m ruff check app` ✓ nach jeder Änderung; `_parse_bewertung_korrektur` gegen reale und synthetische Fälle getestet (erweiterter Prompt in beliebiger Reihenfolge, Standard-Prompt mit False-Positive-Schutz, alte `##BESSER`/`##KEINVORSCHLAG`-Marker); `neu_uebersetze_zeile` mit Stub-`env` gegen vier Szenarien getestet (sofort stimmig, Korrektur, Grammatik-Quelle-Neustart mit Deckelung auf max. 1 Wiederholung, gleichzeitige Korrektur+Grammatik-Quelle mit korrektem Überspringen von Schritt 5); `unused_keys()` vor/nach der i18n-Bereinigung leer.
- **Dateien:** `app/uebersetzung.py`, `app/modul/mod_sprachdatei.py`, `app/modul/sprachdatei_lauf.py`, `app/language.json` + alle `language.<code>.json`/`.review.json`.
...
- **DB (v63 — nicht v62, das war bereits von der PDF-Signatur belegt):** `app/db/db_schema.py` + `app/DB-Pflege.py` (`_to_v63`) — `firma.archiv_pfad`/`archiv_pruef_jahre` (Default 10) und neue Mandantentabelle `archiv_dateien` (firma_id-isoliert, `UNIQUE(firma_id, export_id, dateiname)` für idempotenten INSERT OR REPLACE beim Backfill/Heilung). Migration läuft beim Programmstart.
- **DB-Funktionen (`app/db/db_buchungsexport.py`, firma-isoliert):** `get_archiv_dateien`, `save_archiv_dateien`, `get_buchungsexporte_ab_jahr`; `delete_buchungsexport` löscht die Archiv-Zeilen mit.
- **Kernmodul `app/archiv.py` (neu, UI-frei):** Pfad-/Hash-Helfer (`archiv_basis`/`export_ordner`/`datei_hash`), `archiviere_export` (Main-Thread, synchron beim neuen Export), sowie die Drei-Phasen-Prüfung `baue_pruef_auftrag` (Main-Thread, DB→reine Daten inkl. `firma_id`) → `fuehre_pruefung_aus` (Worker-Thread, **nur** Dateisystem) → `speichere_pruef_ergebnis` (Main-Thread, persistiert nur bei noch aktiver `firma_id`). Policy „hash=''": Heilung/Backfill immer neu aus `pdf_pfad` (Quelle = Wahrheit), sonst `NIE-ARCHIVIERT`. Bewusst **kein** `fallback_log.melde` (kein Ersatzwert im Beleg → CHECK-Logs + Warnfenster sind die Oberfläche, im Docstring begründet).
- **UI:** `app/settings.py` (`SUBDIR_ARCHIV`); Firmenstamm → Pfade „Verzeichnis für das Beleg-Archiv" (`mod_firma_pfade.py` alle 8 Touchpoints + `mod_firma_base.py::_browse_archiv`); Firmenstamm → Parameter → Steuerung „Beleg-Archiv prüfen (Jahre)" (`mod_firma_steuerung.py`, SpinBox 0–30). Warnfenster `app/modul/mod_archiv_warnung.py` (neu, non-modal, Recheck-Timer 60 s, schließt sich bei leerer Mängelliste). Integration in `app/modul/mod_buchungsexport.py`: `showEvent`-Prüfung (Guard + Throttle 10 min, Worker-Thread + Poll-QTimer 500 ms), Archivierung nach neuem Export (best effort), Archivordner-Löschung bei Undo/Storno, `closeEvent` räumt Timer/Fenster ab.
- **i18n (`app/language.json`):** neue Keys `archiv.col.*`, `archiv.status.*`, `archiv.warn.*`, `firma.dlg.archiv_verzeichnis`, `firma.pfade.archiv_verzeichnis`/`info_archiv`, `firma.steuerung.archiv_pruef_jahre`(+`.tooltip`) — DE+EN.
- **Verifikation:** `ruff check app` sauber; `python app/audit_firma_id.py` FEHLER=0 (neue Tabelle als 38. Mandantentabelle erkannt); `py_compile` aller geänderten Dateien; JSON valide. Qt-freier Pipeline-Integrationstest (Mock-DB): Archivierung+Hash, Prüfung heil, gelöschte Datei→FEHLT, veränderte Datei→HASH-DIFFERENZ, Backfill ohne Zeilen, `archiv_pruef_jahre=0`→keine Prüfung, CHECK-Logs geschrieben — alle bestanden. Offscreen-Import + Warnfenster (Füllen/leeren) ok. **Offen:** manueller GUI-Durchlauf in Testfirma 990 (Export erzeugen → Archiv + Logs; Datei löschen → Warnfenster; zurückkopieren → schließt binnen 60 s; Undo → Ordner weg).
...
- **Umsetzung (`app/doku.de.html`):** Neuer Abschnitt „Digitale Signatur der Beleg-PDFs" (id `drucken-signatur`) im Kapitel „Drucken & Journale" (nach Testdruck) + TOC-Eintrag. Inhalt: Zweck (Manipulationserkennung), Aktivierung (Firmenstamm → Parameter → Steuerung: „Zertifikat erzeugen" + Häkchen „PDF-Belege digital signieren"), Ablagepfad (Firmenstamm → Pfade), Testdruck-Ausnahme, Fehlerfall → Fehler-Nachverfolgung (`#fallback-protokoll`). `warn`-Box: selbst-signiert = gelbes Warndreieck, Integritätsschutz wirkt trotzdem.
- **Schwerpunkt (Unterabschnitt id `drucken-signatur-vertrauen`):** „Vertrauenswürdiges Zertifikat (grünes Häkchen)". Klargestellt, dass Vertrauen nicht „erstellt", sondern hergestellt wird. **Weg 1** (kostenlos, mit dem App-Zertifikat): Empfänger stuft das Zertifikat in Adobe Acrobat Reader einmalig als vertrauenswürdig ein — Schritt-für-Schritt; `tip`-Box mit Fingerabdruck-Abgleich (SHA-256) als Sicherheitshinweis; gilt pro Empfänger/Reader. **Weg 2** (kostenpflichtig, universell): Zertifikat eines Vertrauensdiensteanbieters (AATL/EUTL, z. B. D-Trust/GlobalSign/DigiCert); `warn`-Box mit ehrlichem Hinweis, dass das Einbinden eines gekauften `.p12`/`.pfx` derzeit **nicht** über die Programmoberfläche vorgesehen ist (kein Passwort-Eingabefeld — „Zertifikat erzeugen" legt stets ein selbst-signiertes an).
- **Verifikation:** HTML mit `html.parser` fehlerfrei geparst; neue Anker eindeutig (`drucken-signatur`, `drucken-signatur-vertrauen`); TOC-Ziel vorhanden; Link `#fallback-protokoll` gegen bestehenden Anker geprüft; Inhalt gegen Implementierung abgeglichen (Aktivierung/Pfad/Testdruck/Fallback stimmen).
- **DOKU-TODO:** doku.de.html-Teile des Signatur-Punkts entfernt; verbleibt nur der Admin-Readme-Hinweis (pyhanko/cryptography). Englische Doku (`doku.en.html`) offen für nächsten Übersetzungs-Durchgang.

## 2026-07-05 16:16 — Digitale PDF-Signatur der Beleg-PDFs (Manipulationserkennung, DB v62)

- **Anlass (Walter):** An Kunden versendete PDFs sollen nicht unbemerkt geändert werden können; Frage nach einer eingebauten Signatur. Nach Rückfrage festgelegt: **selbst-signiert** (kostenlos, Manipulation erkennbar), für **alle Belegtypen**; Signieren bei **jedem echten Druck** (nicht nur Festschreiben), Zertifikatspasswort **automatisch erzeugt** (kein Eingabefeld).
- **Konzept:** ReportLab erzeugt das PDF wie bisher; anschließend wird das fertige Original-PDF mit einem selbst-signierten Firmen-Zertifikat (PAdES/PKCS#7, inkrementell) signiert. Wird das PDF danach verändert, schlägt die Signaturprüfung im Reader fehl (intakt=False). Selbst-signiert ⇒ „gültig, Identität nicht verifiziert" (gelbes Dreieck); Integritätsschutz wirkt voll. Grünes Häkchen später nur durch Zertifikatstausch, ohne Code-Änderung.
- **Neues Modul `app/pdf_signatur.py`:** `ist_verfuegbar()`, `erzeuge_zertifikat(firma)` (RSA-2048 + selbst-signiertes X.509 → `.p12` via cryptography, Auto-Passwort `secrets.token_urlsafe`), `zertifikat_status(firma)`, `signiere_pdf(pdf, firma)` (pyHanko, Temp-Datei + atomares `os.replace`). Bibliotheken werden lazy importiert — App startet auch ohne sie.
- **DB v62 (beide Pflichtstellen):** `firma`-Spalten `pdf_signieren INTEGER DEFAULT 0`, `signatur_cert_passwort TEXT DEFAULT ''` (auto, admin-only/nie im UI sichtbar), `signatur_pfad TEXT DEFAULT ''` (Pfad-Definition). `DB-Pflege.py::_to_v62` (idempotent, PRAGMA-Prüfung) + `db/db_schema.py::_SCHEMA_SQL`; `CURRENT_VERSION` 61 → 62.
- **Pfad (CLAUDE.md-Regel):** kein Pfad in der DB; `settings.signatur_zertifikat_pfad(firma)` berechnet `{signatur_pfad}\{firmen_nr}\zertifikat.p12`, Fallback `{Exportpfad}\{SUBDIR_SIGNATUR}`. Neuer `SUBDIR_SIGNATUR` in `settings.py`.
- **Einhängung `druck_beleg.py`:** nach dem Merge/der E-Rechnung (ZUGFeRD bettet das unsignierte PDF ein) und **vor** E-Mail-Anhang/Öffnen; nur bei `firma.pdf_signieren` und echtem Druck (Testdrucke laufen über `_testdruck_beleg`, werden nie signiert). **Fallback-Tracking-Regel:** schlägt das Signieren fehl (Zertifikat/Passwort/Pakete), wird der Fall in `fallback_log` (ERROR.DB, modul „PDF-Signatur", firmennr-bezogen) protokolliert und der Benutzer gewarnt (`msg.pdf_signatur_fehler`); der Druck läuft unsigniert weiter.
- **UI:** Firmenstamm → Parameter → Steuerung: Checkbox „PDF-Belege digital signieren", Button „Zertifikat erzeugen" (mit Erneuern-Rückfrage), Statuslabel (fehlt / gültig bis …); Pakete fehlen → Button deaktiviert + Hinweis. Firmenstamm → Pfade: Feld „Signatur-Zertifikate" (`signatur_pfad`) inkl. Browse-Callback + Info-Label. Alle Strings über i18n (`app/language.json`, DE+EN).
- **Abhängigkeiten:** `pyhanko>=0.25`, `cryptography>=42.0` in `requirements.txt`.
- **Verifikation:** `ruff check app` (undefinierte Namen) ok; `py_compile` aller geänderten Module ok; `audit_firma_id.py` ohne FEHLER (nur bestehende 7 Warnungen). End-to-End (Scratchpad): Zertifikat erzeugt (gültig 10 J.), Test-PDF signiert → Validierung **intakt=True, valide=True, vertraut=False** (erwartet bei selbst-signiert); nach Byte-Manipulation **intakt=False** (Manipulation erkannt). Migration isoliert: `_SCHEMA_SQL` frische DB alle 3 Spalten; `_to_v62` ergänzt Bestands-DB, idempotent, Defaults `(0,'','')`. **Die produktive DB-Migration + UI-Laufzeittest löst der nächste App-Start (mit DB-Pflege-Backup) aus — bewusst nicht manuell migriert.**
...
- **Ursache:** Positionen (Verzugszinsen-Betrag inkl. Satz in Bezeichnung/Beschreibung) werden bei der Anlage in `mahnung_positionen` eingefroren, **aber** der Kopf-Infoblock (Zinssatz, Fälligkeit, Zahlbar-in-Tagen, Mahnstufen-Bezeichnung) wurde bei **jedem Druck live** aus `mahnkonditionen` + `basiszinssaetze` neu berechnet (`druck_daten.py::_lade_beleg_daten`). Änderung dieser Stammdaten nach dem Festschreiben verschob den Kopf.
- **Fix — Snapshot der Kopf-Werte (Umfang lt. Walter: ganzer Kopfblock, eingefroren beim ersten Echtdruck, Bestand aus Positionen rückgefüllt):**
  - **DB v60 (Schema-Regel: beide Stellen):** `mahnungen.mahnung_snapshot TEXT DEFAULT ''` in `db_schema.py::_SCHEMA_SQL` **und** `DB-Pflege.py::_to_v60` (+ MIGRATIONEN, CURRENT_VERSION 59→60, Kopf-Doku „Nächste freie v61"). Backfill festgeschriebener Bestands-Mahnungen: Zinssatz aus der höchsten (aktuellen) Verzugszinsen-Position (`Bezeichnung „… (X%, … Tage)"`) per Regex zurückgewonnen (Kopf = Position); übrige Kopf-Werte bleiben in Altbelegen live (kein eingefrorener Ursprung).
  - **`db_belege.py::save_mahnung_snapshot`** (firma-isoliert via `_update_firma`, nur wenn noch leer) friert `{mahnstufe_text, zahlungstage, falligkeit, zinssatz, zinssatz_fallback}` als JSON ein.
  - **`druck_beleg.py`:** beim ersten Echtdruck (wo `erstellungsdatum` gesetzt wird) für Mahnungen `save_mahnung_snapshot` aus den bereits live berechneten `daten` — analog zum `save_festgeschrieben` der Rechnung.
  - **`druck_daten.py::_lade_beleg_daten`:** für Mahnungen überschreibt ein vorhandener `mahnung_snapshot` die Live-Werte **je Schlüssel** (Snapshot vor Live) — volle Snapshots (neue Belege) frieren alle vier Werte ein, Backfill-Snapshots (Bestand) nur den Zinssatz.
- **Dateien:** `app/db/db_schema.py`, `app/DB-Pflege.py`, `app/db/db_belege.py`, `app/druck_beleg.py`, `app/druck_daten.py`.
- **Verifikation:** `ruff check app` ok; `py_compile` ok; `audit_firma_id.py` — keine FEHLER (neue Schreibmethode firma-isoliert); Migration `_to_v60` gegen synthetische In-Memory-DB — höchste Stufe gewinnt (6.28), ganzzahlig „6", keine Verzugszinsen-Position → kein Snapshot, nicht festgeschrieben → kein Snapshot, idempotent; `_lade_beleg_daten` mit Stub — ohne Snapshot live 6.28, Snapshot 6.28 schlägt geänderten Basiszinssatz (live wäre 8.0), voller Snapshot friert auch Fälligkeit/Tage/Stufe/Fallback ein; frisches `_SCHEMA_SQL` legt `mahnung_snapshot` an. Migration **nicht** manuell auf der echten DB ausgeführt (läuft beim nächsten Programmstart).
...
- **Neu `app/modul/sprachdatei_lauf.py` (498 Z., kein PyQt6-Import):** Qt-freier Kern der Lauf-Pipeline. `LaufUmgebung` bündelt Firmen-/Sprachkontext + 8 Callbacks (`ki_call`, `set_row`, `persist`, `token_status` (i18n-Key), `token_tick`, `fortschritt(phase, i, n)` mit Phasen-Codes `vor`/`rueck`/`pruefung`/`aehnlichkeit`/`retry`, `ist_abbruch`); `quellwerte` wird per Referenz geteilt (Grammatik-Übernahmen aktualisieren in place). Funktionen: `lauf` (3 Phasen je Batch, liefert `abgebrochen`), `phase3_kern`, `retry_zeile`, `batch_retry_lauf`, `neu_uebersetze_zeile`, `quelltext_uebernehmen` (Entwickler-Quell-Edit Schritte 1–5 inkl. `i18n.reload` + In-place-Quellwerte-Refresh), `uebernehme_grammatik_quelle` sowie die puren Helfer `unstimmig`, `bestimme_keys`, `fehlende_keys`, `bewertung_rang`; Konstanten `KONTEXT`, `MAX_RETRY`. Logik 1:1 übernommen, Reihenfolge `set_row → fortschritt → persist → token_tick` und alle Abbruch-Prüfpunkte erhalten. Hinweis: `uebersetzung` importiert selbst PyQt6 (Testmodus-Dialoge, Trennung 2026-06 bewusst verworfen) — headless heißt: ohne QApplication lauffähig.
- **Neu `app/modul/sprachdatei_dialoge.py` (221 Z.):** `_TextEditDialog`, `_FortschrittDialog`, `_MarkerHighlightDelegate` 1:1 verschoben (Klassennamen unverändert → gespeicherte Dialog-Geometrien bleiben gültig).
- **`app/modul/mod_sprachdatei.py` (1.593 Z., vorher 2.116):** behält `SprachdateiDialog` (Aufbau, Laden/Anzeigen, `_set_row`, Beherrschung, `_persist_still`/`_save`, `_ki_call`). Neuer Helfer `_lauf_umgebung(firma)` baut die `LaufUmgebung` (Fortschritts-Closure: Phase→Text, `scrollToBottom` nur bei „vor", `processEvents`). `_run`/`_lauf`/`_retranslate_row`/`_pruefe_aehnlichkeit`/`_batch_retry`/`_edit_quelle` sind dünne Wrapper (Validierung, Rückfragen, `_set_running`, Fehlerdialoge); `_bewerte_row` (Einzel-KI-Aufruf) bleibt im Dialog. Kleine bewusste Abweichung: das redundante `processEvents()` am Zeilenende von `_batch_retry` entfällt (der `fortschritt`-Callback der Folgezeile pumpt sofort; während der KI-Aufrufe pumpt `_ki_call` ohnehin).
- **Verifikation:** `ruff check app` sauber; `py_compile` aller drei Dateien; `audit_firma_id.py` Exit 0; kein `PyQt6`-Import in `sprachdatei_lauf.py` (nur Docstring-Erwähnung); Headless-Smoke (Scratchpad, `uebersetzung`-Funktionen gepatcht, ohne QApplication): kompletter Lauf mit Korrektur-Übernahme in Phase 3, Benutzer-Abbruch nach Phase 1, `phase3_kern`-Retry-Pfad, `batch_retry_lauf` — alle 4 Tests grün; Offscreen-Import-Smoke `modul.mod_sprachdatei` + `main` ok. Interaktiver Test durch Walter (Firma 990) steht aus.
- **Dateien:** `app/modul/mod_sprachdatei.py`, `app/modul/sprachdatei_lauf.py` (neu), `app/modul/sprachdatei_dialoge.py` (neu), `DEVLOG.md`.
...
- **Entdeckter Bug:** Die bestehende Anthropic-„Reasoning genutzt" + „Budget genutzt"-Kombination sendete `thinking: {type: "enabled", budget_tokens: N}` — das ist auf neueren Anthropic-Modellen (u. a. `claude-sonnet-5`, das aktuell konfigurierte Modell von Firma 990) **entfernt** und liefert HTTP 400. Live gegen die echte API verifiziert: der alte Body scheitert mit „HTTP Error 400: Bad Request", `thinking: {type: "adaptive"}` (mit/ohne `output_config.effort`) funktioniert einwandfrei.
- **`app/ki_client.py`:** `firma_reasoning(firma, task=None)` — neuer optionaler `task`-Parameter. Bei `anbieter == "anthropic"` liest die Funktion jetzt `ki_anthropic_effort_<task>` und liefert immer `{"effort": <wert-oder-leer>}` (nie mehr `None`); bei openrouter/lokal unverändert (`reason_aktiv`/`budget_aktiv`-Logik). `_apply_reasoning()`: Anthropic-Zweig komplett ersetzt — setzt **immer** `thinking: {"type": "adaptive"}` und ergänzt `output_config.effort`, falls ein Wert vorliegt; die alte `enabled`/`budget_tokens`/`disabled`-Fallunterscheidung ist entfernt. `ki_client.uebersetze()` übergibt jetzt `task="uebersetzung"` an `firma_reasoning()`.
- **`app/uebersetzung.py`:** Die vier `firma_reasoning(f)`-Aufrufe der drei relevanten Aufgaben bekommen den passenden `task`: `uebersetze_batch` (dynamisch `"rueckuebersetzung"`/`"uebersetzung"` je nach `rueck`), `bewerte_und_korrigiere` (`"bewertung"`), `uebersetze_rueck` (`"rueckuebersetzung"`), `_uebersetze_schritt` (`"uebersetzung"`). `_frage_beherrschung`/Rechtschreibprüfung/Sprachcheck bleiben ohne `task` — dort greift automatisch reines adaptives Thinking (kein Schema für diese Aufgaben nötig).
- **`app/db/db_schema.py` + `app/DB-Pflege.py` (Schema v56):** drei neue Spalten `firma.ki_anthropic_effort_{uebersetzung,rueckuebersetzung,bewertung}` (TEXT, Default `''`). Nur eine Spalte je Aufgabe (nicht je LLM 1/2 dupliziert) — konsistent mit der bereits bestehenden Un-Remapptheit der Anthropic-Reasoning-Spalten zwischen LLM 1/2. Die alten `ki_anthropic_reason_*`/`ki_anthropic_budget_*`-Spalten (v54) bleiben unbenutzt in der DB stehen (gleiches Muster wie `ki_llm_neuuebersetzung`).
- **`app/mod_firma_tabs/mod_firma_ki.py`:** Die Anthropic-Reasoning-/Budget-Zeile (`reason_anth`) ist aus `_build_llm_gruppe` (LLM 1 **und** LLM 2) entfernt — OpenRouter/lokal unverändert. In `_build_llm_zuordnung` bekommt jede der drei Aufgabenzeilen zusätzlich zur LLM-1/2-Auswahl ein Anthropic-Effort-Dropdown (Adaptiv/Niedrig/Mittel/Hoch/Sehr hoch/Maximal, neue Konstante `_EFFORT_OPTIONEN`) in derselben Zeile. `_aktive_reasoning()` (Test-Buttons) liefert für `anbieter == "anthropic"` jetzt `None` (kein Feld-Lookup mehr auf die entfernten Widgets) — Testaufrufe laufen dadurch mit reinem adaptivem Thinking.
- **`app/language.json`:** neue Schlüssel `firma.ki.effort_label`, `firma.ki.effort_hinweis`, `firma.ki.effort_opt_{adaptiv,low,medium,high,xhigh,max}`.
- **Verifikation:** `ruff`/`py_compile` sauber. Isolierte Logiktests von `firma_reasoning`/`_apply_reasoning` (ohne Task → adaptiv ohne Effort; mit Task+Wert → adaptiv+Effort; mit Task ohne Wert → adaptiv ohne Effort; OpenRouter-Pfad unverändert). Offscreen-UI-Test von `KiAnbindungTab` (Fake-Instanz ohne DB): alte Anthropic-Reasoning-Felder entfernt, drei neue Effort-Felder vorhanden, Laden/Speichern-Rundlauf über `_fill`/`_collect_data` korrekt. **Echter API-Test** gegen Firma 990/`claude-sonnet-5`: `effort="low"` und reines Adaptiv liefern beide `"OK"` ohne Fehler; der alte Body (`thinking.enabled`+`budget_tokens`) wurde separat gegen dieselbe API bestätigt mit HTTP 400 abgelehnt. Test-Daten im Token-Zähler danach zurückgesetzt.
- **Dateien:** `app/ki_client.py`, `app/uebersetzung.py`, `app/db/db_schema.py`, `app/DB-Pflege.py`, `app/mod_firma_tabs/mod_firma_ki.py`, `app/language.json`.
...
- **`app/ki_client.py`:** neue Funktion `_usage_normalisiert(daten)` liest das `usage`-Feld anbieterübergreifend aus (Anthropic-Stil `input_tokens`/`output_tokens`/`cache_read_input_tokens`/`cache_creation_input_tokens` vs. OpenAI-kompatibler Stil `prompt_tokens`/`completion_tokens`/`prompt_tokens_details.cached_tokens`) und liefert `None`, wenn gar kein `usage`-Objekt vorhanden ist (manche lokale Server) — dann wird nichts protokolliert statt irreführender Nullen. `chat_messages()`, `chat()` und `task_anfrage()` bekommen die neuen optionalen Parameter `firma_nr`/`task` und melden nach jedem erfolgreichen Aufruf über `token_log.melde()` — in einem eigenen `try/except`, damit ein Logging-Fehler nie den KI-Aufruf zum Absturz bringt. `_chat_completion_roh()`/`_extract_content()` bleiben unverändert.
- **`app/token_log.py` (neu):** eigene, von der Haupt-DB entkoppelte SQLite-Datei `daten/TOKENS.DB` nach dem Muster von `fallback_log.py` (kein Eingriff in die DB-Migrationskette). `melde()` (reiner Insert je Aufruf), `summe(firma_nr)` (aggregiert `GROUP BY anbieter, modell, task` inkl. Aufrufe/Zeitraum) und `reset(firma_nr)` (löscht den kompletten Zähler der Firma).
- **Aufrufstellen mit `firma_nr`/`task` versehen** (reine Zusatzparameter, kein Verhalten geändert): `app/uebersetzung.py` (`uebersetze_batch` → „uebersetzung"/„rueckuebersetzung", `bewerte_und_korrigiere` → „bewertung", `uebersetze_rueck` → „rueckuebersetzung", `_frage_beherrschung`/`pruefe_sprachbeherrschung` → „sprachbeherrschung", `_uebersetze_schritt` → „uebersetzung"), `app/ki_client.py::uebersetze()` → „uebersetzung", `app/modul/mod_artikel.py::_ki_korrektur` → „rechtschreibung", `app/mod_firma_tabs/mod_firma_laender.py::_sprachen_pruefen` → „sprachcheck", `app/mod_firma_tabs/mod_firma_ki.py` (`_ls_sprachen`/`_sprachen_ermitteln`, neuer Helfer `_firma_nr()`) → „test".
- **`app/modul/mod_token_verbrauch.py` (neu):** `TokenVerbrauchFenster` (1:1 nach dem Vorbild von `mod_fallback_protokoll.py`) zeigt die Aggregation je Anbieter/Modell/Aufgabe (Aufrufe, Eingabe-/Ausgabe-/Cache-Tokens, Zeitraum) und bietet einen „Zurücksetzen"-Button mit Rückfrage. In `app/main.py` als Menüpunkt „Token-Verbrauch" (Auswertungen-Menü) + `TAB_REGISTRY`-Eintrag registriert; bewusst **kein** Sidebar-Alarm-Button wie beim Fallback-Protokoll, da es hier keinen offen/erledigt-Zustand gibt (reine Statistik).
- **`app/language.json`:** neue Schlüssel `menu.token_verbrauch`, `tab.token_verbrauch`, `token.btn.zuruecksetzen`, `token.col.*` (9 Spalten), `token.msg.reset_confirm`, `token.titel`.
- **Verifikation:** `ruff check app` und `py_compile` sauber. `token_log.melde/summe/reset` per Skript getestet (Aggregation über zwei Aufrufe korrekt, Reset leert). `_usage_normalisiert` mit synthetischer Anthropic-, OpenAI-kompatibler und leerer Antwort geprüft (liefert korrekt normalisierte Werte bzw. `None`). Fehlerpfad mit ungültigem API-Key ausgelöst — der bekannte `RuntimeError` wird weiterhin geworfen, kein zusätzlicher Crash durch das Logging. **Echter End-to-End-Test** mit Firma 990 (aktiver Anthropic-Key, Modell `claude-sonnet-5`): `ki_client.chat(...)` liefert Antwort **und** `token_log.summe('990')` zeigt den plausiblen Verbrauch (17 Eingabe-/4 Ausgabe-Tokens); danach zurückgesetzt (kein Test-Artefakt in der Firmen-Statistik verblieben).
- **Dateien:** `app/ki_client.py`, `app/token_log.py` (neu), `app/uebersetzung.py`, `app/modul/mod_artikel.py`, `app/mod_firma_tabs/mod_firma_laender.py`, `app/mod_firma_tabs/mod_firma_ki.py`, `app/modul/mod_token_verbrauch.py` (neu), `app/main.py`, `app/language.json`.

## 2026-07-02 13:48 — App-Sprachen-Generator: Vorschläge ohne Rückfrage übernehmen, Bestätigung über frische Rückübersetzung

- **Anforderung (Walter):** Verbesserungsvorschläge (Grammatik-/Stil-Vorschläge, Grammatikfehler im Ausgangstext) ohne Bestätigungsdialog übernehmen. Bestätigt die anschließende frische Rückübersetzung die Übersetzung (Quelle == Rückübersetzung), gilt die Zeile als bestätigt.
- **`app/modul/mod_sprachdatei.py` — Dialoge entfernt:** `_frage_verbesserung`/`_frage_grammatik_quelle` (heute Vormittag für die Bestätigung gebaut, siehe 11:29/13:19) sind wieder raus; ersetzt durch die dialogfreie `_uebernehme_grammatik_quelle(key, neuer_text)` (aktualisiert `language.json` sofort, kein `KiKorrekturDialog` mehr).
- **`_lauf` (Phase 3) und `_phase3_kern`:** Jeder Vorschlag — Grammatik-Korrektur des Ausgangstexts (`GRAMMATIK_QUELLE`) **und** jede Genauigkeits-Korrektur (`gut`/`schlecht`/`sehr_gut`) — wird jetzt einheitlich ohne Rückfrage übernommen. **Neues Bestätigungskriterium:** Wurde etwas übernommen (`angewendet`), entscheidet **ausschließlich** eine frische Rückübersetzung des (ggf. neuen) Übersetzungstexts gegen den (ggf. neuen) Ausgangstext (`self._unstimmig(orig, rueck)`) über „bestätigt" — nicht mehr die rohe Bewertungsstufe der KI. Ohne Übernahme (identisch/sehr_gut ohne Vorschlag) zählt weiterhin die Bewertungsstufe, damit KI-Bewertung weiterhin False-Positives der reinen Rückübersetzungs-Prüfung auffangen kann. **Regressionslücke behoben:** Bei einer Grammatik-Korrektur *ohne* begleitende Genauigkeits-Korrektur fehlte die frische Rückübersetzung (stille Weiterverwendung der alten, gegen den alten Ausgangstext berechneten) — jetzt wird sie in diesem Fall gezielt nachgeholt (`rueck_frisch`-Flag), bei `gut`/`schlecht` bleibt die bereits von `_retry_zeile` gelieferte frische Rückübersetzung ohne Doppel-Aufruf erhalten.
- **`app/modul/mod_artikel.py::KiKorrekturDialog`:** optionaler `hinweis_html`-Parameter (heute Vormittag nur für die jetzt entfernten Dialoge gebaut) wieder entfernt — keine verbleibenden Aufrufer.
- **`app/language.json`:** verwaiste Schlüssel `dlg.sprachdatei.verbesserung_titel`/`grammatik_titel` entfernt (nur von den entfernten Dialogen genutzt).
- **Verifikation:** neues Scratchpad-Testskript ruft `_phase3_kern` mit gestubbten `uebersetzung.bewerte_und_korrigiere`/`uebersetze_rueck` sowie gestubbter `language.json`-I/O auf — bestätigt (1) sehr_gut+Korrektur wird ohne Dialog übernommen und über die frische Rückübersetzung bestätigt, (2) eine reine Grammatik-Korrektur (ohne Genauigkeits-Korrektur) löst ebenfalls eine frische Rückübersetzung aus und bestätigt korrekt (Regressionstest für die oben behobene Lücke). Bestehende Offscreen-Smoke-/Parser-Tests weiterhin grün; `ruff`/`py_compile` sauber.
- **Dateien:** `app/modul/mod_sprachdatei.py`, `app/modul/mod_artikel.py`, `app/language.json`.
...
- **`app/modul/mod_sprachdatei.py::SprachdateiDialog._build`:** Die Farberklärung (bisher Grid-Spalte neben Hinweiszeile/Batchgröße) ist jetzt ein eigener Rahmen am rechten Rand (`kopf_zeile`: linke Spalte mit Stretch 1, Farberklärung ohne Stretch, oben ausgerichtet); das `QGridLayout` dafür entfällt. `form.setFieldGrowthPolicy(FieldsStayAtSizeHint)` hält die Eingabefelder auf natürlicher Breite, statt sich über die von der breiten Tabelle vorgegebene Dialogbreite zu strecken. Die beiden Checkboxen („Alle neu übersetzen", „Alle anzeigen") sind jetzt Zeilen desselben Formulars direkt im Anschluss an Quellsprache/Sprache/Code/Name, statt eines separaten, unabhängig eingerückten Blocks. Im zweiten Schritt sind auch „KI-Modell" (neue Zeile, zeigt nur noch den Modellnamen ohne „LLM: "-Präfix, da die Zeilenbeschriftung das jetzt übernimmt) und „Batch-Größe" reguläre Formularzeilen geworden (vorher eigene Hinweis-/Batch-Zeile außerhalb des Formulars) — alle Anzeige-/Eingabefelder stehen dadurch auf derselben Feldspalten-Position. Die verbleibende Hinweiszeile zeigt nur noch Anzahl-Hinweis und Sprachbeherrschung.
- **`app/language.json`:** neuer Schlüssel `dlg.sprachdatei.kimodell` („KI-Modell"/„AI model"); `dlg.sprachdatei.llm` (bisheriger „LLM: {modell}"-Text, durch die Zeilenbeschriftung ersetzt) sowie die durch die vorherige „Durchläufe"-Entfernung verwaisten Schlüssel entfernt.
- **Verifikation:** Offscreen-Qt-Smoke (`SprachdateiDialog` baut sich fehlerfrei auf) + Screenshot-Vergleich (Eingabefelder kompakt, Checkboxen/KI-Modell/Batchgröße auf gleicher Feldspalten-Position, Farberklärung als Kasten rechts); `python -m ruff check app` und `py_compile` sauber.
- **Dateien:** `app/modul/mod_sprachdatei.py`, `app/language.json`.

## 2026-07-02 11:29 — App-Sprachen-Generator: ##BESSER-Bugfix, Grammatikprüfung mit Bestätigungsdialog, „Durchläufe" entfernt

- **Anforderung (Walter):** Den Bewertungs-/Korrektur-Prompt der App-Sprachen-Erstellung prüfen; bei gemeldetem Grammatikfehler das betroffene Item öffnen und die Übernahme bestätigen lassen; sicherstellen, dass ein `##VORSCHLAG`/`##BESSER`-Verbesserungsvorschlag auch tatsächlich übernommen wird. Zusätzlich: die „Durchläufe"-Einstellung entfernen (in der Praxis war immer ein Durchlauf ausreichend).
- **Bug gefunden und behoben (`app/uebersetzung.py::_parse_bewertung_korrektur`):** Eine per `##BESSER:` gelieferte Korrektur wurde bislang **immer** verworfen, sobald die Stufe „sehr_gut" war (`stufe in BEWERTUNG_OK` = `("identisch","sehr_gut")` erzwang eine leere Korrektur) — unabhängig davon, ob die KI tatsächlich einen Vorschlag geliefert hat. Jetzt erzwingt nur noch `"identisch"` eine leere Korrektur; ein echtes `##BESSER` bei `sehr_gut` kommt an.
- **Live-Prompt-Divergenz entdeckt:** Der in Firma 990 hinterlegte Bewertungs-Prompt (`ki_prompt_aehnlichkeit`, über den Markdown-Prompt-Editor selbst editiert) wich strukturell vom Parser ab — Zeile 1 war neu ein Grammatik-Ergebnis (`GRAMMATIK_QUELLE`/`GRAMMATIK_ZIEL`/`GRAMMATIK_OK`), die Genauigkeits-Stufe damit auf Zeile 2 verschoben; zusätzlich wurden Korrekturen teils im neuen Klammer-Format `<([{...}])>`, teils weiter mit den alten `##`-Markern verlangt. Mit Walter abgestimmt: Parser wird **tolerant für beide Prompt-Varianten** (Standard-Prompt bleibt unverändert), neues Zwei-Marker-Format `<(G[...]G)>` (Grammatik-Korrektur) / `<(K[...]K)>` (Genauigkeits-Korrektur) wird bevorzugt geparst, mit Rückfall auf die alten `##`-Marker.
- **`_parse_bewertung_korrektur`** liefert jetzt ein 5-Tupel `(stufe, begruendung, korrektur, grammatik, grammatik_korrektur)`: erkennt automatisch, ob Zeile 1 ein Grammatik-Ergebnis ist (Firma-990-Stil) oder direkt die Genauigkeits-Stufe (Standard-Prompt); `grammatik_korrektur` nur bei `grammatik in ("quelle","ziel")` gefüllt; ein im Rumpf enthaltener `<(G[...]G)>`-Block wird vor der Begründungs-Extraktion entfernt (leckte sonst in die Begründung). `bewerte_und_korrigiere` gibt dasselbe 5-Tupel weiter; alle 6 Aufrufstellen in `mod_sprachdatei.py` sowie `bewerte_aehnlichkeit` (weiterhin 2-Tupel-Wrapper) angepasst.
- **`app/modul/mod_sprachdatei.py` — neue Bestätigungsdialoge** (analog zur bestehenden Quelltext-Rechtschreibprüfung, `KiKorrekturDialog`):
  - `_frage_verbesserung(aktuell, vorschlag)`: bei bereits „sehr gut" bewerteter Übersetzung mit Korrektur (Grammatik-/Stil-Vorschlag) — Dialog statt automatischer Übernahme (Bedeutung stimmt schon).
  - `_frage_grammatik_quelle(key, aktuell, vorschlag)`: bei gemeldetem `GRAMMATIK_QUELLE` (Grammatikfehler im Ausgangstext) — Dialog, bei Bestätigung wird der Quelltext in `language.json` aktualisiert (wie `_rechtschreibpruefung`), die Zeile gilt danach als unstimmig.
  - Eingebaut in `_lauf` (Phase 3 der App-Sprachen-Erstellung) und `_phase3_kern` (Button „Sinngemäße Übereinstimmung prüfen"); bei „gut"/„schlecht" bleibt die automatische Übernahme + iterative Verbesserung (`_retry_zeile`) unverändert.
- **„Durchläufe"-Einstellung entfernt:** Spinbox samt Label aus dem Dialog entfernt (Hinweiszeile mit Anzahl/LLM/Sprachbeherrschung an ihre Stelle gerückt), `_lauf`/`_phase_fortschritt` auf einen einzigen batchweisen Durchgang vereinfacht (kein Runden-Parameter, keine „nächster Durchlauf nur offene Zeilen"-Logik mehr); verwaiste i18n-Schlüssel (`durchlaeufe`, `durchlaeufe_tt`, `confirm_runden`, `lauf_fortschritt_runde`) aus `language.json` entfernt. Offene Zeilen lassen sich weiterhin gezielt über „Nur fehlende übersetzen" / „Sinngemäße Übereinstimmung prüfen" nachbearbeiten.
- **Firma 990:** Bewertungs-Prompt in der DB direkt (nicht per Migration, reine Konfigurationsdaten) auf eine intern konsistente Fassung mit dem neuen Zwei-Marker-Format aktualisiert (weiterhin über den Markdown-Prompt-Editor im Firmenstamm änderbar).
- **Verifikation:** Scratchpad-Testskript (10 Fälle) für `_parse_bewertung_korrektur` — Standard-Prompt unverändert (inkl. Bugfix-Fall), erweiterter Prompt (Grammatik-Zeile + beide Klammer-Marker, auch kombiniert), unklare Antwort — alle OK; Offscreen-Qt-Smoke öffnet `SprachdateiDialog` (keine tote `_durchlaeufe_spin`-Referenz) und `KiKorrekturDialog` fehlerfrei; `python -m ruff check app` und `py_compile` sauber.
- **Dateien:** `app/uebersetzung.py`, `app/modul/mod_sprachdatei.py`, `app/language.json`.
...
- **Umsetzung:** `app/modul/mod_sprachdatei.py::_build` — „Durchläufe" und „Batchgröße" aus dem großen `QFormLayout` herausgelöst und in ein eigenes `QGridLayout` (`dl_grid`) verschoben, damit ein `QFrame` mit `rowSpan=2` rechts neben beiden Zeilen Platz findet (vorher/nachher unverändert: gleiche Widgets, gleiche Reihenfolge, nur andere Layout-Container — Tab-Reihenfolge bleibt erhalten, da Qt sie über die Kind-Widget-Erstellungsreihenfolge im Dialog bestimmt, nicht über das jeweilige Layout-Objekt). Der Rahmen listet die vier Farben aus der Review-Tabelle: rot (Zeilentext, unstimmig), grün (Zeilentext, „identisch" bewertet), gelber/roter Stern (KI-Bewertung „gut"/„schlecht"), roter Hintergrund im Übersetzungstext (fehlerhafter `{…}`-Marker). Farben kommen aus `theme.color(...)` — derselben Quelle wie die echte Tabellen-Darstellung — statt hartkodiert, damit Hell-/Dunkelmodus übereinstimmen.
- **i18n:** 6 neue Keys `dlg.sprachdatei.legende_*` (Titel, rot, grün, stern_gut, stern_schlecht, marker) in `language.json`, DE+EN, ohne `ts`/`h` — werden beim nächsten Öffnen des Dialogs automatisch nachgezogen (`_stamp_main_silent`).
- **Verifikation:** `ruff check app` ✓, `py_compile` ✓. Offscreen-UI-Smoke-Test (`QT_QPA_PLATFORM=offscreen`, Dialog per `__new__`+`QDialog.__init__` ohne DB aufgebaut, `_build()` real ausgeführt): keine Exception, alle erwarteten Widgets vorhanden. Screenshot (`grab()`) bestätigt visuell: Rahmen mit 6 Zeilen (4 davon mit farbigem Quadrat rot/grün/gelb/rot) erscheint rechts neben **beiden** Zeilen Durchläufe und Batchgröße, spannt also korrekt über beide.
- **Nebenbefund:** `language.json` enthielt beim Commit zusätzlich bereits vorhandene, parallel von Walter eingepflegte Wortlaut-Korrekturen (DATEV-Bezeichnungen, Pfade-Texte) — im Commit dokumentiert, nicht Teil dieser Änderung.

## 2026-07-01 01:30 — Fix: Sprach-Generator-Lauf verwirft Phase-3-VORSCHLAG in Folge-Durchläufen

- **Anforderung (Walter):** Der Übersetzungslauf besteht aus 3 Phasen (1. Batch-Übersetzung, 2. Batch-Rückübersetzung, 3. Übereinstimmungsprüfung & Korrektur). Liefert Phase 3 einen `##VORSCHLAG`, wird er als Übersetzung übernommen — danach darf aber keine erneute Phase-1-Übersetzung mehr gestartet werden, da bereits eine verbesserte Übersetzung vorliegt.
- **Ursache:** `app/modul/mod_sprachdatei.py::_lauf` (Haupt-Lauf, Button „Start", „Durchläufe" > 1) startete jeden Folge-Durchlauf für noch offene Zeilen wieder bei Phase 1 (`uebersetzung.uebersetze_werte_batch(..., rueck=False, ...)` ausgehend vom Originaltext) — auch für Zeilen, die in Phase 3 des vorherigen Durchlaufs bereits einen `##VORSCHLAG` erhalten hatten. Dadurch wurde die Korrektur kommentarlos verworfen.
- **Fix:** Neues Dict `aktuelle_ueb` hält je Key die aktuell gültige Übersetzung über Durchläufe hinweg (befüllt aus Phase 1 in Durchlauf 1, aus der Phase-3-Korrektur in allen Durchläufen). Phase 1 läuft nur noch bei `runde == 1`; ab Durchlauf 2 wird `ueb_map` direkt aus `aktuelle_ueb` aufgebaut und der Batch startet mit Phase 2 (Rückübersetzung dieser bereits verbesserten Übersetzung). Phase 2/3 selbst arbeiten unverändert generisch mit `ueb_map` weiter.
- **Nicht betroffen:** `_phase3_kern`/`_retry_zeile`/`_pruefe_aehnlichkeit` (separate Aktion „Sinngemäße Übereinstimmung erneut prüfen" für bereits offene Zeilen ohne Phase 1/2) sowie `uebersetzung.py` — keine Änderung dort nötig.
- **Verifikation:** Headless-Skript (`uebersetzung.uebersetze_werte_batch`/`bewerte_und_korrigiere`/`uebersetze_rueck` gemockt, `_lauf` mit Fake-Objekten statt echtem Qt-Dialog aufgerufen): über 2 Durchläufe genau **ein** Phase-1-Aufruf (`rueck=False`); Durchlauf 2 übergibt exakt die in Durchlauf 1 erzeugte Korrektur an Phase 2/3 statt einer neu übersetzten Zeile. `ruff check app` ✓, `py_compile` ✓.
...
- **Abgestimmt:** Geltung für **alle** Modell-Stellen (5 lokale Server-Slots + OpenRouter/Anthropic je LLM-Gruppe). Wire-Format **anbieterabhängig automatisch**: lokal → `chat_template_kwargs.enable_thinking` + `max_tokens`; OpenRouter → `reasoning.enabled`/`reasoning.max_tokens`; Anthropic → `thinking.type`/`thinking.budget_tokens`.
- **Backward-compat:** „verwenden?"-Haken defaulten **aus** (`*_aktiv=0`) → ohne Haken bleibt der Request-Body unverändert (keine Verhaltensänderung für Bestandsfirmen/strenge Server).
- **DB (v54):** `app/db/db_schema.py` + `app/DB-Pflege.py::_to_v54` — firma: 24 neue Spalten (je LLM 1/2 und Anbieter openrouter/anthropic/lokal die 4 Felder `*_reason_aktiv`/`*_reason_an`/`*_budget_aktiv`/`*_budget`; die `*_lokal_*` sind Spiegel des aktiven Slots). firma_ki_lokal: 4 neue Spalten (Quelle je Slot). `CURRENT_VERSION` 53→54. `app/db/db_firma.py`: `get/save_firma_ki_lokal` um die 4 Felder erweitert.
- **`app/ki_client.py`:** neue `firma_reasoning(firma)` (liest die Reasoning-Spalten des aktiven Anbieters; `None` wenn kein Haken) + `_apply_reasoning(nutz, anbieter, reasoning)` (anbieterspezifischer Body-Eingriff, Anthropic-Budget-Clamp ≥1024 und `max_tokens > budget`). Optionaler `reasoning=None`-Parameter in `_chat_completion_roh`/`chat_messages`/`chat`/`task_anfrage`/`teste_prompt_caching`.
- **Aufrufer:** `app/uebersetzung.py` (alle chat-Stellen + `_firma_fuer_rueck` remappt die Reasoning-Spalten für openrouter+lokal parallel zu den Modell-Spalten), `app/modul/mod_artikel.py`, `app/mod_firma_tabs/mod_firma_laender.py` geben jetzt `firma_reasoning(...)` mit.
- **UI `app/mod_firma_tabs/mod_firma_ki.py`:** je LLM-Gruppe eine Reasoning-Zeile für openrouter und anthropic (sichtbarkeitsgesteuert über `toggle()`); im lokalen Slot-Editor zwei Zeilen (Haken+Combo / Haken+SpinBox), Werte pro Slot in `_lokal_slots`. `_value`/`_set_value`/`_connect_dirty` um `QSpinBox` ergänzt; `_set_value` data_mode-Fix (`str(v)` statt `str(v or "")`, sonst landete `reason_an=0` fälschlich auf „enabled"). `_collect_data` spiegelt den aktiven Slot in die `ki_lokal_*`/`ki_rueck_lokal_*`-Spalten. Test-Buttons (`_ls_test`/`_ls_sprachen`/`_sprachen_ermitteln`/`_ki_erreichbar_testen`) senden die aktuell eingestellten Reasoning-Werte mit (`_aktive_reasoning`/`_ls_reasoning`).
- **`app/language.json`:** neue Keys `firma.ki.budget_verwenden`, `firma.ki.reasoning_label`, `firma.ki.reasoning_verwenden`, `firma.ki.reasoning_an`, `firma.ki.reasoning_aus` (DE+EN+ts+h).
- **Verifikation:** `ruff check app` ✓, `py_compile` ✓, JSON ✓, `audit_firma_id.py` (Exit 0, keine Fehler) ✓. In-Memory: v54-Migration legt 24+4 Spalten mit Defaults an, idempotent, frisches `_SCHEMA_SQL` deckungsgleich ✓. `_apply_reasoning`/`firma_reasoning`-Tests (alle 3 Anbieter, Haken-aus → Body unverändert, Anthropic-Clamp) ✓. DB-Round-Trip firma_ki_lokal + Spiegelspalten ✓. Offscreen-UI-Smoke: 16 Gruppen-Felder registriert, Laden/_collect_data/_aktive_reasoning + lokaler Slot-Pfad + Slot-Wechsel-Persistenz ✓. Funktionstest (reale Antwortzeit) in Firma 990 = manuell; DB-Migration läuft beim nächsten App-Start über DB-Pflege.
...
- **`app/ki_client.py`:** `AEHNLICHKEIT_PROMPT` um die höchste Stufe `IDENTISCH` erweitert (vollständige, exakte Wiedergabe); 4 Stufen IDENTISCH/SEHRGUT/GUT/SCHLECHT; Korrektur nur noch bei GUT/SCHLECHT.
- **`app/uebersetzung.py`:** `BEWERTUNG_OK = ("identisch","sehr_gut")` (gelten als erledigt/stimmig); `_stufe_aus_zeile` erkennt IDENTISCH (Reihenfolge IDENTISCH→SEHRGUT→SCHLECHT→GUT); `_parse_bewertung_korrektur` liefert bei beiden OK-Stufen keine Korrektur; Begründungs-Strip um „identisch" ergänzt.
- **`app/modul/mod_sprachdatei.py`:** Zeilenfarbe in `_set_row` neu — unstimmig→rot, sonst `bewertung=="identisch"`→grün (`rating_sehr_gut`), sonst Standard (schwarz). `_bewertung_rang` um `identisch:3`; `_BEWERTUNG_FARBE` um `identisch`→grün (Stern/Buttons); alle `==/!= "sehr_gut"`-Vergleiche (Lauf-Phase 3, `_retry_zeile`-Stop, `_phase3_kern`, `_batch_retry`, `_bewerte_row`, `_retranslate_row`/`_edit_quelle`) auf `uebersetzung.BEWERTUNG_OK` umgestellt.
- **DB (Migration v53, `DB-Pflege.py`):** `_to_v53` hebt Bestandsfirmen beim Bewertungs-Prompt vom v52- auf den v53-Default — nur bei exaktem Treffer (eigene Anpassungen bleiben). `CURRENT_VERSION` 52→53. Keine Schema-Änderung.
- **`app/language.json`:** neuer Stufen-Label-Key `dlg.sprachdatei.bewertung_identisch`; `bewertung_sehr_gut` zur Abgrenzung umformuliert („Bedeutung gleich, kleine Formulierungsunterschiede").
- **Verifikation:** `ruff check app` ✓, `py_compile` ✓, JSON ✓. `:memory:`-Kette v50→v52→v53 == `ki_client.AEHNLICHKEIT_PROMPT`, individuell angepasste Firma unangetastet, v53 idempotent ✓. Parser-Tests IDENTISCH/SEHRGUT/GUT/SCHLECHT ✓. Funktionstest (Zeilenfarbe) in Firma 990 = manuell.

## 2026-06-30 10:15 — Sprach-Generator: Bewertungs- und Wiederholungs-Prompt zusammengelegt (LLM-Aufrufe nahezu halbiert)

- **Anforderung (Walter):** (1) Zeilen-Button „Neu mit Bewertung" hatte gefühlt keine klare Funktion → umbenennen in „Neue Bewertung", soll **nur** die vorhandene Übersetzung neu bewerten (keine Neuübersetzung). (2) Den Übereinstimmungs-Prompt so erweitern, dass der separate „zweite Übersetzungsversuch (mit Bewertung)" entfällt → LLM-Aufrufe im Nachübersetzungs-Pfad nahezu halbieren. Geklärt: **voller Umbau** (Rückübersetzung in der Schleife nur noch **einmal am Ende**, da sie nicht in die Entscheidung einfließt) und die überflüssigen Altfelder **aus der UI entfernen** (DB-Spalten bleiben).
- **`app/ki_client.py`:** `AEHNLICHKEIT_PROMPT` zum kombinierten Bewertungs-/Korrektur-Prompt erweitert (Zeile 1 = Stufe, Zeile 2 = Begründung, ab Zeile 3 = verbesserte Übersetzung, nur wenn nicht SEHRGUT). `UEBERSETZUNG_RETRY_PROMPT` entfernt.
- **`app/uebersetzung.py`:** neue `bewerte_und_korrigiere(...) → (stufe, begruendung, korrektur)` (ein LLM-Aufruf, leerer System-Prompt); zeilen-genauer Parser `_parse_bewertung_korrektur` (Stufe **nur aus Zeile 1**, damit „gut/schlecht" im Korrekturtext nicht stört) + Helfer `_stufe_aus_zeile`; `bewerte_aehnlichkeit` jetzt dünner Wrapper (gibt nur `(stufe, begruendung)` zurück). `uebersetze_mit_bewertung` entfernt.
- **`app/modul/mod_sprachdatei.py`:** Zeilen-Button → „Neue Bewertung" (`_bewerte_row`, reine Neubewertung). `_retry_zeile` neu: iterative Verbesserung allein über `bewerte_und_korrigiere`, bestes Ergebnis nach Rang behalten, Rückübersetzung **einmal am Ende**. `_phase3_kern`, Hauptlauf-`_lauf` (inline Phase 3) und `_batch_retry` entsprechend auf den kombinierten Aufruf umgestellt (3→ ~1 Aufruf pro Versuch); `TASK_NEU`/`llm_neu` nicht mehr genutzt.
- **`app/mod_firma_tabs/mod_firma_ki.py`:** Prompt-Feld „Zweite Übersetzung (Wiederholung)" und LLM-Auswahl „Neuübersetzung" entfernt; Ähnlichkeits-Feld-Kommentar aktualisiert; `MARKER_BEWERTUNG`-Import entfernt.
- **DB (Migration v52, `DB-Pflege.py`):** `_to_v52` hebt Bestandsfirmen beim Bewertungs-Prompt vom alten auf den neuen Default — **nur bei exaktem Treffer** (eigene Anpassungen bleiben). Keine Schema-Änderung; `ki_prompt_uebersetzung_retry`/`ki_llm_neuuebersetzung` bleiben als ungenutzte Spalten. `CURRENT_VERSION` 51→52. `app/db/db_firma.py`: Seed für `ki_prompt_uebersetzung_retry` entfernt.
- **`app/language.json`:** Zeilen-Button-Label „Neue Bewertung" + Tooltip; Label `firma.ki.prompt_aehnlichkeit` → „… & Korrektur"; obsolete Keys `firma.ki.prompt_uebersetzung_retry` und `firma.ki.llm_task.neuuebersetzung` entfernt.
- **Verifikation:** `ruff check app` ✓, `py_compile` ✓, `language.json` valides JSON ✓. `:memory:`-Test der Migration: alter Default → neuer (== `ki_client.AEHNLICHKEIT_PROMPT`), individuell angepasste Firma unangetastet, idempotent ✓. Parser-Tests (SEHRGUT/GUT/SCHLECHT, inline-Begründung, „gut/schlecht" im Korrekturtext, unklar) ✓. Migration läuft beim App-Start (nicht manuell ausgeführt); KI-Lauf in Firma 990 mit Test-LLM-Protokoll = manueller Test.

## 2026-06-29 09:30 — KI: zwei LLMs (einfach/intensiv) + Aufgaben→LLM-Zuordnung + Quelltext-Rechtschreibprüfung

- **Anforderung (Walter):** (A) Die Bedeutung der zwei LLMs ändern: LLM 1 = einfache Denkprozesse, LLM 2 = intensive. In der App-Übersetzung pro Aufgabe zuweisen, welches LLM sie ausführt (Aufgaben auf 4 echte zusammengefasst: Übersetzung, Rückübersetzung, Bewertung/Prüfung, Neuübersetzung; Default = bisheriges Verhalten; Bedienung im KI-Reiter); Belegverarbeitung immer LLM 1. (B) Zusätzlicher Button „Rechtschreibprüfung" im Generator: prüft Quelltexte mit mehr als zwei Wörtern auf Rechtschreibung/Grammatik/Interpunktion per KI (5. zuweisbare Aufgabe), zeigt Abweichungen mit Bestätigung, übernimmt sie in `language.json`, markiert die Übersetzung als unstimmig und hält je Item den Geprüft-Stand fest (nur Entwicklermodus, da Basis-Sprachdatei geändert wird).
- **DB (Migration v51, `DB-Pflege.py` + `db/db_schema.py`):** 5 neue `firma`-Spalten `ki_llm_uebersetzung`(1) / `ki_llm_rueckuebersetzung`(2) / `ki_llm_bewertung`(1) / `ki_llm_neuuebersetzung`(1) / `ki_llm_rechtschreibung`(1). Defaults = bisheriges Verhalten. `_to_v51` idempotent (PRAGMA-Prüfung), `CURRENT_VERSION` 50→51.
- **`uebersetzung.py`:** `TASK_*`-Konstanten, `llm_nr_fuer_task(firma, task, default)`, `_firma_fuer_llm(firma, llm_nr)`; optionaler `llm_nr`-Parameter in `uebersetze_batch`/`uebersetze_werte_batch`/`uebersetze_rueck`/`bewerte_aehnlichkeit`/`uebersetze_mit_bewertung`/`baue_ctx` (→ `ctx["llm_nr"]` in `_uebersetze_schritt`). Defaults erhalten das aktuelle Verhalten; **Belegpfad unverändert (immer LLM 1)**. Neue Funktion `pruefe_rechtschreibung(firma, sprache, text, llm_nr)` über `ki_prompt_rechtschreibung` (leerer System-Prompt).
- **`mod_firma_tabs/mod_firma_ki.py` + `language.json`:** LLM-Gruppen umbenannt („LLM 1 — einfache Denkprozesse" / „LLM 2 — intensive Denkprozesse"); neue Gruppe „App-Übersetzung: LLM-Zuordnung" mit 5 Auswahlfeldern (LLM 1/2) gebunden an die neuen Spalten + Hinweis „Belegverarbeitung immer LLM 1".
- **`modul/mod_sprachdatei.py`:** an allen Generator-LLM-Aufrufstellen (`_lauf`, Einzel-Neuübersetzung, `_retry_zeile`, `_phase3_kern`, `_edit_quelle`) die per Firma aufgelöste `llm_nr` mitgegeben. Neuer Button „Rechtschreibprüfung" (nur Entwicklermodus) + `_rechtschreibpruefung`/`_rs_kandidaten`: prüft Quelltexte (> 2 Wörter, noch nicht geprüft), zeigt Korrektur über `KiKorrekturDialog`, schreibt Übernahme + Geprüft-Marker nach `language.json`, markiert die Zeile unstimmig, persistiert je Item (abbruchsicher).
- **`lang_tools.py`:** Rechtschreib-Marker je Item+Quellsprache (`rs_<code>`): `rs_text_hash`, `rs_geprueft`, `setze_rs` — beim nächsten Start werden nur neue/geänderte Items erneut geprüft.
- **Verifikation:** `ruff check .` ✓, `py_compile` ✓, `audit_firma_id.py` nur vorbestehende WARNUNGEN. `:memory:`-Test: frisches Schema hat die 5 Spalten (Defaults 1/2/1/1/1), `_to_v51` idempotent, `CURRENT_VERSION=51` registriert. Import-/i18n-Smoke ✓. Migration auf der echten DB läuft beim App-Start (nicht manuell ausgeführt); KI-Lauf + Rechtschreib-Lauf in Firma 990 = manueller Test.

## 2026-06-29 08:05 — Sprach-Generator: Lauf als abbruchsichere Pro-Batch-Pipeline (3 Phasen je Batch)

- **Anforderung (Walter):** Den Lauf „Erstellen/aktualisieren" so zerlegen, dass **jeder Batch vollständig** durchläuft (übersetzen → rückübersetzen → sinngemäße Prüfung), bevor der nächste Batch startet. Bisher wurden alle Items global vorwärts, dann global rückwärts übersetzt; bei einem Abbruch waren **alle** Übersetzungen verloren (Stand nur im Speicher, erst „Speichern" schrieb auf Platte). Geklärt: **nach jeder Phase/Zeile** persistieren; bei sinngemäß „schlecht" genau **eine** Neuübersetzung mit Bewertung, die als Ergebnis stehen bleibt; die **„Durchläufe"** umschließen den batchweisen Durchgang (Standard 1 = ein Gesamtdurchlauf); nach dem Lauf entscheidet der Anwender manuell weiter.
- **`app/modul/mod_sprachdatei.py`:**
  - `_save()` in stillen Kern `_persist_still()` (schreibt Tabellenstand → `language.<code>.json` + `.review.json` ohne Erfolgsmeldung/Reload/Combo-Neuaufbau, liefert `(n_ueb, n_ok)`, OSError propagiert) und UI-Wrapper `_save()` aufgeteilt.
  - `_lauf()` komplett auf eine **Pro-Batch-Pipeline** umgebaut: äußere „Durchläufe"-Schleife, darin Batch-Schleife in Batchgröße. Je Batch nacheinander Phase 1 Vorwärts (`uebersetze_werte_batch`, ein Batch-Aufruf mit Mismatch-Fallback), Phase 2 Rückübersetzung (markiert Unstimmigkeiten per `_unstimmig`), Phase 3 sinngemäße Prüfung der auffälligen Zeilen (`bewerte_aehnlichkeit`); bei „schlecht" eine `uebersetze_mit_bewertung` + frische `uebersetze_rueck`, Ergebnis bleibt stehen, Zeile bleibt offen. `_persist_still()` nach jeder Phase und nach jeder geprüften Zeile. Offen gebliebene Zeilen (nicht „sehr gut") gehen in den nächsten Durchlauf. Abbruch wirkt zwischen Batches/Phasen/Zeilen.
  - Fortschrittsanzeige je Phase über `_phase_fortschritt` mit den Labels „Übersetzen / Rückübersetzen / Sinngemäße Prüfung".
  - `_pruefe_aehnlichkeit` (manueller Button) auf den wiederverwendbaren Kern `_phase3_kern` + Sammelhelfer `_offene_rote_zeilen()` umgestellt (für die manuelle Nachbearbeitung nach dem Lauf).
  - `_run()`: separater Auto-Aufruf `self._pruefe_aehnlichkeit(auto=True)` entfällt — die Prüfung ist jetzt Teil jedes Batches.
...
- **Verifikation:** `python -m ruff check app` ✓, `py_compile` ✓, `audit_firma_id.py` nur vorbestehende WARNUNGEN (keine DB berührt), Import-Smoke ✓. GUI-/LLM-Lauf in Firma 990 (Abbruch mitten im Lauf → fertige Batches vollständig in den Dateien) steht als manueller Test aus.
...
- **Hintergrund:** Die App lud bisher keine eigene Schrift; Windows-Systemschriften decken Latein/Griechisch/Kyrillisch (alle geseedeten europäischen Sprachen) ab, nicht aber z. B. Khmer. `QFontDatabase.addApplicationFont` darf jederzeit aufgerufen werden → Schrift erst bei Bedarf registrieren, Qt nutzt sie danach automatisch als Glyph-Fallback.
- **Assets `app/fonts/`:** `NotoSans-Regular.ttf` (breiter Noto-Unterbau) + `NotoSansKhmer-Regular.ttf` (Khmer) + `OFL.txt` (SIL Open Font License, frei verteilbar). Über jsDelivr-Spiegel von `google/fonts` bezogen, mit fontTools validiert (gültiges SFNT, Khmer-Block U+1780–U+17FF mit 114 Codepoints abgedeckt).
- **`app/fonts.py` (neu):** `ensure_for_text(text)` registriert lazy + gecacht die für `text` nötigen Noto-Dateien. `_SCRIPT_RANGES` mappt Unicode-Blöcke, die Windows nicht abdeckt, auf eine Datei (aktuell Khmer); tritt ein solcher Block auf, wird zusätzlich der `NotoSans`-Unterbau geladen. Reiner europäischer Text lädt **nichts**. Fehlende/defekte Datei: `logging.warning` (rein kosmetisch, kein Datenersatz → bewusst **kein** ERROR.DB/Fallback-Tracking). Kein DB-Pfad (Konvention `app/fonts/`).
- **`app/modul/mod_sprachdatei.py`:** `import fonts`; in `_set_row` vor dem Setzen der Tabellen-Items `fonts.ensure_for_text(ueb)` (Zielsprachen-Spalte `COL_UEB` ist der einzige Ort mit potenziell nicht-lateinischem Text; Original/Rückübersetzung bleiben in der Basissprache).
- **Verifikation:** `ruff check app` ✓, `py_compile` ✓; Qt-Smoke (offscreen): Latein triggert nichts, Khmer registriert „Noto Sans Khmer" + „Noto Sans", Cache greift, alle Khmer-Glyphen `inFont` True; `import modul.mod_sprachdatei` fehlerfrei; `git check-ignore` bestätigt, dass die `.ttf`/`OFL.txt` committet werden; geprüft, dass das Verzeichnis `app/fonts/` das Modul `app/fonts.py` nicht verdeckt (`find_spec` → SourceFileLoader auf `fonts.py`).
...
- **Wiederverwendet:** der bestehende Prompt `firma.ki_prompt_sprach_faehigkeit` mit Fallback `ki_client.SPRACHE_FAEHIGKEIT_PROMPT` (Marker `{sprache}`, Skala 1–10), `ki_client.firma_cfg`/`chat`, sowie `uebersetzung._firma_fuer_rueck`/`_llm2_abweichend`/`vorwaerts_modell`/`rueck_modell`. Vorbild: `mod_firma_laender.py::_sprachen_pruefen`.
- **`app/uebersetzung.py`:** neu `SPRACHBEHERRSCHUNG_SCHWELLE = 6`, `_parse_note` (erste Ganzzahl 1–10, sonst None), `_frage_beherrschung` (ein Modell → `(modell, note, roh)`) und `pruefe_sprachbeherrschung(firma, ziel_sprache)` → `{"llm1", "llm2"|None, "ok"}`; `ok` nur, wenn alle geprüften Noten parsebar und ≤ 6 sind. LLM 2 wird nur bei abweichender Konfiguration zusätzlich gefragt.
- **`app/modul/mod_sprachdatei.py`:** Anzeige-Label `_beherrschung_label` hinter `_llm_label`; Session-Cache `_beherrschung_cache` + Status `_beherrschung_ok`; `_ensure_beherrschung` (Cache, Wartecursor, Statushinweis, Fehler→ok=False), `_zeige_beherrschung` (Note(n) „LLM1 / LLM2", grün/rot, Tooltip mit Roh-Antworten), `_apply_beherrschung_gate` (sperrt `_run`/`_fehlende`/`_aehnl`/`_schlecht`/`_gut`-Buttons) und `_beherrschung_gate` (harte Sperre mit Ablehnungsmeldung). Aufgerufen am Ende von `_on_combo` (Anzeige bei Auswahl) und als Vorab-Sperre in `_run`, `_batch_retry`, `_pruefe_aehnlichkeit` (nur manuell), `_retranslate_row`, `_retranslate_row_feedback`, `_edit_quelle`. `_set_running(False)` ruft das Gate erneut, damit die Sperre nach einem Lauf erhalten bleibt.
- **`app/language.json`:** neue Schlüssel `dlg.sprachdatei.beherrschung`, `…_tt`, `…_pruefe`, `…_abgelehnt`, `…_fehler` (DE+EN; `ts`/`h` werden vom Generator beim nächsten Öffnen gestempelt).
- **Bewusst nicht:** keine Persistenz in der `sprachen`-Tabelle, kein neues Firma-Prompt-Feld/keine Migration.
- **Verifikation:** `python -m ruff check app` ✓, `py_compile` ✓, `language.json` valide ✓. Headless-Smoke (gemocktes `ki_client.chat`): `_parse_note` 4→4, „Note: 7"→7, abc/0/11→None; gleiches LLM ⇒ `ok=True`, eine Prüfung; abweichendes LLM 2 (Note 8) ⇒ `ok=False`, beide Noten, 2 Aufrufe ✓.
...
- **Hintergrund:** Die i18n-Texte enthalten Format-Marker (`{sprache}`, `{n}`, …), die `i18n._` zur Laufzeit per `str.format` ersetzt. Der Batch-Übersetzungspfad (`uebersetzung.uebersetze_batch`) schickt den Rohtext ans LLM (anders als der Einzelpfad `_translate`, der Marker heraussplittet) — die KI kann Marker übersetzen/verstümmeln/weglassen. Ein kaputter Marker bricht `format()` → Rückfall auf den unübersetzten Quelltext. Die Review-Tabelle prüfte Marker bisher nicht.
- **`app/lang_tools.py`:** Qt-freie Single-Source-Logik: `_MARKER_RE = re.compile(r"\{[^}]*\}")` (spiegelt `uebersetzung._PLATZHALTER_RE`), `marker_liste(text)` und `marker_diff(orig, ueb) -> (fehlend, fremd)` (Multiset via `collections.Counter`, Reihenfolge egal). `import collections` ergänzt.
- **`app/modul/mod_sprachdatei.py`:**
  - Neuer `_MarkerHighlightDelegate(QStyledItemDelegate)` auf Spalte `COL_UEB`: rendert die Zelle als Rich-Text (`QTextDocument`), fasst jeden falschen Marker (aus `Qt.UserRole`) invers rot ein (`background error_fg` + `#ffffff`); Basis-Schriftfarbe aus `ForegroundRole` (bleibt rot bei unstimmig), bei Selektion `HighlightedText`. `sizeHint` erhält Word-Wrap/Zeilenhöhe.
  - `_set_row`: berechnet bei gefüllter Quelle+Übersetzung `lang_tools.marker_diff`; bei Abweichung → `unstimmig=True`, `ok=False`. `COL_UEB`-Item trägt `marker_fremd` in `Qt.UserRole` (Delegate-Quelle) und bei Fehler einen Tooltip (`dlg.sprachdatei.marker_fehler_tt`, nennt falsch/fehlend). Greift in allen Render-Pfaden (Lauf/Laden/Retry/Edit). Keine Persistenz/Schema-Änderung — bei jedem Laden neu berechnet.
  - Imports: `QStyledItemDelegate`, `QStyle`, `QStyleOptionViewItem`, `QSize`, `QRectF`, `QTextDocument`, `QPalette`, `re`.
- **`app/language.json`:** neuer Schlüssel `dlg.sprachdatei.marker_fehler_tt` (DE+EN; `ts`/`h` werden vom Generator beim nächsten Öffnen gestempelt).
- **Verifikation:** `python -m ruff check app` ✓, `py_compile` ✓, `language.json` valide + UTF-8-Umlaute ✓. Smoke: `marker_diff` liefert fremd/fehlend korrekt (inkl. Anzahl-Differenz `{n}`×2) ✓; Headless-Delegate-Smoke (offscreen): `_markup` fasst nur den falschen Marker `{numero}` ein (korrektes `{n}` unangetastet), `sizeHint` > 0, `paint` crashfrei, leerer/korrekter Fall ohne Span ✓; i18n-Schlüssel lädt DE+EN mit Platzhaltern ✓.
...
- **`app/ki_client.py`:** `UEBERSETZUNG_RETRY_PROMPT` von Anführungszeichen-Einrahmung (`"{Ausgangstext}"` …) auf **Markdown-Überschriften** umgestellt (`## Ausgangstext` / `## Bisherige Übersetzung` / `## Bewertung …` / `## Aufgabe`). Robust gegen `"` im Text und gegen mehrzeilige Items; bewusst Überschriften statt Code-Fences, um keine Backticks einzuführen.
- **`app/DB-Pflege.py`:** `_to_v48`-Snapshot auf das MD-Format gehoben (frische DBs); neue `_to_v49`, die Bestandsfirmen vom alten v48-`"`-Default auf das MD-Format hebt (nur exakte Default-Treffer; eigene Anpassungen bleiben). `CURRENT_VERSION = 49`, `MIGRATIONEN`-Eintrag.
- **`app/uebersetzung.py`:** neue Funktion `_bereinige_uebersetzung` — entfernt umschließende Code-Fences (```` ``` ````), Backticks und paarweise umschließende Anführungszeichen (gerade + typografisch); innere Anführungszeichen und Platzhalter bleiben. Angewandt auf alle Übersetzungs-Rückgaben: `_uebersetze_text` (Einzel-vorwärts), `uebersetze_rueck` (rück, mit Wiederholung), `uebersetze_batch` (je Item), `uebersetze_mit_bewertung` (Retry).
- **Hinweis:** `marker.anrede` (ES) wurde **nicht** in der Datei geändert (offene App) — nach App-Neustart (v49-Migration + neuer Code) per „Neu übersetzen" sauber neu generieren.
- **Verifikation:** `python -m ruff check app` ✓, `py_compile` ✓. Smoke: Bereinigung über 8 Fälle (Fences/Backticks/Anführungszeichen entfernt, innere `"` + Platzhalter unangetastet) ✓; RETRY-Prompt ohne `"{`-Einrahmung, mit MD-Überschriften ✓; `_to_v49` hebt alt→MD, eigene Anpassungen unangetastet, idempotent, `CURRENT_VERSION=49` ✓.
...
## 2026-07-13 15:10 — API-Keys/Secrets aus der DB in verschlüsselte Pro-Firma-Dateien (DB v71)

**Ziel:** Secrets nie mehr im Klartext in der DB (und damit nie im JSON-Export/GitHub). Je Firma eine eigene, verschlüsselte Datei; Passwort auto-generiert, nie sichtbar; Reset nur über Löschen der Datei.

**Umgesetzt:**
- `app/key_store.py` (neu): verschlüsselte Pro-Firma-Ablage `app/daten/api_keys_{firmen_nr}.json` (Fernet + PBKDF2-HMAC-SHA256, Salt je Datei; Hülle `{salt, token}`, Payload `{firma:{…}, lokal:{…}}`). API: `neues_passwort`, `lade`, `speichere` (merged mit Bestand, atomar via tmp+`os.replace`), `datei_existiert`, `loesche_datei`, `status`, `SECRET_FELDER` (11 firma-Spalten, Single Source). Fehlerfälle nie still: fehlende Datei → leer (kein Log); defekt/falsches Passwort → leer + `fallback_log.melde` (nie auto-löschen).
- `app/db/db_firma.py`: transparente Umleitung. `get_firma` liefert jetzt **dict** (statt Row) mit hineingemergten Secrets, ohne `api_keys_passwort`. `save_firma`/`save_firma_ki_lokal` poppen Secrets → Passwort-Lebenszyklus (fehlt Passwort ODER Datei → neues Passwort + frische Datei) → `key_store.speichere`; DB-Spalten bleiben ''. `get_firma_ki_lokal` mergt `api_key` je Slot aus der Datei. `copy_firma` erzeugt eigenes neues Passwort + eigene Datei (Secrets nie in die DB kopiert). `hard_delete_firma` (komplett) löscht die Datei nach erfolgreichem Commit. Neue Methode `schluesseldatei_status`.
- `app/db_importexport.py`: Export scrubbt die 11 Secret-Felder **und** `api_keys_passwort` auf '' (garantiert key-frei). Import strippt dieselben Felder und bewahrt das vorhandene Ziel-Passwort je Firmennummer (Schlüsseldateien bleiben lesbar).
- Migration **DB v71** (`app/DB-Pflege.py` `_to_v71` + `app/db/db_schema.py`): neue Spalte `firma.api_keys_passwort`; je Firma mit ≥1 nicht-leerem Secret → Passwort erzeugen, Datei schreiben (firma- + lokal-Teil), 12 Quell-Spalten leeren. Idempotent (2. Lauf No-op). Header-Changelog + CURRENT_VERSION=71 + MIGRATIONEN.
- Firmenstamm → Parameter → Steuerung: Status-Zeile „Schlüsseldatei (API-Keys)" (vorhanden/fehlt/defekt, defekt rot). Neue i18n-Keys `firma.steuerung.keydatei*` (DE+EN).
- `.gitignore`: `app/daten/api_keys_*.json`. `Readme.admin.de.md`/`.en.md`: Abschnitt zu verschlüsselten Secrets (Datensicherung/Umzug/Reset) + Import/Export-Hinweise + .gitignore-Liste.
- `app/test_key_store.py` (neu): 9 Tests (Roundtrip, falsches Passwort → leer + Log, fehlende/defekte Datei, Merge, kein Klartext in Datei, atomar, Passwörter verschieden, status).
