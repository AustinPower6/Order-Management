...
- [ ] (2026-07-10) E-Rechnung: Storno jetzt mit positiven Beträgen; Befreiungsgrund aus Klassen-Hinweistext; neue gelbe Spool-Fälle
  - Code: `app/e_rechnung/ubl_2_1.py`/`cii_d16b.py` (`_storno_positiv`, Kategorie-E-`TaxExemptionReason`, `_summen`), `app/e_rechnung/__init__.py` (`_pruefe_und_protokolliere_fallbacks`)
  - Doku (Kapitel E-Rechnung/E-Rechnung-Spool): Drei Ergänzungen. **1. Storno:** Die E-Rechnung einer Stornorechnung weist jetzt **positive** Beträge aus und kennzeichnet die Gutschrift allein über den Rechnungstyp-Code 381 plus Verweis auf die Originalrechnung — bewusste Abweichung vom gedruckten PDF, das weiterhin Negativbeträge zeigt (EN-16931-üblich). **2. Steuerbefreiung:** Bei 0 %-Positionen (ohne igL) übernimmt die E-Rechnung den **Hinweistext der MwSt-Klasse** als Steuerbefreiungsgrund; ist keiner gepflegt, wird ersatzweise „Steuerbefreite Leistung" eingesetzt. **3. Gelbe Spool-Zeilen / Fehler-Nachverfolgung**, neue Fälle: 0 %-Klasse ohne Hinweistext (Ersatztext verwendet), Firmenname leer, USt-IdNr und Steuernummer beide leer, bei XRechnung zusätzlich Telefon leer — jeweils mit Hinweis, das Feld im Firmenstamm bzw. den Hinweistext in den MwSt-Klassen zu pflegen. Qualitätshinweis ohne Bedienungsänderung: Zeilen- und Gesamtbeträge der XML werden normkonform gerundet und können dadurch in seltenen Fällen um 1 Cent vom gedruckten PDF abweichen; das Fälligkeitsdatum der E-Rechnung entspricht immer dem festgeschriebenen Beleg (spätere Änderung der Zahlungskondition wirkt nicht zurück).
...
- [ ] (2026-07-10) Sprach-Generator: keine Kontroll-Rückübersetzung mehr nach übernommener Korrektur
  ...
  - Doku (ebd., Abschnitt Farberklärung/Tabelle, falls beschrieben): neue Kennzeichnung ergänzen — weicht die Rückübersetzung vom Original ab, erscheinen Original- **und** Rück-Spalte **kursiv** (nur kursiv; kursiv-fett bleibt „von der KI geändert"), auch bei grün bestätigten Zeilen; neue Legende-Zeile `dlg.sprachdatei.legende_kursiv_rueck`.
