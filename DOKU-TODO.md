...
- [ ] (2026-06-15) igL-Rechnung: Kunden-USt-IdNr, Pflicht-Hinweistext, Voraussetzungsprüfung (DB v37)
  - Code: DB v37 (`mwst_klassen.hinweis_text`/`igl`), `DB-Pflege.py::_to_v37`, `db_config.py::save_mwst_klasse`, `mod_mwst.py` (Hinweistext-Feld + igL-Checkbox), `druck.py` (`_beleg_info`/`kunde_ust_id`, `_sammle_steuerhinweise`, `steuerhinweis`-Druck, `_pruefe_igl_voraussetzungen`)
  - Doku: (1) MwSt-Reiter — beim Anlegen/Bearbeiten einer MwSt-Klasse die zwei neuen Felder „Hinweistext (Druck)" (erscheint unter den Summen auf dem Beleg) und „Innergem. Lieferung" (Checkbox: aktiviert die igL-Voraussetzungsprüfung) erklären. (2) Druck/Rechnung — die **Kunden-USt-IdNr** wird im Meta-Block gedruckt, sobald sie im Kundenstamm hinterlegt ist; der **Hinweistext** der verwendeten MwSt-Klasse erscheint unter den Summen (Kundenkopie übersetzt). (3) Wichtig: Ist eine MwSt-Klasse als igL gekennzeichnet, wird das **Drucken/Festschreiben einer Rechnung blockiert**, wenn die Voraussetzungen fehlen (Firmen- oder Kundenland am Belegdatum kein EU-Mitglied, Firmen-=Kundenland, oder fehlende Kunden-USt-IdNr) — mit Fehlermeldung, welche Bedingung fehlt. Der Testdruck blockiert nicht.
