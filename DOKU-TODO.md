...
- [ ] (2026-06-21) DATEV-Steuerschlüssel je MwSt-Klasse: Ist ein DATEV-Buchungsexportformat gewählt, erscheint im MwSt-Reiter (und im Klassen-Dialog) zusätzlich eine Spalte/ein Feld **„DATEV"** bzw. **„DATEV-Steuerschlüssel"**. Dort wird der DATEV-konforme Steuerschlüssel (BU-Schlüssel) je Steuerklasse hinterlegt; er wird beim Export als BU-Schlüssel (EXTF) bzw. `<buCode>` (Rechnungsdatenservice) verwendet. **Jede** verwendete Steuerklasse (auch steuerfreie) braucht einen Wert — fehlt er, **blockiert** der Buchungsexport mit Hinweis und protokolliert den Mangel. Bei JSON-Format ist die Spalte/das Feld ausgeblendet. Der interne Steuerschlüssel bleibt unverändert festgeschrieben.
  - Code: `app/mod_firma_tabs/mod_firma_mwst.py` (DATEV-Spalte), `app/modul/mod_mwst.py` (KlasseDialog-Feld), `app/buchungsexport_gen.py` (Anreicherung), `app/datev/extf.py` + `app/datev/rds.py` (BU-Schlüssel/buCode), `app/modul/mod_buchungsexport.py` (`_fehlende_datev_steuerschluessel`), `app/DB-Pflege.py`/`app/db/db_schema.py` (DB v44)
  - Doku: Kapitel „MwSt-Verwaltung" — DATEV-Steuerschlüssel-Spalte/-Feld beschreiben (nur bei DATEV-Format, je Klasse); Kapitel „Anbindung FiBu"/„Buchungsexport" — ergänzen, dass der DATEV-Export den hinterlegten DATEV-Steuerschlüssel nutzt und ohne ihn blockiert.

- [ ] (2026-06-21) Buchungsexport-Ausgabeformat: Im Reiter „Anbindung FiBu" lässt sich jetzt **firmenweit** das **Ausgabeformat** wählen — **JSON (Standard)**, **DATEV-Format (EXTF-Buchungsstapel, CSV)** oder **DATEV Rechnungsdatenservice (Belege, ZIP)**. Für die beiden DATEV-Ausgaben sind zusätzlich **DATEV-Berater-Nr.** und **DATEV-Mandanten-Nr.** zu erfassen (Pflicht); fehlen sie, blockiert der Buchungsexport mit Hinweis und protokolliert den Mangel.
  - **DATEV-Format (EXTF):** CSV-Buchungsstapel, als festgeschrieben gekennzeichnet; WJ-Beginn und Sachkontenlänge ergeben sich automatisch aus Geschäftsjahr/Kontenrahmen.
  - **DATEV Rechnungsdatenservice:** ZIP-Paket aus `document.xml`, je Beleg einer Belegdaten-XML und dem **festgeschriebenen Beleg-PDF** (Rechnungen **und** Mahnungen). Das PDF stammt aus dem letzten Originaldruck; fehlt es (Beleg nie gedruckt/festgeschrieben oder Datei verschoben), blockiert der Export mit Hinweis. Die XML wird vor dem Schreiben gegen die DATEV-Schemata geprüft.
  - Code: `app/mod_firma_tabs/mod_firma_anbindung_fibu.py` (Format-Combo + Berater-/Mandanten-Nr), `app/datev/extf.py` (EXTF), `app/datev/rds.py` + `app/datev/xsd/` (Rechnungsdatenservice + Schemata), `app/modul/mod_buchungsexport.py` (Format-Weiche `_schreibe_export`, `_datev_mangel`, `_fehlende_belegbilder`), `app/DB-Pflege.py`/`app/db/db_schema.py` (DB v43)
  - Doku: Kapitel „Anbindung FiBu" — neues Feld „Ausgabeformat" + DATEV-Berater-/Mandanten-Nr beschreiben; Kapitel „Buchungsexport" — erklären, dass das Format hier zentral gesteuert wird, dass der Rechnungsdatenservice die festgeschriebenen Beleg-PDFs einbezieht und der Export bei fehlenden DATEV-Stammdaten oder fehlendem Beleg-PDF blockiert.
...
- [ ] (2026-06-20) MwSt-Klassen: Der **Steuerschlüssel (Steuerklassennummer)** ist jetzt **festgeschrieben** — er wird nur bei der **Neuanlage einer MwSt-Klasse** vergeben und ist danach **nicht mehr änderbar**. Beim Bearbeiten eines Satzes ist das Feld gesperrt; ein weiterer Satz derselben Klasse **erbt** den Steuerschlüssel automatisch. Hintergrund: Der Buchungsexport ordnet das Erlöskonto über den (unveränderlichen) Steuerschlüssel zu, nicht über die (umbenennbare) Bezeichnung — so bleiben auch nach einer Umbenennung der MwSt-Klasse alle Belege korrekt buchbar.
  - Code: `app/modul/mod_mwst.py` (`SatzDialog`: Steuerschlüssel read-only, `_klassen_steuerschluessel`), `app/buchungsexport_gen.py` (`baue_buchungssaetze`/`_buchung_rechnung`/`_erloeskonto`: Auflösung über Steuerschlüssel)
  - Doku: Kapitel MwSt-Verwaltung (`#mwst`) — ergänzen, dass der Steuerschlüssel pro Klasse festgeschrieben ist (nur bei Neuanlage wählbar) und warum (Buchungsexport-Kontozuordnung). Im Kapitel „Buchungsexport"/„Anbindung FiBu" erwähnen, dass das Erlöskonto über den Steuerschlüssel der Klasse zugeordnet wird (umbenennen der Klasse ist unkritisch).

- [ ] (2026-06-20) Buchungsexport: Über der Export-Tabelle erscheint jetzt eine **Übersichts-Zeile** „Nicht exportierte Belege je Periode (MM·Anzahl)". Sie listet **jedes Jahr, in dem noch nicht exportierte buchungsrelevante Belege existieren**, je Jahr **alle 12 Monate** (auch mit 0), z. B. `2026   01·  3  02·  0 … 12·  0`. Sind alle Belege exportiert, steht dort „Alle Belege sind exportiert.". Die Zeile aktualisiert sich automatisch (nach Neuanlage, Storno, Aktualisieren).
  - Code: `app/db/db_buchungsexport.py` (`unexportiert_pro_periode_alle`), `app/modul/mod_buchungsexport.py` (`_update_info_zeile`, Label über der Tabelle); `app/language.json` (`dlg.buchungsexport.uebersicht_titel`, `dlg.buchungsexport.uebersicht_leer`)
  - Doku: Kapitel „Buchungsexport" (Anchor `buchungsexport`) — die Übersichts-Zeile beschreiben (zeigt offene Belege je Periode über alle Jahre; MM·Anzahl; nur Jahre mit offenen Belegen, darin alle 12 Monate). Abgrenzung zum Hinweis im „Neuer Export"-Dialog (der nur das gewählte Jahr betrifft).
