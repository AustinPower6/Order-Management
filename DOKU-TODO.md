...
- [ ] (2026-07-03) DSGVO-Funktionen für Kundendaten (Anonymisierung/Löschung, Auskunft, Sammellauf, Aufbewahrungsfrist)
  - Code: `app/db/db_kunden.py` (`kunde_fuer_beleg`, `frist_offen`, `anonymisiere_kunde`, `verarbeitung_einschraenken`, `dsgvo_auskunft`, `dsgvo_sammellauf_kandidaten`, `anonymisiere_kunden_batch`), `app/dsgvo_export.py` (neu), `app/modul/mod_kunden.py` (kundenspezifisches DSGVO-Menü), `app/modul/mod_dsgvo_sammellauf.py` (neu, Sammellauf-Dialog + Ablauf), `app/main.py` (Auswertungen-Menü → DSGVO-Sammellauf), `app/mod_firma_tabs/mod_firma_steuerung.py` (Feld Aufbewahrungsfrist), `app/mod_firma_tabs/mod_firma_pfade.py` (DSGVO-Pfad), DB v57–v59 (`kunde_snapshot`, `firma.aufbewahrung_jahre`, `kunden.dsgvo_status/dsgvo_am`, `firma.dsgvo_pfad`)
  - Doku (Kapitel Kundenstamm, id `kunden`): Neuer Button „DSGVO" mit **kundenspezifischem** Menü. **Auskunft (PDF + JSON)** erzeugt zum ausgewählten Kunden eine Datenauskunft (Art. 15) und einen maschinenlesbaren Export (Art. 20) mit Stammdaten + Belegliste. **Anonymisieren/Löschen** (Art. 17) entfernt die personenbezogenen Stammdaten unwiderruflich — festgeschriebene Belege behalten über einen eingefrorenen Datenstand ihre ursprüngliche Anschrift (§14 UStG). Solange für die jüngsten Belege eines Kunden die **Aufbewahrungsfrist** (Firmenstamm → Parameter → Steuerung → „Aufbewahrungsfrist (Jahre)", Standard 10) noch nicht abgelaufen ist, ist nur **Verarbeitung einschränken** (Art. 18) möglich; das System weist darauf hin. Anonymisierte/eingeschränkte Kunden sind in der Kundenliste per Tooltip gekennzeichnet.
  - Doku (Kapitel Auswertungen): Menüpunkt **„DSGVO-Sammellauf …"** (firmenweit, nicht kundenbezogen) listet alle fristfälligen, inaktiven Kunden zur Auswahl und anonymisiert die bestätigten in einem Rutsch (Jahreslauf); danach entsteht ein pseudonymes **Protokoll (PDF + JSON)**. Ablageort der DSGVO-Dateien: Firmenstamm → Pfade → „Verzeichnis für DSGVO-Exporte" (leer = `{Exportpfad}\DSGVO\`).
